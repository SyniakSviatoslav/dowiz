...
/**
 * Hotfix · read_public_menu_all_locales referenced non-existent modifier columns.
 *
 * The all-locales (crawler/SSR) function inherited from 0016 — and re-emitted by
 * 0033 — joined modifiers on `modifier_group_id` and read `m.is_available`, but the
 * modifiers table columns are `group_id` and `available`. This was latent because
 * 0016 also gated the function on status='active', so the demo (status='open')
 * always returned 404 before reaching the bad SQL. 0033 fixed that gating, which
 * exposed the column bug as a 500 on `/s/:slug` for crawlers. Fix: use the real
 * column names (group_id / available), matching the working read_public_menu.
 */
...
export async function down(): Promise<void> {
  // No-op: corrected definition; reverting would reintroduce the bad column refs.
}
