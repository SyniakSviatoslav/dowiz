...
async function runTests() {
  ...
  try {
    ...
    console.log('Testing JWT Forgery...');
    const fakeToken = ownerToken.replace(/\.[^.]+$/, '.forged_signature');
    const resFake = await fetch(`http://127.0.0.1:3003/orders/${data1.id}/status`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${fakeToken}` },
      body: JSON.stringify({ status: 'CONFIRMED' })
    });
    if (resFake.status !== 401) throw new Error('JWT Forgery bypassed auth!');
    console.log('✅ JWT Forgery blocked with 401');

    console.log('Testing Transactional Enqueue Rollback...');
    // Try to create an order with an invalid product ID (non-existent). It should 422/400.
    const invalidPayload = { ...orderPayload, items: [{ productId: crypto.randomUUID(), quantity: 1 }], idempotencyKey: crypto.randomUUID() };
    const resInv = await fetch(`http://127.0.0.1:3003/orders`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(invalidPayload)
    });
    if (resInv.status === 201) throw new Error('Invalid order succeeded');
    // Verify no jobs in pg-boss
    const jobsRes = await pool.query(`SELECT count(*) as count FROM pgboss.job WHERE name = 'order.timeout'`);
    // Note: jobsRes might include the valid job from earlier, so we check total. It should be 1.
    if (parseInt(jobsRes.rows[0].count, 10) > 1) throw new Error('Transactional Enqueue leaked a job!');
    console.log('✅ Transactional Enqueue rollback verified');
