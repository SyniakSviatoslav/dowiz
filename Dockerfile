...
# Menu Characteristics layer (ADR-0014) — storefront render flags baked at vite build time. All default
# false so prod stays DARK; staging passes --build-arg VITE_MENU_CHARACTERISTICS_COMPARISON=true (etc.) to
# verify the surfaces before launch. STEP-0 (the allergen single-source fix) is unconditional, not flagged.
ARG VITE_MENU_CHARACTERISTICS_ENABLED=false
ENV VITE_MENU_CHARACTERISTICS_ENABLED=$VITE_MENU_CHARACTERISTICS_ENABLED
ARG VITE_MENU_CHARACTERISTICS_COMPARISON=false
ENV VITE_MENU_CHARACTERISTICS_COMPARISON=$VITE_MENU_CHARACTERISTICS_COMPARISON
ARG VITE_MENU_CHARACTERISTICS_FILTER=false
ENV VITE_MENU_CHARACTERISTICS_FILTER=$VITE_MENU_CHARACTERISTICS_FILTER
ARG VITE_MENU_ALLERGEN_FILTER=false
ENV VITE_MENU_ALLERGEN_FILTER=$VITE_MENU_ALLERGEN_FILTER
RUN pnpm -r build
