...
test('/admin/login no longer renders blank — redirects to the real login form', async ({ page }) => {
  ...
  // A real login form renders (was 0 inputs/0 buttons before the fix).
  await expect(page.locator('input').first(), 'login form has inputs').toBeVisible();
  await expect(page.locator('button').first(), 'login form has a submit control').toBeVisible();
});
