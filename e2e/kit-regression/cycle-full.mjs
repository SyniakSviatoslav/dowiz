...
const OUT = process.env.OUT || '/tmp/dowiz-cycle';
...
await c.screenshot({ path: `${OUT}/cycle-1-placed.png` }).catch(e => step('screenshot', false, e.message.slice(0, 90)));
...
await o.screenshot({ path: `${OUT}/cycle-2-console.png` }).catch(e => step('screenshot', false, e.message.slice(0, 90)));
...
//
// THE COURIER MUST BELONG TO THE VENUE UNDER TEST, and this is checked BEFORE
// a browser is opened, because the failure it prevents does not look like
// itself. A courier of another venue used to sign in here quite happily, open
// a shift, and then show that other venue's pool for ever -- so the run
...
const CPASS = process.env.QA_COURIER_PASSWORD || creds.QA_COURIER_PASSWORD || creds.COURIER_PASSWORD;
{
  ...
  // The host names a SLUG, and a venue's slug is not its id -- `dubin-sushi`
  // is `dubin-durres`. Comparing the two directly would pass here by
  // coincidence and mislead on the other venue, so the id is asked for.
  ...
  step('the courier belongs to the venue under test', r.status === 200 && !!venue && at === venue,
    r.status === 200 ? `courier is at ${at ?? '?'}, this venue is ${venue ?? '?'}`
                     : `login ${r.status}: ${JSON.stringify(r.body).slice(0, 120)}`);
}
...
await k.fill('#em', CPHONE);
await k.fill('#pw', CPASS);
await k.click('#go');
...
await k.screenshot({ path: `${OUT}/cycle-3-courier.png` }).catch(e => step('screenshot', false, e.message.slice(0, 90)));
...
await c.screenshot({ path: `${OUT}/cycle-4-delivered.png` }).catch(e => step('screenshot', false, e.message.slice(0, 90)));
