...
export default (async function courierShiftsRoutes(fastify: any, opts: any) {
  ...
  fastify.post('/api/courier/shifts/ping', {
    ...
    const courierId = request.user.sub;
    const locationId = request.user.activeLocationId;
    const { lat, lng, accuracy_meters } = request.body;
    
    // GPS Geofence Check (assume center from a db or env lookup, for simplicity here, just mocked out center)
    const client = await db.connect();
    try {
      await client.query(`SET LOCAL app.current_tenant = $1`, [locationId]);
