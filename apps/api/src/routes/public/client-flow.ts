...
export default (async function clientFlowRoutes(fastify: any, opts: any) {
  ...
  async function getThemeAndCSP(slug: string, reply: any) {
    ...
    const csp = `default-src 'self'; img-src 'self' data: https:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdn.jsdelivr.net; font-src 'self' https://fonts.gstatic.com https://cdn.jsdelivr.net; script-src 'self' 'unsafe-inline' 'unsafe-eval'; worker-src 'self' blob:; connect-src 'self' https://cdn.jsdelivr.net https://tiles.openfreemap.org https://router.project-osrm.org https://en.wikipedia.org; frame-ancestors ${frameAncestors}`;
