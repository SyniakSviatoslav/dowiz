...
import { isBot } from '../spa-shell.js';
import { NODE_KEEP_ROUTES } from './node-keep.generated.js';
...
const S1_NODE_KEEP_TEMPLATES: ReadonlySet<string> = new Set([
  ...
  // SPA-fallback page (server.ts SPA_ROUTES) — Node serves index.html; the Rust api has no
  // SPA shell for it by scope-cut design (S1 sweep 2026-07-05: node=200, rust=404).
  '/branding-preview/:slug',
]);
...
export function registerCutoverFrontDoor(
  ...
  fastify.addHook('onRequest', async (request, reply) => {
    // Astro build assets (/_astro/*): the Astro menu page references its own hashed
    // bundles, which are unmapped in ROUTE_TEMPLATES (static-asset class) — without this
    // forward they fall through to the API JSON 404 and every storefront page renders
    // UNSTYLED (staging visual regression, 2026-07-05). Assets are inert bytes: safe to
    // serve regardless of the S1 flag (stale HTML during a degrade window still styles);
    // bodyless GET/HEAD → pre-response upstream failure falls through to Node (404),
    // the same safe direction as the page forward itself.
    const assetPath = (request.raw.url ?? request.url).split('?')[0] ?? '';
    if (astro && assetPath.startsWith('/_astro/') && isBodyless(request.method)) {
      await forwardToRust(request, reply, astro, 'S1', { warn: (o, m) => log.warn(o, m) }, 'astro');
      return;
    }
    ...
    // Per-route strangler gate (node-keep.generated.ts): a surface flip forwards ONLY the
    // routes the Rust tree actually mounts — mapped-but-unmounted routes stay on Node until
    // they exist (regenerating the keep list shrinks it as the port grows). Without this, the
    // S3/S5 surface flips 404'd ~45 working owner/customer routes (staging oracle 2026-07-05).
    const keepTemplate = match.template?.template;
    if (keepTemplate !== undefined && NODE_KEEP_ROUTES.has(`${request.method} ${keepTemplate}`)) {
      return;
    }

    // S1 HTML partition (Q6 sub-target): menu page → Astro for HUMANS; unimplemented
    // storefront pages → Node (explicit keep); everything else S1 (API reads, manifest,
    // media) → Rust. Bot UAs on the menu page skip Astro and take the Rust forward below:
    // UA-partitioning is this page's pre-existing Node semantic (spa-shell.ts BOT_UA →
    // SSR JSON-LD), the Rust surface ports that JSON-LD path (storefront.rs bot_menu_html,
    // proven in the S1 mechanism proof), and the Astro app does not emit JSON-LD yet —
    // routing bots to Astro silently drops the schema.org payload (live-oracle catch,
    // 2026-07-05). Same isBot as Node's own shell = single source of truth.
    const template = match.template?.template;
    if (surface === 'S1' && template !== undefined) {
      if (S1_NODE_KEEP_TEMPLATES.has(template)) return;
      if (S1_ASTRO_TEMPLATES.has(template) && !isBot(request.headers['user-agent'])) {
