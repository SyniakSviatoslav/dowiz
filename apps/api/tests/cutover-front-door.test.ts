...
describe('front-door routing (dark-by-default, forward, fail modes)', () => {
  ...
  test('S1 Astro partition: page → astro, API → rust, unimplemented page → Node (Q6 sub-target)', async () => {
    ...
    try {
      ...
      const branding = await fetch(`${base}/branding-preview/demo`);
      assert.equal(
        branding.headers.get('x-dowiz-cutover'),
        null,
        'SPA-fallback branding-preview stays Node (rust has no SPA shell — sweep catch)',
      );
    } finally {
      await app.close();
      await rust.close();
  ...
  test('/_astro/* build assets forward to the Astro upstream (flag-independent, GET-only) — unstyled-storefront regression 2026-07-05', async () => {
    // The Astro page HTML references its own hashed bundles under /_astro/. They are
    // unmapped in ROUTE_TEMPLATES, so without the explicit passthrough they fell through
    // to the API JSON 404 and EVERY storefront page rendered unstyled (staging incident).
    const rust = await makeUpstream();
    const astro = await makeUpstream();
    const state: FakePoolState = {
      rows: [{ surface: 'S1', target: 'rust', readiness_ok: true }],
    ...
    try {
      await waitFor(() => handle.flags.snapshot().has('S1'));
      const css = await fetch(`${base}/_astro/_slug_.CXu18Y-u.css`);
      assert.equal(css.status, 200, 'asset must be served, not fall through to the API 404');
      assert.equal(css.headers.get('x-dowiz-cutover'), 'astro:S1', 'asset routes to Astro');
      assert.ok(astro.seen.some((s) => s.method === 'GET' && s.url === '/_astro/_slug_.CXu18Y-u.css'));

      // Flag-independent: S1 degraded to node ⇒ stale Astro HTML in a browser still styles.
      state.rows = [{ surface: 'S1', target: 'node', readiness_ok: true }];
      await waitFor(() => handle.flags.targetFor('S1') === 'node');
      const cssAfterDegrade = await fetch(`${base}/_astro/after-degrade.css`);
      assert.equal(cssAfterDegrade.headers.get('x-dowiz-cutover'), 'astro:S1', 'assets survive a surface degrade');

      // Bodied methods are never proxied to the asset host.
      const post = await fetch(`${base}/_astro/evil.css`, { method: 'POST' });
      assert.equal(post.headers.get('x-dowiz-cutover'), null, 'POST must not forward');
      assert.equal(astro.seen.some((s) => s.method === 'POST'), false);
    } finally {
      await app.close();
      await rust.close();
      await astro.close();
  ...
  test('/_astro/* without an Astro upstream stays inert on Node (dark default)', async () => {
    const rust = await makeUpstream();
    const state: FakePoolState = {
      rows: [{ surface: 'S1', target: 'rust', readiness_ok: true }],
      failSelects: false,
    ...
    try {
      await waitFor(() => handle.flags.snapshot().has('S1'));
      const css = await fetch(`${base}/_astro/x.css`);
      assert.equal(css.headers.get('x-dowiz-cutover'), null, 'no upstream → Node keeps the request');
  ...
  test('S1 Astro partition: bot UA on the menu page goes to RUST, not Astro (JSON-LD parity)', async () => {
    // Node's own /s/:slug semantic is UA-partitioned (spa-shell.ts BOT_UA → SSR JSON-LD);
    // the Rust surface ports that JSON-LD path, the Astro app does not emit it yet. Routing
    // a crawler to Astro silently drops the schema.org payload (live-oracle catch 2026-07-05).
    const rust = await makeUpstream();
    const astro = await makeUpstream();
    const state: FakePoolState = {
      rows: [{ surface: 'S1', target: 'rust', readiness_ok: true }],
    ...
    try {
      ...
      assert.equal(page.headers.get('x-dowiz-cutover'), 'rust:S1', 'bot menu page routes to Rust');
      assert.ok(rust.seen.some((s) => s.url === '/s/demo'), 'Rust upstream saw the bot request');
      assert.equal(astro.seen.some((s) => s.url === '/s/demo'), false, 'Astro never sees bot UAs');
    } finally {
      await app.close();
      await rust.close();
      await astro.close();
  ...
  test('per-route strangler gate: unmounted mapped routes stay Node under a flipped surface', async () => {
    // node-keep.generated.ts: S5=rust must forward ONLY what the Rust tree mounts. The
    // settlements family is mapped S5 but has no Rust mount (staging oracle 2026-07-05:
    // the surface-level flip 404'd it); the orders core IS mounted and must forward.
    const rust = await makeUpstream();
    const state: FakePoolState = {
      rows: [{ surface: 'S5', target: 'rust', readiness_ok: true }],
      failSelects: false,
      degradeCalls: [],
    };
    const { app, handle, base } = await makeApp({ state, rustUpstream: rust.url });
    try {
      await waitFor(() => handle.flags.snapshot().has('S5'));
      const kept = await fetch(`${base}/api/owner/locations/11111111-1111-1111-1111-111111111111/settlements`);
      assert.equal(kept.headers.get('x-dowiz-cutover'), null, 'unmounted mapped route stays Node');
      assert.equal(rust.seen.some((s) => s.url.includes('settlements')), false);
      const forwarded = await fetch(`${base}/api/orders/11111111-1111-1111-1111-111111111111`);
      assert.equal(forwarded.headers.get('x-dowiz-cutover'), 'rust:S5', 'mounted route forwards');
    } finally {
      await app.close();
      await rust.close();
    }
  ...
  test('tripped health: money surface answers 503 untouched, non-money degrades globally (REV-C5)', async () => {
    ...
    try {
      ...
      // Money: never silently rerouted — truthful 503. Must be a MOUNTED money route
      // (the settlements family is node-keep-listed until its Rust mount exists, so it
      // never reaches the health gate).
      const money = await fetch(`${base}/api/orders`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: '{}',
      });
