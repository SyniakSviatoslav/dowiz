...
/// JS-number serialization for f64 wire fields (S1 staging oracle 2026-07-05): `JSON.stringify`
/// emits integral Numbers WITHOUT a decimal point (`0`, `5`) while serde emits f64 as `0.0`,
/// `5.0` — a real leaf diff on the parity oracle (taxRate, and any integral lat/lng/rating).
/// Integral in-range values serialize as JSON integers, everything else as the f64 it is.
pub(crate) fn serialize_js_number<S: serde::Serializer>(v: &f64, s: S) -> Result<S::Ok, S::Error> {
    const MAX_EXACT: f64 = 9_007_199_254_740_992.0; // 2^53 — JS integer-exactness bound
    if v.fract() == 0.0 && v.is_finite() && v.abs() <= MAX_EXACT {
        #[allow(
            clippy::as_conversions,
            reason = "guarded: fract()==0, finite, |v| <= 2^53 — exact i64 conversion"
        )]
...
/// `Option<f64>` variant of [`serialize_js_number`] (`null` stays `null`).
pub(crate) fn serialize_js_number_opt<S: serde::Serializer>(
    v: &Option<f64>,
    s: S,
) -> Result<S::Ok, S::Error> {
    match v {
        Some(inner) => serialize_js_number(inner, s),
        None => s.serialize_none(),
    }
}

/// JS `Date.toISOString()` wire parity for timestamps (staging oracle 2026-07-05): node-postgres
/// parses `timestamptz` into a JS `Date`, so EVERY Node wire timestamp is `toISOString()` — exactly
/// 3-digit milliseconds + trailing `'Z'` (`"2026-06-20T19:36:37.915Z"`). chrono's serde default
/// emits microseconds (`"...37.915496Z"`) — 49 leaf diffs on GET /api/owner/menu/products
/// (`createdAt`), and the same class on every other wire-serialized `DateTime<Utc>`.
pub(crate) fn serialize_js_instant<S: serde::Serializer>(
    v: &chrono::DateTime<chrono::Utc>,
    s: S,
) -> Result<S::Ok, S::Error> {
    s.serialize_str(&v.to_rfc3339_opts(chrono::SecondsFormat::Millis, true))
}

/// `Option<DateTime<Utc>>` variant of [`serialize_js_instant`] (`null` stays `null`).
pub(crate) fn serialize_js_instant_opt<S: serde::Serializer>(
    v: &Option<chrono::DateTime<chrono::Utc>>,
    s: S,
) -> Result<S::Ok, S::Error> {
    match v {
        Some(inner) => serialize_js_instant(inner, s),
        None => s.serialize_none(),
    }
}
...
pub struct PublicLocationInfo {
    ...
    /// PARITY (S1 staging oracle 2026-07-05): JS serializes Number 0 as `0`, serde serializes
    /// f64 0.0 as `0.0` — integral values must emit as JSON integers like the source.
    ...
    #[serde(rename = "googleRating", serialize_with = "serialize_js_number_opt")]
...
mod tests {
    ...
    /// The JS-number wire pin (S1 staging oracle): integral f64s serialize WITHOUT a decimal
    /// point (`"taxRate":0`, `"googleRating":5`), non-integral keep their fraction, null stays
    /// null — exactly `JSON.stringify`'s Number emission.
    #[test]
    fn js_number_fields_serialize_like_json_stringify() {
        #[derive(Serialize)]
        struct Probe {
        ...
        .unwrap();
        assert_eq!(json, r#"{"a":0,"b":0.155,"c":5,"d":null}"#);
    }

    /// The JS `Date.toISOString()` wire pin (staging oracle 2026-07-05): exactly 3-digit
    /// milliseconds (microseconds TRUNCATED, `.915496` -> `.915`), trailing `Z` (never `+00:00`),
    /// and `None` stays `null`.
    #[test]
    fn js_instant_fields_serialize_like_date_to_iso_string() {
        #[derive(Serialize)]
        struct Probe {
            #[serde(serialize_with = "serialize_js_instant")]
            a: chrono::DateTime<chrono::Utc>,
            #[serde(serialize_with = "serialize_js_instant_opt")]
            b: Option<chrono::DateTime<chrono::Utc>>,
            #[serde(serialize_with = "serialize_js_instant_opt")]
            c: Option<chrono::DateTime<chrono::Utc>>,
        }
        let ts = chrono::DateTime::parse_from_rfc3339("2026-06-20T19:36:37.915496Z")
            .unwrap()
        ...
        assert_eq!(
            json,
            r#"{"a":"2026-06-20T19:36:37.915Z","b":"2026-06-20T19:36:37.915Z","c":null}"#
        );
    }
