...
fn build_owner_states(
    ...
    routes::owner::OwnerCatalogStates {
        ...
        dwell_settings: routes::owner::dwell_settings::DwellSettingsState {
            auth: auth.clone(),
            repo: Arc::new(routes::owner::dwell_settings::PgDwellSettingsRepo::new(
                pools.operational.clone(),
            )),
        },
        fallback_settings: routes::owner::fallback_settings::FallbackSettingsState {
            auth: auth.clone(),
            repo: Arc::new(
                routes::owner::fallback_settings::PgFallbackSettingsRepo::new(
                    pools.operational.clone(),
                ),
            ),
        },
        onboarding: routes::owner::onboarding::OnboardingStateSvc {
            auth: auth.clone(),
            repo: Arc::new(routes::owner::onboarding::PgOnboardingRepo::new(
                pools.operational.clone(),
            )),
        },
        owner_settings: routes::owner::owner_settings::SettingsState {
            auth: auth.clone(),
            repo: Arc::new(routes::owner::owner_settings::PgSettingsRepo::new(
                pools.operational.clone(),
            )),
        },
        // R2a batch: owner location PATCH + ADR-0014 allergen flag-confirm.
        locations: routes::owner::locations::LocationsState {
            auth: auth.clone(),
            repo: Arc::new(routes::owner::locations::PgLocationsRepo::new(
                pools.operational.clone(),
            )),
        },
        menu_confirm: routes::owner::menu_confirm::MenuConfirmState {
            auth,
            repo: Arc::new(routes::owner::menu_confirm::PgMenuConfirmRepo::new(
                pools.operational.clone(),
            )),
        },
    }
