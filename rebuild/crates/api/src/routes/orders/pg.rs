...
impl OrdersRepo for PgOrdersRepo {
    async fn create_order(&self, cmd: CreateOrderCommand) -> Result<CreateOutcome, RepoError> {
        ...
        let loc: Option<LocationRow> = sqlx::query_as(
            "SELECT lat, lng, published_at::text, tax_rate, price_includes_tax,
        ...
        match state::idempotency_decision(existing_key.as_ref(), &request_hash) {
            ...
            IdempotencyDecision::Replay => {
                ...
                let row: Option<(Uuid, String, i64, i64, String, Option<String>)> = sqlx::query_as(
                    "SELECT id, status::text, subtotal::bigint, total::bigint,
                            created_at::text, timeout_at::text FROM orders WHERE id = $1",
                )
                .bind(order_id)
                .fetch_optional(&mut *txn)
                .await?;
        ...
        let order_row: Result<(Uuid, String, i64, i64, String, Option<String>), sqlx::Error> =
            sqlx::query_as(
                "INSERT INTO orders
                   (location_id, customer_id, type, status, delivery_address, delivery_lat, delivery_lng,
                    subtotal, total, payment_method, cash_pay_with, metadata)
                 VALUES ($1,$2,$3,'PENDING',$4,$5,$6,$7::integer,$8::integer,'cash',$9::integer,$10)
                 RETURNING id, status::text, subtotal::bigint, total::bigint,
        ...
        crate::db::with_user(&self.pool, owner_user_id, |txn| {
            Box::pin(async move {
                ...
                let cur: Option<(String, Uuid, String)> = sqlx::query_as(
                    "SELECT o.status::text, o.location_id, o.type
                       FROM orders o
...
async fn apply_transition(
    ...
    let hist: Result<_, sqlx::Error> = sqlx::query(
        "INSERT INTO order_status_history (order_id, location_id, from_status, to_status, actor, comment)
         VALUES ($1, $2, $3, $4, 'system:updateOrderStatus', NULL)",
    )
    .bind(order_id)
    .bind(location_id)
    .bind(current_str)
...
type CreatedRow = (Uuid, String, i64, i64, String, Option<String>);
...
fn order_row(r: CreatedRow) -> Result<OrderCreatedResponse, RepoError> {
    let (id, status_str, subtotal, total, created_at, timeout_at) = r;
...
mod tests {
    ...
    async fn anonymous_create_admitted_only_with_no_app_user_id() {
        ...
        let admitted = {
            ...
            let res = sqlx::query(
                "INSERT INTO orders (location_id, type, status, subtotal, total, payment_method)
                 VALUES ($1, 'pickup', 'PENDING', 0, 0, 'cash')",
            )
            .bind(location_id)
            .execute(&mut *txn)
            .await;
