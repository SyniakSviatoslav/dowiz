...
//! This port hashes `pin.lat`/`pin.lng` as the **integer** `round(coord · 1e5)` (an `i64`), NOT the
//! re-divided float. An integer serializes byte-identically under V8 `JSON.stringify` and serde_json
//! (`42`, never `42.0`; no `-0`; no ryu shortest-float ambiguity) — so the canonicalization is
//! provably byte-identical across both stacks BY CONSTRUCTION, proven by a property test over the
//! float domain ([`tests::projection_is_stable_over_the_float_domain`]), not by one vector.
//!
...
//! a false-422 (annoying, retryable), never a duplicate order and never a duplicate charge (cash does
//! not charge at create; crypto stays dark). Cross-stack byte-identity is a cutover-harness gate
//! (REV-S5-8): Node adopts this same integer projection before the S5 flip.
...
pub struct CanonicalRequestInput {
    ...
    /// Delivery pin (`None` for pickup) — raw floats; projected to integers inside.
...
/// `super::round_f64_to_i64`, the one confined f64→i64 cast site), matching Node `Math.round` for
/// the small magnitudes coordinates ever take (|lat| ≤ 90, |lng| ≤ 180 → |coord·1e5| ≤ 1.8e7,
/// exactly representable in f64, so no rounding ambiguity).
...
/// SHA-256 of the canonical JSON body, lowercase hex.
pub fn build_request_hash(input: &CanonicalRequestInput) -> String {
    ...
    let pin = input.pin.map(|(lat, lng)| CanonicalPin {
        lat: project_coord(lat),
        lng: project_coord(lng),
    });
...
mod tests {
    ...
    fn base_input() -> CanonicalRequestInput {
        CanonicalRequestInput {
            ...
            pin: Some((41.327_953, 19.819_025)),
    ...
    fn modifier_order_does_not_change_the_hash() {
        ...
        assert_eq!(
            build_request_hash(&base_input()),
            build_request_hash(&swapped)
        );
    }
    ...
    fn pin_is_serialized_as_integers_no_float_token() {
        ...
        // 41.327953 → round(4132795.3) = 4132795 ; 19.819025 → round(1981902.5) = 1981903 (half-up).
        assert!(
    ...
    /// float domain, not a single golden vector" the RESOLVE mandates.
    #[test]
    ...
    /// the fingerprint actually incorporates identity (a `.userId` read yielded `undefined` and
    /// silently degraded it; we read `.sub`).
    #[test]
