...
/// The create response — mirrors the Node replay/insert select shape (`orders.ts:406`,
/// `insertOrderWithItems`): `{ id, status, subtotal, total, created_at, timeout_at }`. Money fields
...
pub async fn create_order(
    ...
    let outcome = state.repo.create_order(cmd).await.map_err(|_e| {
        ApiError::new(
            ErrorCode::Internal,
            "internal_error",
...
#[utoipa::path(
    patch, path = "/orders/{id}/status", tag = "orders",
    params(("id" = Uuid, Path)),
...
pub fn orders_router(state: OrdersState) -> axum::Router {
    ...
    axum::Router::new()
        ...
        .route("/orders/{id}/status", patch(owner_update_status))
        .route(
...
mod handler_tests {
    ...
    async fn get_order_customer_cross_order_is_404() {
        ...
        let resp = app
            .oneshot(
                Request::builder()
                    .uri(format!("/orders/{}", Uuid::new_v4())) // a DIFFERENT order
                    .header("authorization", format!("Bearer {token}"))
                    .body(Body::empty())
                    .unwrap(),
            )
