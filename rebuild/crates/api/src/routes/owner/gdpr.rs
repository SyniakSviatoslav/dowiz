...
/// .toString('base64url')` (`gdpr.ts:192`). `JSON.stringify` on a pg-parsed `Date` goes through
/// `toISOString()` — millis-precision + trailing `Z`, hence `to_rfc3339_opts`, not `to_rfc3339`.
fn encode_cursor(requested_at: DateTime<Utc>) -> String {
    use base64::Engine;
    let json = serde_json::json!({
        "requestedAt": requested_at.to_rfc3339_opts(chrono::SecondsFormat::Millis, true)
    });
