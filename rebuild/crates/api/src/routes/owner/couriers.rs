...
//!
//! ## R2a addition: token-derived `GET /api/owner/couriers` (`spa-proxy.ts:452-503`)
//! [`list_couriers_token_derived`] ports the SPA-proxy owner roster (no `:locationId` — the
//! location resolves from the token via [`super::spa_proxy_location_id`], 401 on none). Same
//! `with_tenant` seam (Node seats `app.current_tenant` explicitly there, `spa-proxy.ts:457-458`).
//! ⚠️ FLAG (carried verbatim, NOT fixed): Node's spa-proxy query has NO REV-S7-2
//! synthetic-courier exclusion — unlike `couriers.ts:40`'s roster — so the dev-visual-net
//! synthetic courier IS visible through this route on Node today, and this port reproduces that
//! for parity. Raised in the R2a build report as a candidate FIX-IN-PORT for a council to bless;
//! silently diverging from Node would trip the parity oracle.
//!
...
/// One row of the TOKEN-DERIVED `GET /api/owner/couriers` (R2a, `spa-proxy.ts:452-503`) — the
/// roster projection MERGED with the SAVEPOINT-guarded per-courier avg rating (both queries run
/// in the one seated transaction, exactly like Node's `BEGIN … SAVEPOINT r … COMMIT`).
#[derive(Debug, Clone)]
pub struct SimpleCourierRow {
    pub id: Uuid,
    /// `Option` here (unlike `CourierRosterRow`'s required field) because Node's handler branches
    /// on `r.full_name_encrypted ?` (`spa-proxy.ts:488`) — a NULL column renders `'Unknown'`,
    /// never a crash. Carried so this port can't fail where Node wouldn't.
    pub full_name_encrypted: Option<Vec<u8>>,
    /// `COALESCE(cs.shift_status, 'offline')` — the LATERAL latest-open-shift status.
    pub courier_status: String,
    pub deliveries_completed: i64,
    /// `ROUND(AVG(rating)::numeric, 1)` from the rating query (`spa-proxy.ts:479-484`); `None`
    /// covers BOTH "no rating rows" and "rating query rolled back to the SAVEPOINT" — Node's
    /// `ratingByCourier[r.id] ?? 0` renders both as `0`.
    pub avg_rating: Option<f64>,
}
...
pub trait CouriersRepo: Send + Sync {
    ...
    /// R2a token-derived `GET /api/owner/couriers` (`spa-proxy.ts:452-503`): the simple roster +
    /// the SAVEPOINT-guarded rating aggregate, one `with_tenant` transaction. NOTE (flagged, not
    /// fixed): Node's spa-proxy query has NO synthetic-courier (REV-S7-2) exclusion — unlike
    /// `list_roster` above — and this port carries that verbatim for parity; see module doc.
    async fn list_simple(&self, location_id: Uuid) -> Result<Vec<SimpleCourierRow>, RepoError>;
}
...
pub struct CourierRosterEntry {
    ...
    #[serde(
        rename = "lastLoginAt",
        serialize_with = "crate::dto::serialize_js_instant_opt"
    )]
...
/// The token-derived `GET /api/owner/couriers` per-courier wire shape (`spa-proxy.ts:489-493`).
/// PII-PARITY (R2a brief): `name` is the FULL decrypted name (Node decrypts, does NOT mask —
/// `spa-proxy.ts:488`), `phone` is ALWAYS the empty string (Node hardcodes `phone: ''` and never
/// decrypts `phone_encrypted`). Widened NOTHING: no email, no masked variants.
#[derive(Debug, Clone, Serialize, Deserialize, ToSchema)]
pub struct SimpleCourierEntry {
    pub id: Uuid,
    pub name: String,
    /// Always `""` on the wire (`spa-proxy.ts:490`).
    pub phone: String,
    /// `'online' | 'busy' | 'offline'` — mapped from the shift status (`spa-proxy.ts:491`).
    pub status: String,
    #[serde(rename = "deliveriesCompleted")]
    pub deliveries_completed: i64,
    /// `ratingByCourier[r.id] ?? 0` — integral values serialize as JSON integers like Node.
    #[serde(serialize_with = "crate::dto::serialize_js_number")]
    pub rating: f64,
}
...
/// R2a: `GET /api/owner/couriers` (`spa-proxy.ts:452-503`) — the TOKEN-DERIVED simple roster
/// (no `:locationId` in the URL; the location comes from [`super::spa_proxy_location_id`], 401
/// when it resolves to none — `spa-proxy.ts:454`). Response is a BARE array (no envelope).
///
/// PII-PARITY (byte-for-byte with Node): full decrypted `name` (`decryptPII(...) || 'Unknown'`,
/// any decrypt error/absence -> `'Unknown'`), `phone` always `''`, nothing widened.
#[utoipa::path(
    get,
    path = "/api/owner/couriers",
    responses(
        (status = 200, description = "Simple courier roster for the token-derived location (bare array)", body = [SimpleCourierEntry]),
        (status = 401, description = "No resolvable active owner location", body = domain::ErrorEnvelope),
    ),
    tag = "owner-couriers"
)]
pub async fn list_couriers_token_derived(
    Extension(state): Extension<CouriersState>,
    OwnerClaimsExt(owner): OwnerClaimsExt,
    Extension(request_id): Extension<RequestId>,
) -> Result<impl IntoResponse, ApiError> {
    let correlation_id = correlation_id_string(&request_id);
    let location_id = super::spa_proxy_location_id(&state.auth, &owner, &correlation_id)
        .await?
        .ok_or_else(|| {
            ApiError::new(
    ...
    let rows = state
        .repo
        .list_simple(location_id)
        .await
        .map_err(|_err| internal_error(correlation_id.clone()))?;

    let cipher = state.auth.pii_cipher.as_ref();
    let couriers: Vec<SimpleCourierEntry> = rows
        .into_iter()
        .map(|row| {
            // `spa-proxy.ts:487-488`: 'Unknown' when the column is NULL, when decrypt throws,
            // and when decrypt yields '' (`|| 'Unknown'`).
            let name = row
                .full_name_encrypted
                .as_ref()
            ...
            let status = match row.courier_status.as_str() {
                "available" => "online",
                "on_delivery" => "busy",
                _ => "offline",
            };
...
impl CouriersRepo for PgCouriersRepo {
    ...
    async fn list_simple(&self, location_id: Uuid) -> Result<Vec<SimpleCourierRow>, RepoError> {
        crate::db::with_tenant(&self.pool, TenantId::from(location_id), move |txn| {
            Box::pin(async move {
                // `spa-proxy.ts:459-473` verbatim (minus the projected-but-never-read
                // `c.phone_encrypted`/`c.status` columns — narrower read, identical rows,
                // identical wire output). NO synthetic-courier exclusion — Node has none here
                // (REV-S7-2 gap carried for parity, flagged in the module doc).
                let base: Vec<(Uuid, Option<Vec<u8>>, String, i64)> =
                    sqlx::query_as(SIMPLE_ROSTER_QUERY)
                        .bind(location_id)
                        .fetch_all(&mut **txn)
                        .await?;

                // `spa-proxy.ts:474-485`: SAVEPOINT-guarded so a missing `order_ratings` table
                // (before its migration) can't abort the outer transaction. Node runs this query
                // UNCONDITIONALLY (even with zero roster ids) and WITHOUT a location filter —
                // both carried verbatim.
                let ids: Vec<Uuid> = base.iter().map(|(id, ..)| *id).collect();
                sqlx::query("SAVEPOINT r").execute(&mut **txn).await?;
                let ratings: std::collections::HashMap<Uuid, f64> =
                    match sqlx::query_as::<_, (Uuid, Option<f64>)>(
                        "SELECT courier_id, ROUND(AVG(rating)::numeric, 1)::float8 AS avg \
                    ...
                    .await
                    {
                        // `Number(x.avg)` — a NULL avg lands as `0` in Node's map; same here.
                        Ok(rows) => rows
                            .into_iter()
                ...
                Ok(base
                    .into_iter()
                    .map(
                        |(id, full_name_encrypted, courier_status, deliveries_completed)| {
                            SimpleCourierRow {
                                id,
                                full_name_encrypted,
                                courier_status,
                                deliveries_completed,
                                avg_rating: ratings.get(&id).copied(),
                            }
                        },
                    )
                    .collect())
...
/// R2a token-derived roster (`spa-proxy.ts:459-473`). `COUNT(*)` decodes `::bigint` (#77);
/// the LATERAL open-shift pick and its `$1` reuse mirror Node's one-bind-two-uses exactly.
const SIMPLE_ROSTER_QUERY: &str = "SELECT c.id, c.full_name_encrypted, \
     COALESCE(cs.shift_status, 'offline') AS courier_status, \
     (SELECT COUNT(*) FROM courier_assignments ca2 WHERE ca2.courier_id = c.id AND ca2.status = 'delivered')::bigint AS deliveries_completed \
     FROM couriers c \
     JOIN courier_locations cl ON c.id = cl.courier_id \
     LEFT JOIN LATERAL ( \
...
pub mod fake {
    ...
    use super::{
        ...
        PatchCourierOutcome, RepoError, SimpleCourierRow,
    };
    ...
    pub struct FakeCouriersRepo {
        ...
        /// location_id -> rows for the R2a token-derived `GET /api/owner/couriers`.
        pub simple_rows: Mutex<HashMap<Uuid, Vec<SimpleCourierRow>>>,
    }
    ...
    impl FakeCouriersRepo {
        ...
        pub fn seed_simple(&self, location_id: Uuid, row: SimpleCourierRow) {
            self.simple_rows
                .lock()
                .unwrap()
                .entry(location_id)
...
mod tests {
    ...
    // ── R2a token-derived GET /api/owner/couriers ────────────────────────────────────────────

    /// PII-parity pin (`spa-proxy.ts:486-494`): full decrypted name (NOT masked), `phone` always
    /// `""`, shift-status mapping, `?? 0` rating default, and the BARE-array response shape —
    /// asserted on the exact serialized JSON so a widened field would fail here.
    #[tokio::test]
    async fn list_couriers_token_derived_matches_node_wire_shape_byte_for_byte() {
        let user_id = Uuid::new_v4();
        let loc = Uuid::new_v4();
        let cipher = test_cipher();
        let repo = FakeCouriersRepo::default();
        ...
        repo.seed_simple(
            ...
            SimpleCourierRow {
                id: online_id,
                full_name_encrypted: Some(cipher.encrypt("Ada Lovelace").unwrap()),
                courier_status: "available".to_string(),
        ...
        repo.seed_simple(
            ...
            SimpleCourierRow {
                ...
                courier_status: "offline".to_string(),
        ...
        let response = list_couriers_token_derived(
            Extension(state),
            OwnerClaimsExt(owner),
            Extension(request_id()),
        )
        ...
        assert_eq!(
            ...
            "bare array; full plaintext name; phone always ''; integral rating as JSON int"
        );
    }

    /// `spa-proxy.ts:491`: `on_delivery` -> `busy` (the third status branch).
    #[tokio::test]
    async fn list_couriers_token_derived_maps_on_delivery_to_busy() {
        let user_id = Uuid::new_v4();
        let loc = Uuid::new_v4();
        let cipher = test_cipher();
        let repo = FakeCouriersRepo::default();
        repo.seed_simple(
            ...
            SimpleCourierRow {
                id: Uuid::new_v4(),
                full_name_encrypted: Some(cipher.encrypt("Busy Bee").unwrap()),
                courier_status: "on_delivery".to_string(),
        ...
        let bytes = axum::body::to_bytes(response.into_body(), usize::MAX)
            ...
            .unwrap();
        let body: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(body[0]["status"], "busy");
        assert_eq!(
            body[0]["rating"], 5,
            "integral 5.0 serializes as JSON 5 (Number parity)"
        );
    }

    /// `getLocationId` -> null -> 401 (`spa-proxy.ts:454`); ALSO pins the stale-baked-claim HARD
    /// 401 — spa-proxy does NOT fall back to another active membership (unlike
    /// `resolve_owner_location`).
    #[tokio::test]
    async fn list_couriers_token_derived_401_when_no_location_and_when_baked_claim_is_stale() {
        ...
        // Stale baked claim + a DIFFERENT live membership: Node's getLocationId returns null
        // (401), it does NOT resolve the other membership (`spa-proxy.ts:70-77`).
        let user_id = Uuid::new_v4();
        let other_loc = Uuid::new_v4();
        let stale_baked = Uuid::new_v4();
        let state = state_with(
            FakeCouriersRepo::default(),
            owner_with_location(user_id, other_loc),
        );
        let err = crate::error::expect_err(
            list_couriers_token_derived(
                Extension(state),
                OwnerClaimsExt(OwnerClaims::new(user_id, Some(stale_baked))),
                Extension(request_id()),
            )
            .await,
        );
    ...
    /// Requires a live Postgres — proves the R2a roster SQL + SAVEPOINT-guarded rating SQL
    /// bind/decode against the real schema (#77 class: `COUNT(*)::bigint`,
    /// `ROUND(..)::numeric::float8`, `= ANY($1)` uuid-array bind), read-only (random location ->
    /// zero rows; the rating query still executes with the empty id array, like Node).
    #[tokio::test]
    #[ignore = "requires a live Postgres — set DATABASE_URL_OPERATIONAL/DATABASE_URL_SESSION and run with --ignored"]
    async fn live_pg_list_simple_executes_roster_and_savepoint_rating_queries() {
        let config =
        ...
        let repo = PgCouriersRepo::new(pools.operational.clone());
        let rows = repo
            .list_simple(Uuid::new_v4())
            .await
            .expect("both queries must bind and decode against the live schema");
        assert!(rows.is_empty(), "a random location has no couriers");
    }
}
