...
/// spa-proxy's `getLocationId` (`apps/api/src/routes/spa-proxy.ts:57-91`) — the token-derived
/// location resolution the R2a spa-proxy owner routes (`GET /api/owner/couriers`,
/// `POST /api/owner/courier-invites`) use. DELIBERATELY DISTINCT from
/// [`resolve_owner_location`]: a baked `activeLocationId` whose live membership re-check fails is
/// a HARD `None` (`spa-proxy.ts:70-77` returns `null`, it does NOT fall back), while only a token
/// with NO baked claim resolves the caller's first active membership (`spa-proxy.ts:79-85`,
/// `LIMIT 1` — `active_owner_locations`' deterministic ordering is a superset of Node's
/// unordered `LIMIT 1`). `Ok(None)` = the per-route caller decides (401 for the couriers list,
/// the `pending: true` placeholder branch for invite mint); `Err` = repo failure only.
pub async fn spa_proxy_location_id(
    auth: &AuthState,
    owner: &OwnerClaims,
    correlation_id: &str,
    ...
    Ok(match owner.active_location_id {
        Some(baked) => active.contains(&baked).then_some(baked),
...
pub struct OwnerCatalogStates {
    ...
    pub customers: customers::CustomersState,
    ...
    /// S3 owner dwell-threshold settings (`dwell_settings`) — GET/PUT of `locations.dwell_thresholds`
    /// via the same `OwnerClaimsExt`/`with_user` seam as every other S3 owner-config op.
    pub dwell_settings: dwell_settings::DwellSettingsState,
    /// S3 owner fallback-config settings (non-money `locations.fallback_config`).
    pub fallback_settings: fallback_settings::FallbackSettingsState,
    /// S10 owner onboarding-progress (`onboarding`) — GET state/complete + POST step complete/skip
    /// over `locations.onboarding_state` jsonb (same owner+loc `with_user` seam as dwell/fallback).
    pub onboarding: onboarding::OnboardingStateSvc,
    /// Owner settings (`owner_settings`) — GET/PUT `/api/owner/settings`, the token-derived location's
    /// config (name/phone/geo/hours + the delivery-fee/tax pricing inputs S5 reads).
    pub owner_settings: owner_settings::SettingsState,
    /// R2a `locations` — PATCH `/api/owner/locations/:locationId` (owner partial location update,
    /// dynamic-SET + full-row RETURNING; same `with_user` + membership-assert-first seam).
    pub locations: locations::LocationsState,
    /// R2a `menu_confirm` — the ADR-0014 per-product allergen flag-confirm (`confirm-allergens`).
    pub menu_confirm: menu_confirm::MenuConfirmState,
}
...
pub fn owner_catalog_router(states: OwnerCatalogStates) -> axum::Router {
    ...
    axum::Router::new()
        ...
        // ── customers.rs (Phase 2.3 — owned customer data + erasure oracle) ──
        .route(
            "/api/owner/locations/{locationId}/customers",
            get(customers::list_customers),
        )
        .route(
            "/api/owner/locations/{locationId}/customers/{customerId}",
            get(customers::get_customer).delete(customers::delete_customer),
        )
        // ── dwell_settings.rs (owner dwell thresholds GET/PUT) ──
        .route(
            "/api/owner/locations/{locationId}/settings/dwell",
            get(dwell_settings::get_dwell_settings).put(dwell_settings::put_dwell_settings),
        )
        .route(
            "/api/owner/locations/{locationId}/settings/fallback",
            get(fallback_settings::get_fallback_settings)
                .put(fallback_settings::put_fallback_settings),
        )
        // ── onboarding.rs (S10 owner onboarding-progress state machine) ──
        .route(
            "/api/owner/onboarding/{locationId}/state",
            get(onboarding::get_state),
        )
        .route(
            "/api/owner/onboarding/{locationId}/step/complete",
            axum::routing::post(onboarding::complete_step_handler),
        )
        .route(
            "/api/owner/onboarding/{locationId}/step/{stepNum}/skip",
            axum::routing::post(onboarding::skip_step_handler),
        )
        .route(
            "/api/owner/onboarding/{locationId}/complete",
            get(onboarding::get_complete),
        )
        // ── owner_settings.rs (GET/PUT /api/owner/settings — token-derived location) ──
        .route(
            "/api/owner/settings",
            get(owner_settings::get_settings).put(owner_settings::put_settings),
        )
        // ── R2a batch (backend-tail-batches.md §R2a) ──
        .route(
            "/api/owner/couriers",
            get(couriers::list_couriers_token_derived),
        )
        .route(
            "/api/owner/courier-invites",
            post(courier_invites::create_courier_invite_token_derived),
        )
        .route(
            "/couriers/invites",
            post(courier_invites::create_courier_invite_body_location),
        )
        .route(
            "/api/owner/locations/{locationId}",
            patch(locations::patch_location),
        )
        .route(
            "/api/owner/locations/{locationId}/products/{productId}/confirm-allergens",
            post(menu_confirm::confirm_allergens),
        )
        .route(
            "/api/owner/locations/{locationId}/degradation",
            get(fallback_settings::get_degradation),
        )
        ...
        .layer(axum::Extension(states.customers))
        ...
        .layer(axum::Extension(states.dwell_settings))
        .layer(axum::Extension(states.fallback_settings))
        .layer(axum::Extension(states.onboarding))
        .layer(axum::Extension(states.owner_settings))
        .layer(axum::Extension(states.locations))
        .layer(axum::Extension(states.menu_confirm))
...
mod tests {
    ...
    /// The spa-proxy `getLocationId` contract (R2a) — the ONE divergence from
    /// `resolve_owner_location` is load-bearing: a STALE baked claim is a hard `None`
    /// (`spa-proxy.ts:70-77`), NOT a fall-back to the first active membership.
    #[tokio::test]
    async fn spa_proxy_location_id_hard_nones_a_stale_baked_claim_and_resolves_otherwise() {
        let user_id = Uuid::new_v4();
        let mine = Uuid::new_v4();
        let stale = Uuid::new_v4();
        let repo = Arc::new(FakeAuthRepo {
        ...
        // Valid baked claim -> that location.
        let got = spa_proxy_location_id(&state, &owner_claims(user_id, Some(mine)), "c")
            .await
            .unwrap();
        assert_eq!(got, Some(mine));

        // STALE baked claim -> hard None, even though `mine` is a live membership.
        let got = spa_proxy_location_id(&state, &owner_claims(user_id, Some(stale)), "c")
            .await
            .unwrap();
        assert_eq!(
            got, None,
            "no fall-back on a stale baked claim (spa-proxy.ts:70-77)"
        );

        // No baked claim -> first active membership (legacy-auth branch).
        let got = spa_proxy_location_id(&state, &owner_claims(user_id, None), "c")
            .await
            .unwrap();
        assert_eq!(got, Some(mine));
    }
    ...
    fn test_states() -> OwnerCatalogStates {
        ...
        OwnerCatalogStates {
            ...
            dwell_settings: dwell_settings::DwellSettingsState {
                auth: auth.clone(),
                repo: Arc::new(dwell_settings::fake::FakeDwellSettingsRepo::default()),
            },
            fallback_settings: fallback_settings::FallbackSettingsState {
                auth: auth.clone(),
                repo: Arc::new(fallback_settings::fake::FakeFallbackSettingsRepo::default()),
            },
            onboarding: onboarding::OnboardingStateSvc {
                auth: auth.clone(),
                repo: Arc::new(onboarding::fake::FakeOnboardingRepo::default()),
            },
            owner_settings: owner_settings::SettingsState {
                auth: auth.clone(),
                repo: Arc::new(owner_settings::fake::FakeSettingsRepo::default()),
            },
            locations: locations::LocationsState {
                auth: auth.clone(),
                repo: Arc::new(locations::fake::FakeLocationsRepo::default()),
            },
            menu_confirm: menu_confirm::MenuConfirmState {
                auth,
                repo: Arc::new(menu_confirm::fake::FakeMenuConfirmRepo::default()),
            },
        }
    }
