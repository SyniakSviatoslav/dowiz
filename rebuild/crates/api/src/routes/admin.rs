...
// S10 REV-S10-2: platform-read DEFINER on owner_notification_targets — migration (gates the FLIP,
// not this dark build). Before S10 flips onto NOBYPASSRLS, the `ont`-joined aggregate below MUST run
// via an auditable platform-read (a SECURITY DEFINER fn or a named platform-read role) or the health
// COLUMNS silently zero for every tenant. DoD: `fallback/health` returns fleet channel counts
// (not 0) under NOBYPASSRLS. The `locations` half needs no change (public_select).
pub const FALLBACK_HEALTH_SQL: &str = "SELECT l.id, l.name, l.slug,
   ...
   GROUP BY l.id ORDER BY l.name LIMIT 100";
...
mod tests {
    ...
    #[test]
    fn fallback_health_sql_targets_the_b3_blocker_table() {
        // REV-S10-2: the platform-read is over owner_notification_targets (the FORCE-isolated table),
        // NOT locations (which reads fine via public_select). Pin that the SQL joins the right table.
        assert!(FALLBACK_HEALTH_SQL.contains("owner_notification_targets"));
        assert!(FALLBACK_HEALTH_SQL.contains("FROM locations"));
    }
}
