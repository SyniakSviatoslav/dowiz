...
#[openapi(
    paths(
        ...
        crate::routes::orders::patch_order_metadata,
        ...
        // ── R2a batch (backend-tail-batches.md §R2a — spa-proxy token-derived owner routes,
        // locations PATCH, allergen flag-confirm, degradation read).
        crate::routes::owner::couriers::list_couriers_token_derived,
        crate::routes::owner::courier_invites::create_courier_invite_token_derived,
        crate::routes::owner::courier_invites::create_courier_invite_body_location,
        crate::routes::owner::locations::patch_location,
        crate::routes::owner::menu_confirm::confirm_allergens,
        crate::routes::owner::fallback_settings::get_degradation,
        ...
        // ── dwell-threshold settings (owner) — GET/PUT locations.dwell_thresholds.
        crate::routes::owner::owner_settings::get_settings,
        crate::routes::owner::owner_settings::put_settings,
        crate::routes::owner::onboarding::get_state,
        crate::routes::owner::onboarding::complete_step_handler,
        crate::routes::owner::onboarding::skip_step_handler,
        crate::routes::owner::onboarding::get_complete,
        crate::routes::owner::dwell_settings::get_dwell_settings,
        crate::routes::owner::dwell_settings::put_dwell_settings,
        crate::routes::owner::fallback_settings::get_fallback_settings,
        crate::routes::owner::fallback_settings::put_fallback_settings,
    ...
    components(schemas(
        ...
        // ── R2a batch schemas ──
        crate::routes::owner::couriers::SimpleCourierEntry,
        crate::routes::owner::locations::LocationRow,
        crate::routes::owner::menu_confirm::ConfirmedResponse,
        crate::routes::owner::fallback_settings::DegradationChannel,
        crate::routes::owner::fallback_settings::DegradationResponse,
...
mod tests {
    ...
    fn openapi_document_lists_the_s3_catalog_operations_and_omits_deferred_rows() {
        ...
        // Deferred/out-of-scope proof-of-absence. (The locations PATCH and confirm-allergens
        // rows moved OUT of this list when the R2a batch ported them — see the R2a pin below.)
        assert!(
            !paths.contains("/api/owner/menu/import/commit"),
            "deferred/out-of-scope S3 row must be absent: /api/owner/menu/import/commit"
        );
    }

    /// R2a batch (backend-tail-batches.md §R2a) — the five node-kept routes ported 2026-07-05.
    /// A future edit that drops a `#[utoipa::path]` or its `paths(...)` entry fails HERE.
    #[test]
    fn openapi_document_lists_the_r2a_batch_operations() {
        let doc = ApiDoc::openapi();
        let paths: std::collections::HashSet<&str> =
            doc.paths.paths.keys().map(String::as_str).collect();
        for path in [
            ...
            assert!(paths.contains(path), "missing R2a path: {path}");
        }
    }
