...
SELECT
  ...
  (SELECT count(*)::bigint FROM order_ratings r
...
///
/// SWAPPED (2026-07-05, staging placement done): `gdpr_claim_erasure_batch(p_limit)` is that
/// SECURITY DEFINER fn — pinned search_path, table-owner execution, EXECUTE granted to
/// `dowiz_app`, inner SQL verbatim the previous constant (claim-and-mark single statement,
/// `FOR UPDATE SKIP LOCKED` semantics preserved inside the fn). Same returned row shape.
const CLAIM_PENDING_SQL: &str = "SELECT id, location_id, customer_id, subject_phone, metadata FROM gdpr_claim_erasure_batch($1::int)";
...
mod tests {
    ...
    #[test]
    fn claim_sql_calls_the_definer_queue_claim_never_reads_the_table_directly() {
        assert!(
            CLAIM_PENDING_SQL.contains("gdpr_claim_erasure_batch($1::int)"),
            "REV-S9-2: under FORCE RLS + NOBYPASSRLS this worker seats no GUC, so a direct \
             claim-UPDATE matches ZERO rows and strands every request pending forever; the \
             claim MUST run through the SECURITY DEFINER fn (atomic claim-and-mark + \
             FOR UPDATE SKIP LOCKED live INSIDE it, verbatim the previous constant)"
        );
        assert!(
            !CLAIM_PENDING_SQL
                .to_uppercase()
                .contains("FROM GDPR_ERASURE_REQUESTS"),
            "no direct table read may reappear here — it would silently no-op post-B3"
        );
        assert!(
            CLAIM_PENDING_SQL.contains("id, location_id, customer_id, subject_phone, metadata"),
            "returned row shape frozen — every claim_pending caller depends on it"
        );
    }
