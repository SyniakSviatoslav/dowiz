...
pub struct LocationInfoRow {
    ...
    /// Raw `locations.geo` jsonb — the bot JSON-LD's GeoCoordinates source (`ssr-renderer.ts:83`
    /// reads `loc.geo`, the raw column, NOT the `lat`/`lng` float columns). Carried raw because
    /// Node's truthiness/key-shape quirks must carry verbatim (an empty `{}` still emits the
    /// entity, with latitude/longitude omitted).
    pub geo: Option<Json>,
...
impl PublicRepo for PgRepo {
    ...
    async fn location_info(&self, slug: &str) -> Result<Option<LocationInfoRow>, RepoError> {
        ...
        let row: Option<LocationInfoRow> = sqlx::query_as(
            // Decode-parity casts (staging oracle 2026-07-05): `delivery_fee_flat`/
            // `free_delivery_threshold`/`min_order_value` are INTEGER columns read into i64
            // (sqlx rejects int4→i64), and `tax_rate`/`google_rating` are NUMERIC read into
            // f64 (sqlx rejects numeric→f64) — the uncast SELECT made every /info request a
            // 503 at runtime while psql ran it fine. Same class the S5 lane already casts
            // (orders/pg.rs `min_order_value::bigint`, `max_distance_km::double precision`).
            "SELECT l.id, l.name, l.slug, l.currency_code, l.currency_minor_unit, l.default_locale,
                    l.lat, l.lng, l.delivery_paused, l.hours_json, l.geo, l.address, l.phone, l.kitchen_busy_until,
                    l.delivery_fee_flat::bigint, l.free_delivery_threshold::bigint, l.min_order_value::bigint,
                    l.tax_rate::float8, l.price_includes_tax,
                    ...
                    lt.google_rating::float8, lt.google_review_count, lt.google_maps_url,
