...
#
#    Gate 2 (disallowed-methods): catches the clock/entropy calls that DO compile
#            to wasm (SystemTime::now / Instant::now / env::var) and would slip
#            Gate 1. Config: crates/domain/clippy.toml. Scoped to `--lib` so
#            #[cfg(test)] modules (which may use Uuid::new_v4 via the dev-dep) are
#            exempt — only production core code is policed.
#
...
echo "── Gate 2/2: disallowed-methods (clock/entropy that compile to wasm) ──"
CLIPPY_CONF_DIR="$CORE_DIR" cargo clippy --quiet --manifest-path "$CORE_MANIFEST" --lib -- -D warnings
echo "   ✔ no disallowed clock/entropy calls in production core"
