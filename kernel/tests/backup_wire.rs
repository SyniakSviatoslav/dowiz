...
fn b4_snapshot_and_restore_is_byte_identical() {
    ...
    let (restored, stats) =
        snapshot_and_restore_local(&mut organ, &data).expect("snapshot + restore must succeed");
    ...
    let (restored2, stats2) =
        snapshot_and_restore_local(&mut organ, &data).expect("second snapshot + restore");
...
fn b4_corrupt_snapshot_restores_fail_closed() {
    ...
    let (restored, _stats) =
        snapshot_and_restore_local(&mut organ, &data).expect("first snapshot+restore");
