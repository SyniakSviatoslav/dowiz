...
| `gate-oracle` | `117 gates, 0 without an oracle` `:382` | `:374-375` — **`os.path.exists(bench/oracles/<name>.py)` only** | asserts a *filename*. `touch bench/oracles/foo.py` (0 bytes) makes a gate "oracled". Does not check that `run_all.sh` runs it — and §2.1 row 6 found 11 oracles it never runs |
