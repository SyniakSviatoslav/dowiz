...
| self-compile, second cut (2026-09-06 evening) | 15.6 s | 1.5 s | 10x | the data-section walk re-ran str_len(s) per byte of the 218 KB source (14.6 s of 15.6); one hoist (a27b594). chain + battery: 125 s -> 34 s |
| transient COMPILEFAIL rc=90 under J>=3 | 1-3 % of parallel compiles, blamed on the phantom cap | 0 | | files a bebop program creates had mode 000 (emit_sys_open passed mode 0 to openat); proot fake-root chmod races -> EACCES; mode 0644 (e7cc07f) |
...
| # | item | state | evidence |
|---|---|---|---|
| 1 | automated minimiser (creduce-like, one command) | DONE | bench/fuzz/ladder.py (b37e1c0): hoist rung (a block becomes `let x = 0;` per symbol it binds), compile-before-oracle, deadline in every rung; 42122: 3301 -> 398 B / 21 lines in 67 s, 20056: 4970 -> 142 B / 11 lines in 100 s |
| 2 | granular compilation (parser / IR / backend as separate binaries) | NOT WORTH IT | SPEEDUP-ANALYSIS section 7: the 15 s was one loop (hoisted, a27b594: self-compile 1.5 s); there is no IR to hand between binaries, one text-to-words pass costs 1.45 s, an emitter edit rebuilds everything anyway |
...
| 6 | binary AST/IR cache for unchanged modules | DEFERRED | .becache (T108) already replays a whole compile (70 ms hit); a per-fn cache would take 1.45 -> ~1.15 s (6a, low risk) or ~0.2 s for a one-fn edit (6b, medium risk: relocate every bl/data word); open when the 1.5 s loop is felt |
| 7 | parallel per-fn codegen | SKIP | emission is 0.93 of 1.45 s, ideal 3x on 3 cores = ~0.85 s total, high risk (shared arena/T126, fntab mutated during emission). The `<= 8 live symbols in the clone fn` risk that used to sit in this row is RETIRED: measured 2026-09-12 on the source-built compiler 7c7d1f77, a spawn region with 32 symbols live across it compiles, runs, and returns the child's marker. |
