...
| `zeros32(n)` | ROADMAP A8 step 2 (2026-09-14): allocate n zeroed **4-byte** cells and return an x17-relative index in 4-BYTE units. Implemented as `zeros(ceil(n/2))` with the returned cell index doubled, so it shares the arena and the same exit-80 trap. A `let` bound to it carries type tag 4 (`[u32]`), and THAT tag -- from this `let` or from a `: [u32]` signature -- is what selects the 4-byte load/store forms: a store TRUNCATES to 32 bits (`str wv,[x17,xm,lsl #2]`) and a load ZERO-extends (`ldr wd,[x17,xm,lsl #2]`). `t[0] = 4294967296 + 7` therefore reads back 7, and `t[0] = 0 - 1` reads back 4294967295, never -1. Verified by bench/parity_constructs/c77_u32.bp |
| `smulh(a, b)`, `umulh(a, b)` | the HIGH 64 bits of the 128-bit product, signed and unsigned (single AArch64 words `smulh`/`umulh`; ROADMAP A9, folded into A8 step 2). `smulh(2^62, 4)` = 1 and `umulh(0 - 1, 2)` = 1, both measured 2026-09-14. These are the exact-arithmetic half of a Q32 fixed-point multiply -- the wide product without an i128 |
...
| `hvham(a, b, n)`, `hvham2(...)` | NEON popcount of a^b over **n rounded DOWN to a multiple of 4** words -- the last `n mod 4` words are SILENTLY IGNORED. Measured 2026-09-13 by triggering it: with `a[0]=255, b=0` it returns 0 at n=1, 2 and 3 and 8 at n>=4; with the same byte moved into the tail (`a[4]=255, n=5` or `a[5]=255, n=6`) it returns **0**, and 8 only once n reaches 8. This cell previously said "over n words" with no caveat. Pass an n that is a multiple of 4, or the distance comes back too small with no diagnostic. The only live caller, `bench/vs_rust/kernels/k7neon.bp:61`, uses `let n = 16` and is therefore safe; the hazard is latent, not currently biting |
