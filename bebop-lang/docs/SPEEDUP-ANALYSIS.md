...
| 7 | parallel emission, 3 A78 workers over the balanced partition | ~0.85 s | ~0.85 s | high: shared bump arena (T126) needs per-worker arenas, fntab is mutated during emission (ft_cache, literal counters), determinism only if each worker writes its own [start, start+size) slot; the box budget (boxguard 450 %, fuzzd on the little cores) eats part of the 3x. The `clone-spanning fn <= 8 live symbols` constraint this row used to carry is RETIRED -- measured 2026-09-12 on 7c7d1f77, 32 live symbols across a spawn are fine. |
