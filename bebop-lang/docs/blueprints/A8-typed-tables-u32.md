...
## 3.1 The 4th stab cell and layout sizing

The 4th stab cell at `stab[4*i+3]` holds BOTH new tags: tag 6 `fp` for Q32 fixed-point values and tag 7 `[u8]` for string buffers (declared-length arrays at width 1, feeding F3). Every site that allocates the stab table uses `zeros(385)` (bebop.bp:4433, :5896, :6150), which must grow from 1 + 3*128 = 385 cells to 1 + 4*128 = 513 cells to accommodate the type field. This layout MUST be sized once for all tags in a single commit; growing the stab separately per tag would require re-deriving the layout three times, touching every call site.

## 4. Files and functions touched

| file:fn | change | anchor |
|---|---|---|
| bebop.bp:sym_bind / sym_lookup / stab layout (every `zeros(385)` at bebop.bp:4433,5896,6150 = 1 + 3*128 -> 1 + 4*128 = 513) | type cell | 182, every `zeros(385)` |
...
| bebop.bp:emit_call_or_ctor, T122 table; docs/blueprints/A9-neon-builtins.md | add `smulh`/`umulh` dispatch (A9 lists `umulh` at line 3; both builtins are register-parameterised single words for Q32 fixed-point multiply via `smulh xd,xa,xb` and `umulh xd,xa,xb`) | 1527, A9:1,3 |
