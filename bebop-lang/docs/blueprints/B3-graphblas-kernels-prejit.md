...
**Steps 1-3 as landed (2026-09-07; supersedes the step text above where they differ).** Step 1 = 6183ce8 (objects, gb_roundtrip G9a gate 775084997 + python oracle). Step 2 = accf51a (gen_gb.bp mxv/vxm templates x semirings 1-4, gb_gen gates: 12 generated kernels == tier 0 == oracle incl. a directed u<v matrix; gb_bfs via gb_mxv_generic; `%` is the language's signed remainder -- gb_umod restores the python residue). Step 3 = tier-0 mxm (Gustavson + dense SPA with a monotonic stamp; the write-out scans `stamp` ascending instead of keeping a touched list -- ponytail simplification, ceiling ~1M rows, restore the touched list when a 1M mxm row is measured), eWiseAdd/Mult, select (val!=0 / upper / >= thresh), apply, reduce row/col/scalar; G9b gates gb_tc (over ring_chords, random_lcg and ring2 -- the first two have 0 triangles), gb_cc (synchronous min-label propagation), gb_sssp (Bellman-Ford over gb_mxv min-plus), gb_pr (Q32, 10 iterations, schoolbook multiply -- no umulh yet). DEVIATION: sgraph2.bp's frontier BFS is NOT rewritten on gb_mxv: tier-0 gb_mxv_generic is a dense O(n) row scan with no sparse-push mode, so it cannot meet the frontier row (23 ns/edge-slot Beamer push/pull vs 116 ns queue BFS on 1M/5M) -- the rewrite needs the generated sparse push/pull kernels and therefore moves to step 4 (pool + specialised kernels), where its gate is "sgraph2.sh folds unchanged, frontier row <= 2x". gb_mxv_generic-based BFS is gated separately (gb_bfs).
Each step one chain-gated commit (`--codegen` only for step 4's builtin words).
Step 4(f) as landed (2026-09-09): gen_gb.bp op=5 ships BOTH mains -- _path (fmt=1, gated by gb_bfs_gen 997) and _addr (fmt=2, gated by gb_bfs_gen_addr 997: gb_bfs_gen_addr_gate.bp dispatches the kernel via fork+sys_run with the already-mapped base + matrix offset, the exact sgraph2.bp bfs_frontier shape, against bench/oracles/gb_bfs_gen_addr.py).
