...
**Step 0 as landed (2026-09-07, fixpoint aebd2f49; supersedes the numbers above).** The window list was already 512 entries (1536 cells) when step 0 ran, so the map above (window at 3000+3i, everything at 3655+ unchanged) did not fit. Landed map (tools/check_abi.py ZONES is the source of truth): fn cap 300 -> 512 (worst-case floating zone1/2/3 + enum + ft_cache envelope reaches 2046); b1_facts 2200+i (512 wide), b1_scratch 2712-2714, window entries 2800+3i (512 entries, 2800..4335), fold 4405-4411, jumps 4412-4449, slots 4450-4546, window_hdr 4547-4548, window_cs 4573-4577, bank 4640-4648, literal headers 4649-4652, budget 4750, literal table 5000+i (cap 1000). fntab/ptab zeros(4096) -> zeros(8192) at 5 sites. Cost: bebop.bin 36059 -> 36195 words (+136: bases above 4095 need one movk per use site); constructs 0 WORD_DELTA, census branches unchanged. Two latent bare-literal sites found by the relayout: `ptab[1802]` and `patch_jumps(insns, fntab, 3662/3680, ...)` (not in the `fntab[N]` form).
...
**Commit 2 as landed (2026-09-07, 0f29018, fixpoint 9694b780; supersedes the cell numbers above).** Pairs live at fntab[4578 + 2k] (value) / [4579 + 2k] (register), k < 4 (check_abi ZONES `(4578, 4585, "hoist")`); cs mask/cursor/cs_hi/tsp/S are 4573-4577 after the step-0 relayout. New fns: hoist_scan (text scan of the body, strn from the caller), hoist_record, hoist_lookup, hoist_mask, hoist_release, find_body_end; vs_push turns a matching CONST into a CS entry inside the loop; emit_body's per-statement leak check subtracts hoist_mask. Limitation (deliberate, ponytail): a loop whose body text contains a nested `while` is not hoisted at all -- the outer loop's hoisted cs register stayed live across the inner loop's entry assertion (k1ht/k4t/ptrless/deltasync); lift it by releasing/re-taking around the inner loop when a nested-loop kernel needs it. Result: k8h_loopwords 22 -> 14 (the predicted -8), K8H honest 2.2x -> 1.9x, gate 1.2x UNMET (ROADMAP row A2: decision pending); bebop.bin 37693 -> 38975 words (compiler-side cost only); census bcond 1113 -> 1116 (the guard's own tests).
