...
static FieldValue afv_arena[1 << 25]; /* 8M slots: selfcompile of v2 fast-path compiler needs >4M boxes/run */
    ...
    switch (t->kind) {
        ...
        case TERM_BIN: {
            ...
            switch (t->op) {
                ...
                case BOP_BXOR: v.i = l.i ^ r.i; break;
                case BOP_BOR: v.i = l.i | r.i; break;
                case BOP_EQ:
        ...
        case TERM_EXEC: {
             ...
             * W^X compliant: try dual mapping (memfd) first, fall back to mprotect. */
            ...
            int fd = -1;
            int use_dual = 0;
            /* Try dual mapping for W^X compliance */
            fd = memfd_create("bebop_jit", MFD_CLOEXEC);
            if (fd >= 0) {
                if (ftruncate(fd, (off_t)psz) == 0) {
                    rw = mmap(NULL, psz, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
                    rx = mmap(NULL, psz, PROT_READ | PROT_EXEC, MAP_SHARED, fd, 0);
                    if (rw != MAP_FAILED && rx != MAP_FAILED) {
                        use_dual = 1;
                    } else {
                        if (rw != MAP_FAILED) munmap(rw, psz);
                        if (rx != MAP_FAILED) munmap(rx, psz);
                        rw = rx = NULL;
                    }
                }
                if (!use_dual) { close(fd); fd = -1; }
            }
            void *mem = NULL;
            if (use_dual) {
                memcpy(rw, code, sz);
                __builtin___clear_cache((char *)rw, (char *)rw + sz);
                /* Ensure rx sees the writes */
                __builtin___clear_cache((char *)rx, (char *)rx + sz);
                mem = rx;
            } else {
                mem = mmap(NULL, psz, PROT_READ | PROT_WRITE,
                             MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
                if (mem == MAP_FAILED) { free(code); v.kind = -1; return v; }
                memcpy(mem, code, sz);
                __builtin___clear_cache((char *)mem, (char *)mem + sz);
                if (mprotect(mem, psz, PROT_READ | PROT_EXEC) != 0) {
                    /* Fallback: try mmap with RWX directly */
                    munmap(mem, psz);
                    mem = mmap(NULL, psz, PROT_READ | PROT_WRITE | PROT_EXEC,
                             MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
                    if (mem == MAP_FAILED) { free(code); v.kind = -1; return v; }
                    memcpy(mem, code, sz);
                    __builtin___clear_cache((char *)mem, (char *)mem + sz);
        ...
        case TERM_SYSOPEN: {
            /* sys_open(p,n,flags): pack element bytes, raw openat. Mirrors JIT. */
            Value vp = eval(t->a, env);
            Value vn = eval(t->b, env);
            Value vf = eval(t->c, env);
            if (vp.kind != 6 || vn.kind != 0 || vf.kind != 0) { v.kind = -1; return v; }
            long len = vn.i;
            if (len < 0 || len > vp.nfv || len > 4096) { v.kind = -1; return v; }
            char tmp[4097];
            for (long q3 = 0; q3 < len; q3++) tmp[q3] = (char)(vp.fv[q3].val.i & 0xFF);
            tmp[len] = 0;
            v.kind = 0;
            v.i = bp_syscall3(56, -100L, (long)tmp, vf.i);
            return v;
        }
        case TERM_SYSREAD: {
            ...
            Value vb = eval(t->b, env);
            Value vn = eval(t->c, env);
            if (vf.kind != 0 || vb.kind != 6 || vn.kind != 0) { v.kind = -1; return v; }
            long len = vn.i;
            if (len < 0 || len > vb.nfv || len > 4096) { v.kind = -1; return v; }
            char tmp[4096];
            long r = bp_syscall3(63, vf.i, (long)tmp, len);
            for (long q3 = 0; q3 < r; q3++) vb.fv[q3].val.i = (unsigned char)tmp[q3];
            v.kind = 0;
            v.i = r;
            return v;
        }
