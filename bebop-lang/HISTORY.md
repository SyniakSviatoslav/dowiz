...
- **T104 peephole pass** — PARTIAL 2026-09-05 (madd fusion landed: K4 loop 14 words, fixpoint 0f0da434); T104b step 1 DONE ✓ 2026-09-06: `x * 2^k` with the right operand a single `movz x1,#c` becomes `lsl x0,x0,#k` (UBFM, the movz retracted; shl_try) and `xB + xA * 2^k` — now [mov x0,xA][lsl][mov x1,x0][mov x0,xB] + add — becomes ONE `add x0,xB,xA,lsl #k` (addshift_try); every constant derived by python and checked against as+objdump (lsl #3 d37df000, add x0,x5,x3,lsl #3 8b030ca0); probe `b + a*8` -> 8b130e80, values unchanged; constructs c16 c19 c33 c35 c40 -1 word, c44 -4; kernels k3 -1, k5 -2, k6 -1, k7 -3 words, folds equal; still open: `x*c1*c2`, mul by a non-power constant -> shift-add, LICM of loop-invariant movz. gate K4 <= 3.0 ms, construct folds unchanged.
