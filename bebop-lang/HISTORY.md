Status: 2026-09-05 CURRENT (D11-L: the full pre-split ROADMAP text, verbatim — pulls, task bodies, progress log, decisions D1-D11 with their evidence; ROADMAP.md holds the thesis, TG-DONE table, critical path and measured table; TASKS.md the ledger)

# HISTORY — the roadmap in full
...
**T42 · fix R3.x(a)-(e) at the root, then delete the laws** — DONE ✓ 2026-09-04 ((b) closed by D9(5): `>>>` = asrv literal form, gate c32_asr = 96138 == bpref, fuzz gen.py emits it; the R3.x block no longer exists in SESSION-HANDOFF; earlier history:) (c25-c31 + neg/c28-c29 construct gates; (a) C precedence LANDED per D5: `tools/prec_switch.sh` measured zero fold delta on 85/85 bpref-measurable gates, bebop.bp emit_cmp > bor > bxor > band > shift > expr > term > factor, bpref.py default = C precedence (`BPREF_OLDPREC=1` archaeology), fixpoint 46ddf919, construct 31/31 word-identical (no construct mixed the tiers); (b) decided D9(5): `>>>` = ASR literal form (gate c32_asr, bpref mirror), `>>` stays LOGICAL — R3.x(b) is now documented semantics, not a ban; (c) trap 97, (d) `++` trap 96, (e) c27_zeroarg, array-literal order c31, unary/hex c30 all landed; history in the journal 1788538390..)
...
**T47 · `use "path"` textual module inclusion with content-digest dedup** — DONE ✓ 2026-09-05; T47b nested `use` DONE ✓ 2026-09-06 (use_scan recurses into each fresh module before appending it — dependencies first — and use_comment neutralises every module's own use lines; bpref's expand_use mirrors it; construct c47_usenest (main uses outer, outer uses inner) = 51071 — under the one-level expander it compiled to a silent 0, which is T130; fixpoint 2e50133c, census +2 bcond +1 cbz ALLOWed, battery green) (one level, path-hash dedup, `<out>.use` scratch, gate usemod == oracle; fixpoint 446e5515; nested `use` in an included file = T47b; migrating the 25 prelude-headered gates from gen_selfsrc concatenation to `use` = T47c, DONE 2026-09-05: 0 `// prelude:` headers remain, 36 gate sources carry `use` lines)
...
**T80 · content-addressed imports `use "cas://sha256:<hex>"`** — DONE ✓ 2026-09-06: use_scan resolves a `cas://sha256:<64 hex>` path to `.bcas/<hex>.bp` (cas_prefix/cas_path), slurps it and verifies the SHA-256 of the bytes against the name with the sha256 prelude now compiled into the compiler (`use "selfhost/prelude/sha256.bp"` is bebop.bp's first line); a mismatch is exit 88 (cas_verify); `bebop.bin cas add <file>` (cli_cas) writes the file under its digest and prints the `cas://sha256:<hex>` line (verified against sha256sum on mods/inner.bp); bpref mirrors the resolution + digest check (SystemExit 88); constructs c50_cas (the T47b inner module by digest = 7136) and neg c51_casbad (a digest naming other bytes = COMPILEFAIL:88); fixpoint gen2==gen3==gen4 0b0f07f0 (no codegen change: every frozen construct byte-identical); census 68327->71844 words, bcond 1477->1544, cbz 160->177 ALLOWed (mostly the sha256 module). Pitfall on the way: an 8-deep parenthesised `((((f(a)*16+f(b))*16+...` expression hit exit 95 in the compiler — written as a loop instead (journal).
...
D12. (2026-09-06, operator answers to docs/ANALYSIS-2026-09-06.md §5 and
     docs/EVALS-RESEARCH-2026-09-06.md §3, session 15)
    A. Evals: all fourteen E1-E14 of the research menu land, in the order E7
       (thermal/freq/load validity guard) + tools/perf.py substrate -> E1
       self-compile wall/utime/stime/maxrss trend (E5 memory and the E6
       energy proxy = A78 CPU-seconds x freq fall out of the same wait4) ->
       E3 size trend with a budget (bebop.bin words, entry stub words, per-fn
       words) -> E2 K-kernel interleaved A/B p50/p95 -> E11 docs/PERF.md report
       at the end of chain.sh -> E12 bisect runner, E13 per-construct word
       rows, E14 chain wall/cpu-seconds -> E4 per-gate timing + flake ledger,
       E8 fuzz throughput reader, E9 store/graph phase deltas, E10 becache
       floor. Storage = bench/perf.csv (one row per metric per run, validity
       stamped) + one journal line per run; rules per the menu (exact counts
    ...
    B. T96 P2 goes through the IR rung (T101: an op-list per fn with a
       ...
       words and <= 3.0 ms in-process pinned, fixpoint, constructs re-frozen.
    C. Fuzz honesty: TRAP-UNPREDICTED splits into TRAP-81 (frame heap, by
       design, classified) and TRAP-82 (SIGSEGV/SIGBUS = ALERT class, 0
       tolerated, repros kept); fuzzd keys its seed counter by the promoted
       binary's md5 and TG-DONE 8 reads "N on <md5>" (today 28.5k on
       d785e062, 0 on e14dd55e).
       LANDED 2026-09-06 (session 17): bench/fuzz/fuzz.sh's summary keeps
       TRAP-UNPREDICTED as the 80+81+82 total and adds TRAP-81=/TRAP-82=
       fields; its exit code now excludes TRAP-81 (by design, a pass) but
       still fails on TRAP-82 alone. tools/perf.py's fuzz() reads the new
       fields (old journal lines without them default TRAP-82 to 0, never
       alerted on) and records fuzz_trap82 per promoted binary; check()
       alerts (report() exit 1) on any fuzz_trap82 > 0. Repro files for
       TRAP-82 seeds already landed under $REPROS via fuzz_batch.py's one()
       (unchanged: any category other than OK/BPREF-DEPTH gets one). No
       fuzzd.sh change needed: it already keys per-bin via the journal's
       bin= field, read by perf.py's fuzz(). Gates: fuzz.sh run shows the
       new fields (TRAP-81=0 TRAP-82=0 on bin e14dd55e); perf.py report on
       the real journal is 0 alerts rc=0; a synthetic TRAP-82=1 journal
       line reproduces the fuzz_trap82 alert (rc=1) via EXP_JOURNAL=<path>,
       a new env override on perf.py's journal reader, then perf.csv was
       restored; tools/chain.sh ends battery: GREEN, perf 71 metrics 0
       alerts, rc=0.
    D. Hygiene, one commit: golden refresh (bench/golden/bebop-<rev>.bin +
       sha256, the old one predates use/return/cas); invariants rung
       `as seed.S | objcopy == seed/build/seed`; TASKS.md regenerated by a
    ...
    F. D11-I thresholds frozen now: a = PK lookup >= 4x native sqlite,
       ...
       (measured 22.8x / 30.7x / 2.5x, G7). The real workload W stays open.
    G. D1(a) "<= 1.0x Rust" stays the long target; TG-DONE 1's gate is
       <= 2.0x per row, the 1.0x column is reported without MET/UNMET.
    H. T52-T54 predication: a branchy honest kernel K8 first, csel code only
       when its row proves the branch cost. Mutation sensitivity: a
       diff-scoped rung in chain.sh (only gates whose .bp/oracle changed)
       plus a weekly full sweep. T48 rest: checked types move INTO bebop.bp
       (M effort, zero speed); the python census stays as the negative sample.

D13. (2026-09-06, session 16, operator: "бери усі 12 пропозицій і усе збережи") All twelve
     proposals of docs/RETRO-SESSIONS-2026-09-06.md §5 are accepted as work items, each with
     its acceptance test from that table:
     1. process-count gate in the runners (tools/reap.sh --check 26 in chain.sh, battery.sh,
        fuzz.sh, fuzz_batch.py; exit 97 above the cap);
     2. Agent-spawn PreToolUse hook in ~/.claude/settings.json (deny when ps -e | wc -l > 24
        or another Agent is running, marker file in the scratchpad);
     3. apply adbfix (~/adbfix.sh; the operator pairs adb once) -- removes the 32-phantom cap;
     4. `--codegen` implies FREEZE=1 in chain.sh; invariants.sh prints the census_allow.txt
        candidate line on a census delta;
     5. hookify warn-pkill-literal -> block;
     6. sleep-poll block hook (hookify, bash, `sleep N; tail|cat|ls|wc|grep`);
     7. tools/check_words.py in battery.sh: every new em()/st[] literal in the bebop.bp diff
        must appear in $BEBOP_TMP/words.objdump;
     8. self-copy exec in chain.sh/battery.sh (editing a running script cannot change the run);
     9. timing gates serialised: lcjit and any `# timing` gate run last, single-threaded, after
        boxguard status;
     10. SessionStart hook in /root/dowiz/.claude/settings.json: cat SESSION-HANDOFF.md +
        tools/reap.sh;
     11. journal linter tools/journal_lint.sh (transient|flaky|not reproducible requires
        errno=|rc= and EXPECT:), run by the pre-push hook;
     12. pre-commit rule: a `fix(bebop)` commit touching bebop.bp must also touch
        bench/parity_constructs/ or bench/diag_neg/ (L15 mechanised).
     Also from the retro: retire dead letters L1 -> 7, L14/L15 -> 12, L18 -> 6, L19(c) -> 1-2,
...
**T96 · register tier for ordinary expressions (no x9-x13 window)** — STEP 1 DONE 2026-09-04 (`pop` retracts the push it immediately follows and emits `mov x<rd>,x0` / nothing for x0; branch targets and rd=0 retractions are barred through the per-fn label cell fntab[3660] — two holes found and fixed on the way: an rd=0 retraction leaves an OLDER push adjacent with x0 != its value (bitset SIGBUS), and the label cell leaking across fns/passes made the planning pass and the emission pass disagree by 3 words (ordfsm: every bl 3 words late); K1 loop 51 -> 25 words, bebop.bin 92526 -> 67161 words (-27%), every construct re-frozen with its word delta, fuzz 150/150, fixpoint 104b6291; step 2 DONE 2026-09-04 (pop2(): `push a ; P ; push b` with P = mov x0,xR / ldr x0,[x15,#k] / movz x0,#imm is retracted and P re-emitted with rd=x1 in emit_binop_plain + emit_cmp_op; the discarded while-tail literal `0` is retracted too; K1 loop 25 -> 14 words with ZERO memory ops = the D2 DONE-CHECK <= 16; fixpoint 3aae4ad8); step 3 (`let` from x0) fell out of step 1 (pop x0 elided before bind_reg); P3 DONE 2026-09-06 (cmp_try in emit_cmp_regs: `cmp x0,x1` whose right operand was a single `movz x1,#imm` (imm <= 4095) or `mov x1,xS` takes it directly -- `cmp x0,#imm` / `cmp x0,xS` -- and when the left operand was `mov x0,xR` the cmp reads xR; both retracted under the shl_try label/barrier rule (a label AT the pair start is fine, one inside bars the fold); words python-derived and objdump-checked (SUBS-imm f100001f + imm<<10 + ra<<5, SUBS-reg eb00001f + rs<<16 + ra<<5: cmp x20,#0 f100029f); K4 loop 17 -> 15 words (`cmp x20,#0 ; b.le` head), K1 loop 10 (gate <= 12), bebop.bin 75692 -> 74804 words (-888), 16 constructs re-frozen at -1..-8 words, kernels k1..k7 -2..-15, values equal, census bcond 1588 -> 1604 ALLOWed (cmp_try's 16 selector ifs), census.txt --freeze, pool_parity 5/5, fixpoint e14dd55e); next: P2 (operands in x1-x7; K4 <= 13 loop words / <= 3.0 ms))
...
- **T102 condition fusion** — DONE ✓ 2026-09-05 (cmp + cset + cmp#0 + b.eq -> cmp + b.<inverse>; K1 loop 12 words, fixpoint 3239034d): `while`/`if` conditions emit `cmp xR,#imm` / `cmp xR,xS` + `b.cond`; gate K1 <= 12 words (met), K4 <= 4.7 ms (honest.sh after T104).
...
- **T104 peephole pass** — PARTIAL 2026-09-05 (madd fusion landed: K4 loop 14 words, fixpoint 0f0da434); T104b step 1 DONE ✓ 2026-09-06: `x * 2^k` with the right operand a single `movz x1,#c` becomes `lsl x0,x0,#k` (UBFM, the movz retracted; shl_try) and `xB + xA * 2^k` — now [mov x0,xA][lsl][mov x1,x0][mov x0,xB] + add — becomes ONE `add x0,xB,xA,lsl #k` (addshift_try); every constant derived by python and checked against as+objdump (lsl #3 d37df000, add x0,x5,x3,lsl #3 8b030ca0); probe `b + a*8` -> 8b130e80, values unchanged; constructs c16 c19 c33 c35 c40 -1 word, c44 -4; kernels k3 -1, k5 -2, k6 -1, k7 -3 words, folds equal; T104b step 2 DONE ✓ 2026-09-06 (mulc_try: `x * c` with a single movz right operand, c = 2^k+1 -> `add x0,x0,x0,lsl #k`, c = 2^k-1 -> `lsl x1,x0,#k ; sub x0,x1,x0`, c = 2^j+2^k -> `add x0,x0,x0,lsl #(k-j) ; lsl x0,x0,#j`; the movz retracted under the shl_try label/barrier rule; 17 probes 3..65535 equal to bpref; c19 c20 c33 c40 -1 word, c44 -4; census bcond 1542 -> 1559 ALLOWed; fixpoint 70cddb59); CLOSED 2026-09-06 as DONE-as-measurable (operator): the two open items `x*c1*c2` and LICM of loop-invariant movz have no target -- the K4 loop after mulc is 17 words with no movz and no constant chain, and no kernel/gate/construct source has a c1*c2 chain (only 4 generated fuzz repros); the remaining K4 words are P3 (cmp xR,#imm + b.cond directly) and P2 (operands in registers) = T96 rest. gate K4 <= 3.0 ms moves to T96, construct folds unchanged.
- **T105 hardware division** — DONE ✓ 2026-09-05 (`clz(x)` builtin = CLZ, emit_clz, fixpoint 362c0e02, construct c41_clz, census +2 ALLOWed; prelude isqrt = clz-seeded Newton descent, tdg fp_div = base-2^k long division on sdiv with k = max(1, clz(b)-1); both bit-exact by construction and against math.isqrt / (a<<32)//b on 1.2M + 100k values; every fp gate fold unchanged, bpref agrees; micro-bench 1M calls pinned A78: isqrt 276-296 ms -> 38-44 ms (~7x), fp_div 246-260 ms -> 16-29 ms (~10x); the tq/tdg gate walls (150 ms) are process-dominated and did not move — the 1M-point geodesic bench is not built, so the '<= 50 ms' clause stays unmeasured): fp_div/isqrt software loops in tq/tdg -> `sdiv` + clz-seeded Newton isqrt; gate tq fold unchanged, tq geodesic on 1M points <= 50 ms.
...
- **T107 incremental-substrate curve** — DONE ✓ 2026-09-05 (bench/substrate_spike/incr.sh -> RESULT-incr.md: 16x4096 grid DAG, dirty-bitset sweep with clz drain vs full recompute, bebop and Rust twin, folds equal for every (k, mode); us per rep bebop sweep/full = 15/1031 (k=1), 234/984 (16), 1828/1078 (256), 5281/1109 (4096); Rust 4/132, 50/127, 525/129, 1446/135; crossover k = 256 = 0.39% of N in BOTH engines — the sweep pays only where activity is below ~0.4% of the cells, which is the D8 rule 'sweep only where sparse' with a number; bebop full recompute is 8x behind Rust on this indirect-load loop, the next codegen target after T104b): N = 2^16 cells, change k in {1,16,256,4096}, sweep vs full recompute, Rust twin of both; pass = the crossover k/N is recorded (<= 5% expected after T101).
...
- **T121 K5 self-compile + K6 nnidx scan as bench rows** — DONE ✓ 2026-09-06: honest.sh's report now ends with a K5 row (the pinned self-compile from the ROADMAP Measured table, single run, no Rust twin — rustc is not a fair twin of a one-pass compiler) and a K6 row (the nnidx 1M scan from bench/tq_sqlite/RESULT.md against the sqlite scan), each with its provenance; the next `R=11 honest.sh` run on a quiet box prints them.
...
- **T123 mutation sensitivity** — DONE ✓ 2026-09-06: tools/mutate_gate.sh (3 operator sites of the EXPANDED program per gate) over all 99 gates: 5 insensitive (base64, hex, lsm, dispatcher, mvcc), each fold extended to exercise the mutated fns (decoders + invalid inputs, FWHT coefficients + a dense probe, bt_offset on a dense shape, absv/pc4) with new goldens taken from oracle == bebop == bpref; every one of the 5 now changes on 3/3 mutants (the old sweep's 15 were mostly gates whose text had no operator before the expanded-program rewrite).
- **Dev-loop speed-ups (operator priority, 2026-09-06)** — tools/chain.sh (gen2, then gen3->gen4 in parallel with the battery), tools/battery.sh (every gate script in parallel, one summary), tools/std_par.sh (std_golden in J contiguous shards on the A78 cores, standalone retry of a failed gate: 99/99 in 36 s), tools/pcprof.sh (gdb-sampling profiler), gen.py loop-name fix (fuzz 0.05 -> 1.5 programs/s), the third compile pass removed (emit_offsets; fixpoint 85cdfe78, gen2 187 s); then the real hot spot: str_len is a byte scan to the NUL and the emitter called it on the whole source at 43 per-token sites — compile_fn_at now carries the length in pos[1] and slen(s, pos) returns it after validating the end (fixpoint aa41e8a1): SELF-COMPILE 187 s -> 15 s, a std gate 19.9 s -> 0.6 s, 4000 statements 25 s -> 0.2 s. Measured: a full chain + battery 439 s before slen vs ~25 min of orchestrated runs at the start of the day; with slen a generation is 15 s. Open: a smaller per-call term (4000 calls 1.8 s), the parallel fn compile, a capacity trap for a fn emitting > 65536 words (exit 82 today).
- **T124 fold specifications** — DONE ✓ 2026-09-06: docs/FOLDS.md states the fold of csr, bt, store, tq, mvcc, stm, sort and rng in prose with the frozen value and the oracle path; the oracle stays the executable twin.
- **T125 `&&` / `||` in bpref** — DONE ✓ 2026-09-06: tokenized and evaluated as the `&` / `|` tiers on 0/1 comparison values (no short circuit, exactly what bebop.bin does: its `&` tier consumes one `&` and the right operand parses the second); construct c46_andor = 111100 agrees between bpref and bebop.bin; morph.bp is no longer excluded from the type census (0 findings); morph's fold still differs under bpref only because of its nine sys_ calls (stubs), which is why its oracle is morph.py.
