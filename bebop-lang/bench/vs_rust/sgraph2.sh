...
log_ns=$(bb l t); stall=$(bb m f); nbrlog=$(bb n f); nbr_t=$(bb n t)
...
o0=$(taskset -c "$PIN" python3 bench/oracles/sgraph2.py nbr0); olog=$(taskset -c "$PIN" python3 bench/oracles/sgraph2.py nbrlog)
o1=$(taskset -c "$PIN" python3 bench/oracles/sgraph2.py nbr); ob=$(taskset -c "$PIN" python3 bench/oracles/sgraph2.py "$NSRC")
...
| insert 1M edges through the log, 100 batches with an L0 rebuild each: amortized ns per edge / max batch stall ms | $log_ns / $stall |
| neighbours of v after the log, us per query (L1 slice + L0 slice + tombstone bits, 3 slices) | $nbr_t |
