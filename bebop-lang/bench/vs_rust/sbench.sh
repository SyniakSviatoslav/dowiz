...
B[compact]=$(bb compact t); B[lookup3_f]=$(bb lookup f); B[size2]=$(bb z f); B[blocks2]=$(( $(stat -c %b sbench.store) * 512 ))
...
read -r _ S[size1] _ <<<"$(sq size)"; read -r _ S[compact] _ <<<"$(sq compact)"; read -r _ S[size2] _ <<<"$(sq size)"
