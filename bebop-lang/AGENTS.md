...
| 15 | **A committed mirror with no producer and no consumer** | `bebop.bp.use` is tracked, is updated by hand in every commit that touches `bebop.bp`, and NOTHING in `tools/`, `bench/` or any gate reads or writes it — `grep -rn 'bp\.use'` over the scripts finds zero hits. So it can silently go stale against its own source and no gate notices; a lane regenerated it BEFORE its last edit to `bebop.bp` and the mtimes were the only evidence | no automatic check today. **Verified 2026-09-13: it is byte-identical to the chain's `<out>.use`** (committed `c7bed812` == `gen4.bin.use` of the same source), so regenerate it as `cp $OUT/gen4.bin.use bebop.bp.use` from the authoritative chain rather than by hand, and never trust a lane's copy. Worth a real invariant: the committed mirror must equal the expansion the chain just produced |
