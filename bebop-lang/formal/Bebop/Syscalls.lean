...
axiom sys_open_spec (cells len flags : Val) (s : State) :
  ∃ (result : Val) (s' : State),
    ...
    (∀ i, i < s.arena.cells.size → s.arena.cells.get ⟨i, by decide⟩ == s'.arena.cells.get ⟨i, by decide⟩) ∧
    True
...
axiom sys_read_spec (fd buf n : Val) (s : State) :
  ∃ (result : Val) (s' : State),
    ...
    (∀ i, (i < buf.toNat ∨ i >= (buf + result).toNat ∨ i >= s.arena.cells.size) →
           s.arena.cells.get ⟨i, by decide⟩ == s'.arena.cells.get ⟨i, by decide⟩) ∧
    True
...
axiom sys_readbuf_spec (fd len : Val) (s : State) :
  ∃ (result : Val) (s' : State),
    ...
    (result < 0 → result ∈ #[ -3, -5, -9, -11, -14 ]) ∧
    (∀ i, (i >= len.toNat ∨ i >= s.arena.cells.size) →
           s.arena.cells.get ⟨i, by decide⟩ == s'.arena.cells.get ⟨i, by decide⟩) ∧
    True
...
axiom sys_slurp_spec (fd len : Val) (s : State) :
  ∃ (result : Val) (s' : State),
    (result >= 0 → result == s.arena.cells.size) ∧  -- returns arena offset
    (result < 0 → result ∈ #[ -3, -5, -9, -11, -14 ]) ∧
    (∀ i, (i >= len.toNat ∨ i >= s.arena.cells.size) →
           s.arena.cells.get ⟨i, by decide⟩ == s'.arena.cells.get ⟨i, by decide⟩) ∧
    True
...
axiom sys_mmap_spec (addr len prot flags fd off : Val) (s : State) :
  ∃ (result : Val) (s' : State),
    ...
    (∀ i, (i < addr.toNat ∨ i >= (addr + len).toNat ∨ i >= s.arena.cells.size) →
           s.arena.cells.get ⟨i, by decide⟩ == s'.arena.cells.get ⟨i, by decide⟩) ∧
    True
...
axiom sys_export_spec (cells n path len : Val) (s : State) :
  ∃ (result : Val) (s' : State),
    ...
    (∀ i, (i < cells.toNat ∨ i >= (cells + n).toNat ∨ i >= s.arena.cells.size) →
           s.arena.cells.get ⟨i, by decide⟩ == s'.arena.cells.get ⟨i, by decide⟩) ∧
    True
...
def sys_read_footprint (buf n : Val) : Footprint :=
  ⟨#[], List.range (Nat.abs n.toNat).toUSize.toNat |>.map (fun i => (Nat.abs buf.toNat + i)), #[ -3, -5, -9, -11, -14 ]⟩
...
def sys_mmap_footprint (addr len : Val) : Footprint :=
  ⟨#[], List.range (Nat.abs len.toNat).toUSize.toNat |>.map (fun i => (Nat.abs addr.toNat + i)), #[ -12, -13, -17, -22, -27, -35 ]⟩
