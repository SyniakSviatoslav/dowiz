...
# Usage: tools/battery.sh <candidate.bin> <tmp-root> [FREEZE=1]
...
# invariants.sh is NOT here: it hardcodes ./bebop.bin — run it after promotion.
cd "$(dirname "$0")/.." || exit 1
...
( BEBOP_TMP=$T/pool BEBOP_BIN=$BIN bash bench/vs_rust/pool_parity.sh > "$T/pool.log" 2>&1;
  bash bench/oracles/run_all.sh > "$T/oracles.log" 2>&1 ) &
