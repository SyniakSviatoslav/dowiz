...
| A2b | K8H follow-up: lift the hoist limitation for loops with a nested `while` (release/re-take the loop cs temps around the inner loop), then profile the remaining K8H gap (data-dependent `if` already a csel; per-iteration mul/add/shift/and vs the Rust twin) and close it or record why it cannot close | docs/blueprints/A2-csel-and-const-hoist.md §3 "Commit 2 as landed" | K8H <= 1.2x Rust (honest.sh row) | A2 |
