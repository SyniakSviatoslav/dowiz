...
| A14b | the 3 leftovers of A14's 330-repro corpus sweep: (i) seed 100744 still exits 89 at a NON-arm-spanning site (bebop.bp:5:2049 diag, a `let` colliding with a call temp outside an if) -- shrink it and lift or fail-closed-document that case; (ii) seeds 100671 + 100828 compile but trap 81 (frame heap) because A14's wider x15 slot region takes 1280 bytes out of the 16 KiB frame -- either a per-fn x14 chosen from the facts pass (two verified encodings, 0x300 for <= 64 slots and 0x900 above) or wait for A6, which deletes exit 81 entirely | (inline; repros in ~/.cache/bebop/fuzzd/repros/UNSUPPORTED-89-1007*.bp) | corpus sweep 330/330 match bpref, rc=89 = 0, TRAP-81 = 0; fixpoint; K gates unchanged | after A14 (codegen) |
| A15 | compile-time diagnostic for an unbound symbol: `let _ = v0 = 0 in 0` with v0 never declared compiles on every bebop.bin (pre-A1 prints 0, 05aa91ac prints nothing / traps `brk #0` in emit_let_chain) and bpref answers 0 -- all three must reject it: diag_exit(<free code>, "unbound symbol <name>") at the sym lookup in emit_let_chain / emit_compound_stmt / emit_ident, bpref raises the same, gen.py never emits an undeclared write (found by the round-2 shrinker, repro session-24 scratchpad div3/sh/146987.bp) | (inline) | neg/c93_unbound gate COMPILEFAIL:<code>; fixpoint | with A13 (codegen, diag words only) |
