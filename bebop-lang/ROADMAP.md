...
| A4 | DIVERGE round 1 LANDED 2026-09-07 (bebop.bin aa807ab9: RC1 compound-assign snapshot, RC3 array-literal flags clobber fixed; RC2 if-arm cs relocation fail-closed as exit 89; 31 repros: 7 match, 8 fail-closed, 16 STILL DIVERGE + new 148648 -> round 2) -- window RESTARTS after the register-model DIVERGE fix (ALERT 2026-09-07 19:20: batches 145600-148100 on e9159318 = 11 DIVERGE + 376 COMPILEFAIL rc=89; bisect: pre-A1 bin 9e53878 matches bpref, every bin from a77adc6 diverges -- the register model was never fuzzed, fuzzd had been paused since session 18; fuzzd paused again, one Sonnet worker on the miscompiles, rc=89 class -> A14) -- window OPENED 2026-09-07 ~18:00 on e9159318 (operator: before A2b/A3; any later codegen promotion restarts the count on its own md5) -- 24 h fuzz freeze window (process, no code): fuzzd on the promoted md5 until fuzz_seeds_on_bin >= 10^5 | docs/blueprints/A4-fuzz-freeze.md | 0 CRASH/DIVERGE, TRAP-82 = 0 | A1-A3 |
...
| A14 | lift the register model's `let ... in` restriction: REGISTER-MODEL-BLUEPRINT §1.1 exits 89 (`let binding while a call temp is live`) when a `let` inside an expression needs a register a parked cs temp owns -- the pre-A1 compiler accepted these programs, and the fuzz generator produces them in ~25 % of seeds (fuzzd batches 145600-148100 on e9159318: COMPILEFAIL 131/132/113 of 500, all rc=89). Until lifted, bench/fuzz classifies them as UNSUPPORTED-89 (pass, counted, never an ALERT; blueprint §1.1's "bpref predicts it" was never done). Fix = spill the colliding cs temp to a temp slot at sym_bind time (or park into the next free slot) instead of refusing; probe c88_letlive from the shrunk repro | (inline; repros in ~/.cache/bebop/fuzzd/repros/COMPILEFAIL-14*.bp) | fuzz 500 seeds: UNSUPPORTED-89 = 0; c88 gate; fixpoint | after the DIVERGE fix (codegen) |
