...
| A14b | the 3 leftovers of A14's 330-repro corpus sweep: (i) seed 100744 still exits 89 at a NON-arm-spanning site (bebop.bp:5:2049 diag, a `let` colliding with a call temp outside an if) -- shrink it and lift or fail-closed-document that case; (ii) seeds 100671 + 100828 compile but trap 81 (frame heap) because A14's wider x15 slot region takes 1536 bytes out of the 16 KiB frame (verified 2026-09-08: x14 0x300 -> 0x900 = 768 -> 2304, slot region 512 -> 2048, frame heap 15616 -> 14080; the earlier 1280 was wrong) -- either a per-fn x14 chosen from the facts pass (two verified encodings, 0x300 for <= 64 slots and 0x900 above) or wait for A6, which deletes exit 81 entirely | (inline; repros in ~/.cache/bebop/fuzzd/repros/UNSUPPORTED-89-1007*.bp) | corpus sweep 330/330 match bpref, rc=89 = 0, TRAP-81 = 0; fixpoint; K gates unchanged | after A14 (codegen) |
