...
| 2 | **Button hierarchy** — mostly a FALSE read (missing icons distorted it): primaries/empty-states already use the shared `<Button>`/`<EmptyState>`. The ONE real gap — "Export CSV" styled 3 ways — is ✅ consolidated to `<Button variant="secondary" size="sm">` (`e85d990a`, validated). | admin (Export CSV) | ~~Critical~~ → **Mostly done** | `<Button>` exists + is widely used; remaining ad-hoc buttons are bespoke (sticky CTAs etc.), migrate opportunistically |
