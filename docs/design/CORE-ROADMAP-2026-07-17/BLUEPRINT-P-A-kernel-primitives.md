...
## 11. Codegen-leg status correction (A1/A2/A3/A6) — added 2026-07-18

**This section corrects a stale dispatch premise, per §0's own rule that ground truth is
non-discussible.** A swarm-dispatch brief for this pass assumed A1 (eqc-rs `Asin`/`Atan2`/
`DivHalfUp` + checked-int emission), A2 (`ema_next` generated organ), A3 (money-law shadow
organ), and A6 (integer-CORDIC sin/cos) were the only Layer-A items still OPEN. Live
verification this pass (working tree on `main`, 2026-07-18) shows **all four are already
built, committed, and green** — T1/A2, T5/A1, T6/A3, T8/A6 from §10's own task list are done.
This is the same class of correction commit `aebbbe199` already made for A4/A5/A7 ("live
ground-truth status map — corrects stale blueprint RED-claims") — that commit's status map
never mentioned A1/A2/A3/A6 (confirmed by grep of `docs/design/GROUND-TRUTH-2026-07-17.md`,
zero hits for those items); this section closes that specific omission for Layer A.

### 11.0 Ground truth this pass (§2 item 1)

| Item | Dispatch-brief premise | Live status, 2026-07-18 | Evidence |
|---|---|---|---|
| A1 | OPEN — needs building | **DONE, GREEN** | `tools/eqc-rs/src/lib.rs:68,71,75` (`Asin`/`Atan2`/`DivHalfUp` variants), `:98` (`IntEmissionUnsupported`), `:393` (`emit_int_checked_rust`); commit `14d3ab6fa` "feat(eqc-rs): T5/A1 Asin/Atan2/DivHalfUp + integer-checked emission", ancestor of current `HEAD` (`git merge-base --is-ancestor 14d3ab6fa HEAD` → true) |
| A2 | OPEN — needs building | **DONE, GREEN** | `kernel/src/eqc_gen.rs:8-13` (`ema_next_f64`, "GENERATED by eqc-rs" header); `kernel/src/geo.rs:518` (`EMA_PARITY_FIXTURES`, 21 cases incl. ±0.0/subnormal/±∞/α edge cases), `:552` (`ema_next_generated_parity_bit_identical`); commits `9f346b57b`(RED)→`b753fa6e7`(GREEN)→`f957e2360`(merge), all ancestors of `HEAD` |
| A3 | OPEN — needs building | **DONE, GREEN** | `kernel/src/money.rs:22` (`pub const MONEY_SCALE_MICRO`), `:567` (`apply_tax_generated_parity_exact_integers`), `:569` (`MONEY_TAX_FIXTURES`); `kernel/src/eqc_gen.rs:26-40` (`apply_tax_exclusive_int`/`apply_tax_inclusive_int`); commits `afde0fd05`(RED)→`b2801d313`(GREEN, "T6/A3 money-law shadow organs"), ancestors of `HEAD`. `apply_tax` itself (`money.rs:269-287`) unchanged — shadow-only, confirmed |
| A6 | OPEN — needs building | **DONE, GREEN, with one open leg (honestly declared)** | `tools/eqc-rs/src/cordic.rs` (pure-integer Q30 CORDIC, `ITERS: u32 = 31`); `tools/eqc-rs/tests/cordic_digest.rs:19` (`cordic_digest_pinned`, green), `:30` (`cordic_digest_has_teeth`, green), `:57` (`cordic_cross_arch_identical`, `#[ignore]`d — matches the blueprint's own accepted honesty protocol at §3.6, not a defect); commits `d692c59fc`(feat)→`9eacf59ba`(merge "T8/A6 integer-CORDIC sin/cos, digest-pinned"), ancestors of `HEAD` |

Commands run live this pass, both green:
- `cd tools/eqc-rs && cargo test --release` → **16 passed, 0 failed, 1 ignored** (the declared
  cross-arch seam) — unit_tests (8, incl. `atan2_refused_by_fixed_subset`,
  `div_half_up_refused_by_f64_emission`, `sqrt_refused_by_int_checked_subset`,
  `int_checked_overflow_never_wraps`) + `tests/proof.rs` (6, incl. `atan2_f64_proven`,
  `div_half_up_int_mode_proven`, `tax_f64_and_fixed_proven`) + `tests/cordic_digest.rs` (2 of 3).
- `cd kernel && cargo test --lib` → **540 passed, 0 failed**, including
  `geo::tests::ema_next_generated_parity_bit_identical ... ok` and
  `money::tests::apply_tax_generated_parity_exact_integers ... ok` by name.
- `grep eqc kernel/Cargo.toml` → no match — confirms §4.3's isolation claim
  ("generated code has zero runtime dependency on the compiler crate") live, not asserted.

### 11.1 Role & responsibility

The eqc-rs codegen leg is the "equations-not-primitives" pattern: an organ that a hand could
plausibly write (an EMA update, a half-up tax rounding, a sin/cos rotation) is instead expressed
once as a symbolic `Expr` tree and *compiled* — via `emit_f64_rust`/`emit_int_checked_rust` —
into committed, hand-inspectable, hand-editable-never Rust. The equation is the single source of
truth; the emitted file (`kernel/src/eqc_gen.rs`) is a derived artifact, diffable like any other
patch but never independently hand-drifted from its equation. This matters here specifically
because A4 and A5 (already-landed, §3.4/§3.5) exist *precisely because* the pre-eqc kernel had
exactly this drift class already happen by hand — two verbatim inline copies of the eig2x2
quadratic that silently diverged by a line, and a hash path that silently stopped canonicalizing.
The codegen leg makes that whole bug class unrepresentable for the organs it covers: a
parity/proof test compares the generated body against the hand-written law (A2/A3) or against an
independent tree-walking interpreter (`Expr::eval`, A1's proof programs), so "the equation and
the code disagree" becomes a compile-/test-time failure, not a silent runtime drift.

### 11.2 Definition of DONE — falsifiable, verified already-satisfied 2026-07-18

Reframed from the prospective RED→GREEN table in §5 (still the right target shape for *future*
organs) into a retrospective proof that each row is presently satisfied:

| Item | Falsifiable criterion | Command | Result, live 2026-07-18 |
|---|---|---|---|
| A1 | `Asin`/`Atan2`/`DivHalfUp` exist as `Expr` variants; `emit_int_checked_rust` exists and is exercised by proof programs, not just unit-asserted behavior | `cd tools/eqc-rs && cargo test --release` | 16/16 (1 declared-ignored) green |
| A2 | `ema_next_generated_parity_bit_identical` compares `crate::eqc_gen::ema_next_f64` (the *compiled* artifact) against the hand law across 21 fixtures, `to_bits()`-exact | `cd kernel && cargo test --lib -- ema_next_generated` | green (named pass, above) |
| A3 | `apply_tax_generated_parity_exact_integers` compares the generated int-checked organs against `apply_tax` across the fixture corpus, exact-integer; `apply_tax` itself untouched | `cd kernel && cargo test --lib -- apply_tax_generated` | green (named pass, above); `money.rs:269-287` logic unchanged per §0's own MATCH row |
| A6 | `CORDIC_SINCOS_DIGEST` recomputed from `cordic_sincos`'s live output equals the pinned constant; an `ITERS−1` mutation changes the digest (teeth proven, not asserted) | `cd tools/eqc-rs && cargo test --release -- cordic` | `cordic_digest_pinned` + `cordic_digest_has_teeth` green; cross-arch leg `#[ignore]`d, honestly labeled |

The compiler-leg criterion the dispatch brief asked to double-check — *"does §5 say the eqc-rs
compiler itself must exist and be invoked via build.rs or an explicit codegen step, not just
'the organ behaves right'"* — is satisfied by construction, not just by the parity tests: the
generator is `tools/eqc-rs/src/bin/gen_kernel_organs.rs`, an explicit binary (not a `build.rs`)
whose `main()` builds the `Expr` trees for `ema_next`/`apply_tax_exclusive`/`apply_tax_inclusive`
and calls `.emit_f64_rust()`/`.emit_int_checked_rust()`; its stdout is what was pasted into
`kernel/src/eqc_gen.rs` (the file's own header states this provenance). The GREEN commits'
diffs (`b753fa6e7`, `b2801d313`) touch only `eqc_gen.rs` + the parity-test files — never the
`Expr`/emitter internals in the same commit as a parity flip — which is the structural signature
of "regenerate and paste," not "hand-tune until the test passes."

### 11.3 Definition of NOT-done / explicit anti-scope

1. **"A1/A2/A3/A6 are open" is false as of this pass** — do not re-dispatch a build swarm for
   them without first re-running the two commands in §11.0; if a future session's dispatch brief
   says otherwise again, the brief is stale, not the code (ground truth is non-discussible, §0).
2. **Hand-writing an equivalent organ that happens to match the equation would not be "done"** —
   moot for this section, since live inspection (the GREEN-commit diff shape, §11.2) confirms
   these four *are* the compiler-emitted artifacts, not hand transcriptions. Restated here as the
   standing rule for anyone touching `eqc_gen.rs` by hand in the future: the file's own header
   ("do not hand-edit") is the enforcement, a human/agent diff review is the gate.
3. **A6's CORDIC-into-Sin/Cos wiring is a real, narrow, still-open gap.** `tools/eqc-rs/src/cordic.rs`
   is promoted and proven standalone (digest tests green), but `emit_int_checked_rust`'s own
   `Sin`/`Cos` arms still hard-refuse (`tools/eqc-rs/src/lib.rs:610-612`,
   `Err("sin/cos not in the integer-exact subset")`) rather than routing through
   `cordic::cordic_sincos` — even though T5/A1 (the stated precondition, §3.6/§10 T8) has landed.
   Do not claim T8/A6 fully closed without either doing this wiring or writing an explicit,
   dated deferral (per item 19 of the standard: skipping it silently is not a valid reason).
4. **`REGRESSION-LEDGER.md` row 25's consumption claim does not match live kernel source.** The
   row states "`kernel` callers route through it [`cordic_sincos`]"; live grep
   (`grep -rn "cordic_sincos" kernel/`) returns zero matches, and `causal.rs::empirical_identify`
   (the function the row names as the motivating caller) contains no `.sin()`/`.cos()` calls at
   all today. Do not propagate that row's claim into new planning without re-verifying it; it
   needs its own correction, out of this section's scope (docs-only edit to one file, per this
   task's charter).
5. **No `REGRESSION-LEDGER.md` row exists for A2 or A3 specifically** — only row 25 (A6/CORDIC)
   is present. §5 of this blueprint states A3 gets a ledger row ("money law parity pin —
   guardrail: kernel unit test, CI cargo-test"); live grep of the ledger finds no such row. This
   is real remaining paperwork (or an explicit written exemption, e.g. "A2/A3 are compiler
   self-tests, not behavior fixes" — but that exemption does not currently exist in writing
   either), not code — flagged, not fixed here.
6. **A3's authority flip remains explicitly OUT of scope**, unchanged by this section: the shadow
   organ passing its parity test is not license to switch `apply_tax`'s callers to
   `eqc_gen::apply_tax_*_int`. That flip is W4-L1, operator docket R-4 (§3.3, §4.1) — this
   verification pass does not touch or weaken that gate.
7. **`CORE-ROADMAP-STANDARD-2026-07-17.md` §3's Layer-A "New this pass" cell is stale** — it
   still reads "Wire `eqc-rs` into `geo.rs:39`/`domain.rs:95` (already identified, unstarted)",
   directly contradicted by §11.0's live evidence. Not corrected in this pass (that file is
   outside this task's one-file charter); named here so the next reader doesn't re-inherit it.
8. **CI coverage for A2/A3's specific tests was not independently re-verified this pass.** The
   `eqc-proofs` CI job (`.github/workflows/ci.yml:30-38`) runs only `tools/eqc-rs`'s own suite
   (covers A1/A6's proof/digest tests) — it does not run `cargo test -p kernel`, where A2's and
   A3's parity tests actually live. Whether *some* kernel-scoped CI job exists and is gating on
   `kernel`'s full `cargo test --lib` was not confirmed by name in this pass; state this as an
   open verification item rather than asserting either way.

### 11.4 Context & docs

- **Depends on:** nothing hard — Wave 0 per the master roadmap's execution-model rule (§8.5) and
  this standard's §4 wave assignment; `tools/eqc-rs` has zero runtime dependency from `kernel`
  (confirmed live, `grep eqc kernel/Cargo.toml` → no match, §11.0).
- **What depends on it:** per this blueprint's own §1 table, A1/A2/A3/A6 together are the W1-L1
  lane (compiler/organ/CORDIC legs) cited from `BLUEPRINT-BEBOP2-MESH-MASTERWORK-SYNTHESIS-V2.md`
  §F, grounded in Batch 1 + Batch 8. `CORE-ROADMAP-INDEX.md` §1's Layer-A row cites "P30 lanes
  W1-L1/L5/L10" as the rollup target. Nothing in the numbered P01–P30 set is presently blocked on
  these four items being open, because — per this pass — they are not.
- **Standard items this section satisfies:** `CORE-ROADMAP-STANDARD-2026-07-17.md` §2 item 1
  (ground truth, re-verified live — §11.0), item 2 (DoD — §11.2, retrospective form), item 17
  (regression tracking — partially satisfied; the two gaps are named in §11.3.4-5, not papered
  over), item 19 (reuse-first — this section proposes zero new machinery, pure verification).
- **`CORE-ROADMAP-INDEX.md`** Layer A row (§1, line 29: "Core kernel primitives... P30 lanes
  W1-L1/L5/L10") is the crosswalk entry this section's items roll up under.
- **Precedent this section extends:** commit `aebbbe199` ("live ground-truth status map —
  corrects stale blueprint RED-claims (A4/A5/A7/B/C/D/E/G all landed+green)") and
  `docs/design/GROUND-TRUTH-2026-07-17.md` — neither mentions A1/A2/A3/A6; this section is that
  same correction, scoped to the four items this pass was dispatched to check.
