...
| 62 | (spec-only in prose) | **DONE-VERIFIED** | `fdr/schema.rs` — `Absence::NoParent`, `span_id: Option<u64>` + `parent_span_id: Option<Reading<u64>>` on `FdrEvent`; `fdr/mod.rs` — `SPAN_SEQ` atomic minter, `SpanGuard` parent-child threading, `emit_span_close` linkage; `decision/mod.rs` golden-string test updated. 4 new tests: `span_close_root_serializes_no_parent_absence`, `span_close_child_serializes_parent_span_id_as_value`, `event_byte_identity_preserved_after_item_62`, `span_tree_reconstruction_from_recovered_ring`. Kernel 1152 pass; P3 grep proof green (no span_id in hash/gate/replay). |
