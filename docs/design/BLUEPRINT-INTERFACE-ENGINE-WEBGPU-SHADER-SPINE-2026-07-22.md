...
**Status:**EXECUTION-READY — ALL stages S0–S7 LANDED this pass: S0 (vendor menu), S0.5 (vendor assets), S1 (role-screen layouts), S2 (pixel-verification harness), S3 (WGSL shader spine + SINGLE-WRITER audit gate), S3.5 (academy-driven AI ranker + naga WGSL compile gate), S4 (compose_ui fragments wired to real vendor data), S5 (web shell renders composed vendor menu via CPU raster path + real vendor photos from CDN), S6 (real checkout → kernel order FSM via kernel's PUBLIC API, no kernel source edit), S7 (30-screen layout variants — 18 unique FNV signatures — across 15 owner + 5 courier sub-screens). Each stage has a falsifiable done-check that passes. **174+ engine tests green** (146 lib + 4 floor_parity + 1 router + 1 modal + 19 pixel_verify + 6 shader_audit [3 regex + 3 naga]), `cargo fmt --check` clean, `cargo tree -e no-dev` offline-clean (zero external crates; only the existing `dowiz-kernel` path-dep — naga is gated behind `feature="gpu"` so the default build is byte-unchanged). **Web smoke green**: `node web/src/render/compose.smoke.mjs` (8/8), `node --check web/src/app.js` (real Dubin & Sushi menu, food-hero photos from CDN, integer Lek pricing), `node web/src/render/fieldsim.smoke.mjs` (kernel-wasm buffers deterministic). Verified against a clean checkout of `kernel@9e576e467` (a temporary worktree used only because the live working tree carries uncommitted, unrelated edits that broke kernel's lib build; those edits are out of scope for this blueprint and were not touched).
...
- **Done-check (passes):** `cd engine && cargo test --test shader_audit` → 3/3 green (`every_read_write_storage_binding_has_single_writer_proof`, `audit_shader_rejects_unproven_binding` — the falsifier, `audit_shader_accepts_proven_binding`). Additionally `cargo test --features gpu --test shader_audit` adds 3 more (6/6 total): `naga_validates_every_shader` (catches syntax/type errors the regex cannot), `naga_rejects_invalid_wgsl` (falsifier), `naga_read_write_binding_has_single_writer_proof` (naga AST-backed equivalent of the prologue-scan — binds the variable NAME to the proof, not just proximity).
- **Innovate ceiling (RESOLVED — naga LANDED):** text-regex audit replaced by a typed WGSL AST audit. `naga::front::wgsl::parse_str` now validates every shader behind `feature="gpu"` (naga-30.0.0, offline-cached). The AST cross-check resolves each `read_write` storage binding by name, searches the source for its declaration, and asserts the `// SINGLE-WRITER:` proof window — structurally stronger than the regex prologue-scan. Default build remains offline-clean (zero external crates) because naga is gated behind `feature="gpu"`, matching the DECART and the air-gapped build mandate. The `ui.wgsl` shader was fixed from invalid-WGSL tuple syntax (which the regex audit could not catch) — a concrete find from the naga gate proving its value.
...
### S3.5 — Academy-driven deterministic AI ranker + naga WGSL compile gate (LANDED this pass)
...
**Act on Q2 (RESOLVED — naga LANDED):** naga was added as `cargo test --features gpu --test shader_audit` (6/6, 3 new naga tests) behind `feature="gpu"`, matching the offline-clean mandate. The S3 innovate-ceiling ticket is closed: the regex prologue-scan is now backed by a full WGSL parse via `naga::front::wgsl::parse_str`, and the AST cross-check binds each `read_write` variable by name. A concrete bug was found and fixed: `ui.wgsl` used invalid WGSL tuple returns (`-> (vec3, vec3, vec3)`) that no regex could catch. Follow-up remaining: when `wgpu` itself unlocks behind the same feature flag, the naga parse can move from a CI-only gate to a runtime compile step.
