...
|---|---|---|---|---|
| **Замкнутий регулятор** (насос+тріада+сепаратор) | `resonator.rs` / `resonator.ts` | bebop2 core; dowiz agent-governance | **90%** (control shape) | Rust-версія **не зареєстрована в lib.rs — мертвий код**; TS-порт розходиться у 4 місцях |
...
1. **`resonator.rs` не зареєстрований у `lib.rs`** (bebop2) — єдиний оркестратор стеку = **мертвий код**,
   6 тестів не бігають. Найдешевший unlock усієї системи: додати `#[cfg(feature="host")] pub mod
   resonator;`. Після цього Rust-оригінал (зі строгою метрикою, initial checkpoint, chaos-termination,
   baked-in rollback) стає reference, до якого звіряється TS-порт. **1 рядок.**
