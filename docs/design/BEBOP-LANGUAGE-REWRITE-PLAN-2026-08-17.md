...
1. Transactional snapshots (CoW arena, nanosecond rollback). 2. Fuzzy hypervector types (type = hypervector invariant, NTT-checked). 3. Self-Healing SMT (JIT-check of agent code → math counterexample). 4. Living Memory primitive (`memory`, mmap + NTT index + timestamps). 5. Zero-copy token streams (token-scene I/O, spectral filtering on the fly).
