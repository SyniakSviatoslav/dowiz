...
- A: money-tween + skin (RESOLVED-in-canonical 2026-07-12, ground-truth-verified): the canonical Svelte/Astro Storefront (`web/`) calls REAL kernel `place_order_js` with NO money-tween — node test PASSES: `real order id=ord_0 status=PENDING`, no count-up (Storefront.svelte calls real `mod.placeOrder`; CourierTrack.svelte comments "NO money tween / count-up"). The legacy Node/TS `apps/web` oracle STILL animates money in **4** sites (verified by grep): ClientLayout:154 `AnimatedNumber`, AnalyticsPage:262 `AnimatedNumber`, EarningsPage:47/176 `CountUpPrice`, DashboardPage:421/423 `AnimatedNumber` (revenue stat). `paperSkinAttr` is ACTIVE in 3 routes (CourierRoutes:45/57, ClientLayout:114, AdminRoutes:121) — there is NO `bebopSkinAttr` in the repo, so the original "bebop-skin token-flip" sub-task has no target (it would mean removing `paperSkinAttr()` spreads). Operator decision 2026-07-12: skip legacy edits; canonical stack satisfies the trust cue (money-never-tweens). RED (count-up assertion) satisfied by the canonical Storefront. Legacy oracle smell deferred to G11 swap.
- B: degrade-storm ratchet finish + sw.js push handler. RED = flags reset on restart; push renders. — **DONE+verified (2026-07-12, commit ab48fd3f):** ratchet implemented in server/src/reliability.rs (boot-grace + latched storm flag, process-global LazyLock, exposed via GET /api/healthz); sw.js push handler already existed (public/sw.js) — fixed its two defects: resubscribe now POSTs /api/courier/push/resubscribe (route added, was 404) + notification icons added (web/public/icons/*, were 404). 8/8 server tests pass incl. red_flags_reset_on_restart. (push renders = satisfied.)
- C: gitleaks CI hard-fail + sync-CRDT fence + P7 gate scaffolding. RED = canary fails scan; price in MenuDelta breaks CI. — **gitleaks CI hard-fail DONE+verified (2026-07-12, commit b10a7bfe):** gitleaks installed + `.gitleaks.toml` + git-mode `verify-secrets.ts` wired into CI (ci.yml `pnpm verify:secrets`); clean tree = 0 leaks, SHIPPING code stays strictly scanned. (sync-CRDT fence + P7 gate scaffolding remain — see Tier-4.)
- D: OG <300KB + channel attribution reader + QR+?ch=. RED = content-length<300KB; broken-QR detected. — **DONE+verified (2026-07-12, commit faa89541):** static og/card.png (3.4KB <300KB) + og:title/description/image + twitter meta in Layout.astro; ?ch=<venue> read in index.astro -> Storefront stamps order.channel (attributed via GET /api/orders/channel); client-side QR of ?ch= share link via qrcode dep. Channel-attribution READER already existed (OwnerDashboard + GET /api/orders/channel) — only OG + QR+?ch= were absent. Verified live: /og/card.png 200 @3384B, og: meta present, channel=tiktok order attributed.
...
Tier 2 (quality bars): design 13-item "stable enough" + storefront zero-diff Playwright gate (BUILT+VERIFIED 2026-07-12: e2e/tests/tier2-storefront-contract.spec.ts, 3/3 passed, chromium installed); GTM 8-point per-venue gate; courier out-of-app signal (N1/N2).
