...
| 13 | A whole CSS rule silently dropped by the browser — present in file/served/dist CSS, invisible to grep/typecheck/build/lint (the `[data-skin="paper"]` token block didn't apply) | A literal `*/` in CSS comment PROSE (`--ink-*/--paper-*`) closed the block comment early; the browser's error-recovery consumed the next rule until resync | `E2E` (live-DOM) + `lesson` | `e2e/tests/paper-skin-tokens.spec.ts` — addStyleTag(tokens.css)+getComputedStyle asserts the paper scope resolves (green) + a bugged-shape arm proves the check bites (red). Lesson `docs/lessons/2026-06-23-css-comment-star-slash.md` (TRIGGER `packages/ui/src/theme/**.css`) | 2026-06-23 · this change |
