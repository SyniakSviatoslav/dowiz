...
export function SkeletonBase({ className = '' }: { className?: string }) {
  return (
    <div className={`animate-pulse rounded bg-brand-surface-raised ${className}`} />
  );
}
...
export function EmptyState({ title, description, icon, action, fullPage = false }: { title: string; description: string; icon?: ReactNode; action?: ReactNode; fullPage?: boolean }) {
  ...
  const paperFallback = !icon && isPaperSkinEnabled()
    ...
    : null;
  const card = (
    <div className={`flex flex-col items-center justify-center p-8 text-center rounded-lg border border-dashed border-brand-border bg-brand-surface`}>
      {icon && <div className="mb-4 text-4xl text-brand-text-muted">{icon}</div>}
      {paperFallback && (
      ...
      <p className="mb-6 text-sm text-brand-text-muted max-w-sm">{description}</p>
      {action}
    </div>
  );
...
export function OfflineBanner({ fallbackPhone }: { fallbackPhone?: string }) {
  const { t } = useI18n();
  return (
    <div className={`fixed top-0 left-0 right-0 z-50 bg-semantic-warning text-brand-text px-4 py-2 text-sm font-medium text-center shadow-md`}>
...
export function WSStatusDot({ status }: { status: 'connecting' | 'connected' | 'disconnected' | 'reconnecting' | 'error' }) {
  const colors = {
    ...
    connecting: `bg-semantic-warning animate-pulse`,
    reconnecting: `bg-semantic-warning animate-pulse`,
    disconnected: 'bg-brand-text-muted',
