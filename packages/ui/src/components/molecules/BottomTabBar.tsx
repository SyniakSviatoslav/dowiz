...
export function BottomTabBar({ tabs, activeKey, onTabClick, className = '' }: BottomTabBarProps) {
  ...
  return (
    <nav
      className={`embed-hidden sticky bottom-0 left-0 right-0 z-sticky bg-brand-surface border-t border-brand-border ${className}`}
        ...
        {tabs.map(tab => {
          ...
          return (
            ...
            >
              {tab.icon && (
                <i
                  className={`${tab.icon} text-xl ${isActive ? 'text-brand-primary' : 'text-brand-text-muted'}`}
                />
              )}
              <span
              ...
              {tab.badge !== undefined && tab.badge > 0 && (
                <span className={`absolute -top-0.5 right-1/4 min-w-[18px] h-[18px] flex items-center justify-center rounded-full bg-semantic-danger text-white text-step-2xs font-bold px-1 leading-none`}>
