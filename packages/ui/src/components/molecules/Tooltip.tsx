...
export function Tooltip({ content, position = 'top', children }: TooltipProps) {
  ...
  return (
    <div className="relative inline-flex" onMouseEnter={show} onMouseLeave={hide}>
      ...
      >
        <div className="relative bg-[var(--brand-surface-raised)] border border-[var(--brand-border)] text-[var(--brand-text)] px-3 py-1.5 rounded-md text-xs max-w-[200px] whitespace-normal shadow-elevation-2">
          {content}
        </div>
        <div
          className={`absolute w-2.5 h-2.5 bg-[var(--brand-surface-raised)] border border-[var(--brand-border)] rotate-45 ${pos.arrow}`}
