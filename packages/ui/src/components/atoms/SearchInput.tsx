  ...
  return (
    <div className={`relative ${containerClassName}`}>
      ...
      <input
        type="search"
        value={value}
        className={`w-full ${sizeCls} ${onClear ? 'pr-10' : 'pr-4'} bg-brand-surface border border-brand-border rounded-[var(--brand-radius-sm)] text-brand-text placeholder-brand-text-muted font-body outline-none transition-[border-color,box-shadow] duration-150 ease-[var(--ease-soft)] motion-reduce:transition-none hover:border-brand-text-muted/60 focus:ring-2 focus:ring-brand-primary [&::-webkit-search-cancel-button]:hidden ${className}`}
        {...props}
      />
      {onClear && hasValue && (
        <button
