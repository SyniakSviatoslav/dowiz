...
// ── *-liveness family (SOFT — H3 anti-silent-skip, meta-loop-gap-map-2026-07-02) ─────────
// Four siblings of telemetry-liveness above: each lights up a silent death in the harness
// wiring (a producer that runs / a consumer that never drains) so pattern #12 holds —
// "a step that ends in prose with no artifact a machine inspects dies within a week".
// ALL SOFT / ADVISORY-LIVENESS-ONLY: the mirror is never a stick — they make omissions
// VISIBLE in the digest, they NEVER change the verdict or block unrelated work. They live
// INSIDE this guard region on purpose: their literal ledger/loops-runs references are
// stripped from the advisory-forever + ingestion-authority self-scans below, so surfacing
// the ledger does not read as wiring it into a gate.

// ── prediction-resolution-liveness (SOFT — GAP 1) ─── ADVISORY-LIVENESS-ONLY ─────────────
// The calibration ledger is a MIRROR (model-calibration.md:104 "never a stick"). It is
// write-only in practice: the resolve half never runs. Warn when unresolved predictions
// exceed BACKLOG_MAX (default 0) or the oldest unresolved is older than STALE_DAYS
// (default 14). Absent ledger → warn (no fact half at all). Soft: never gates.
const PRED_MAX = Number(process.env.PLANE_PREDICTION_BACKLOG_MAX || 0);
const PRED_STALE_DAYS = Number(process.env.PLANE_PREDICTION_STALE_DAYS || 14);
const predPath = 'loops/runs/predictions.jsonl';
let predOk, predDetail;
if (!has(predPath)) {
  predOk = false;
  predDetail = `no ${predPath} — calibration mirror has a prediction half and no fact half (H3: silence made VISIBLE; soft by design)`;
} else {
  let unresolved = 0, total = 0, oldestUnresolvedTs = Infinity;
  for (const line of read(predPath).split('\n')) {
    const s = line.trim();
    if (!s) continue;
    ...
    if (row && row.resolved === false) {
      unresolved++;
      const t = Date.parse(row.ts_predicted || '');
      if (Number.isFinite(t) && t < oldestUnresolvedTs) oldestUnresolvedTs = t;
    }
  }
  const oldestAgeD = Number.isFinite(oldestUnresolvedTs) ? (Date.now() - oldestUnresolvedTs) / 86400000 : 0;
  predOk = !(unresolved > PRED_MAX || oldestAgeD >= PRED_STALE_DAYS);
  predDetail = unresolved === 0
    ? `${total} prediction(s), all resolved — calibration fact-half is running`
    : `${unresolved}/${total} prediction(s) unresolved (oldest ${oldestAgeD.toFixed(1)}d) — the resolve half never ran (backlog>${PRED_MAX}; soft by design)`;
}
rec('prediction-resolution-liveness', `predictions resolved (backlog ≤ ${PRED_MAX})`, 'soft', predOk, predDetail);

// ── inbox-drain-liveness (SOFT — GAP 4) ─── ADVISORY-LIVENESS-ONLY ───────────────────────
// The Stop-hook FORCES a reflection into INBOX on every qualified change; nothing forces the
// librarian to drain it (production gated, consumption ungated). Warn when INBOX *.md count >
// DRAIN_MAX (default 3) or the oldest is older than DRAIN_DAYS (default 14). Curation needs a
// model → this can never be a hard gate; it is the smoke alarm, the librarian is the actuator.
// Empty/absent INBOX = drained = healthy.
const INBOX_MAX = Number(process.env.PLANE_INBOX_DRAIN_MAX || 3);
const INBOX_DAYS = Number(process.env.PLANE_INBOX_DRAIN_DAYS || 14);
const inboxDir = 'docs/reflections/INBOX';
let inboxOk, inboxDetail;
if (!has(inboxDir)) {
  inboxOk = true;
  inboxDetail = `${inboxDir} absent — nothing to drain`;
} else {
  const mdFiles = readdirSync(join(ROOT, inboxDir)).filter((f) => f.endsWith('.md'));
  let oldestTs = Infinity;
  for (const f of mdFiles) {
    const mt = statSync(join(ROOT, inboxDir, f)).mtimeMs;
    if (mt < oldestTs) oldestTs = mt;
  }
  const oldestAgeD = mdFiles.length ? (Date.now() - oldestTs) / 86400000 : 0;
  inboxOk = !(mdFiles.length > INBOX_MAX || oldestAgeD >= INBOX_DAYS);
  inboxDetail = mdFiles.length === 0
    ? 'INBOX empty — drained'
    : `${mdFiles.length} reflection file(s) un-curated (oldest ${oldestAgeD.toFixed(1)}d) — librarian backlog (max ${INBOX_MAX}; soft by design)`;
}
rec('inbox-drain-liveness', `INBOX drained (≤ ${INBOX_MAX} files)`, 'soft', inboxOk, inboxDetail);

// ── scout-liveness (SOFT — GAP 2) ─── ADVISORY-LIVENESS-ONLY ─────────────────────────────
// scout-feeds.mjs + asset-surface-scan.mjs were built with tests but nothing on any cadence
// invokes them, so their cursors never get written. Warn when a scout's script exists but its
// cursor is absent (never ran) or stale (> LIVENESS_DAYS, default 7). No script → N/A. Absent
// cursor IS the warn — a built-but-never-run scout produces zero signal.
const SCOUT_DAYS = Number(process.env.PLANE_SCOUT_LIVENESS_DAYS || 7);
const scoutPairs = [
  ['scripts/scout-feeds.mjs', 'loops/runs/scout-cursor.json'],
  ['scripts/asset-surface-scan.mjs', 'loops/runs/asset-surface-baseline.json'],
];
const scoutInstalled = scoutPairs.filter(([s]) => has(s));
let scoutOk, scoutDetail;
if (scoutInstalled.length === 0) {
  scoutOk = true;
  scoutDetail = 'no scout scripts installed — N/A';
} else {
  const problems = [];
  for (const [script, cursor] of scoutInstalled) {
    if (!has(cursor)) { problems.push(`${script} never ran (no ${cursor})`); continue; }
    const ageD = (Date.now() - statSync(join(ROOT, cursor)).mtimeMs) / 86400000;
    if (ageD >= SCOUT_DAYS) problems.push(`${cursor} stale ${ageD.toFixed(1)}d`);
  }
  scoutOk = problems.length === 0;
  scoutDetail = problems.length === 0
    ? `${scoutInstalled.length} scout(s) fresh (< ${SCOUT_DAYS}d)`
    : `scouts silent: ${problems.join('; ')} (soft by design — silence made VISIBLE)`;
}
rec('scout-liveness', `scout cursors fresh (< ${SCOUT_DAYS}d)`, 'soft', scoutOk, scoutDetail);

// ── health-pass-freshness (SOFT — GAP 7) ─── ADVISORY-LIVENESS-ONLY ──────────────────────
// agent-health-pass (counsel Duty B) is wired into no automation. The newest
// docs/governance/agent-health-*.md must be < FRESHNESS_DAYS old (default 7). Absent → warn
// (health pass produced no artifact). Soft: advisory freshness, never blocks CI.
const HEALTH_DAYS = Number(process.env.PLANE_HEALTH_FRESHNESS_DAYS || 7);
const govDir = 'docs/governance';
let healthTs = 0, healthSrc = 'none';
if (has(govDir)) {
  for (const f of readdirSync(join(ROOT, govDir))) {
    if (!/^agent-health-.*\.md$/.test(f)) continue;
    const mt = statSync(join(ROOT, govDir, f)).mtimeMs;
    if (mt > healthTs) { healthTs = mt; healthSrc = `${govDir}/${f}`; }
  }
}
const healthAgeD = healthTs ? (Date.now() - healthTs) / 86400000 : Infinity;
rec('health-pass-freshness', `newest agent-health < ${HEALTH_DAYS}d`, 'soft', healthAgeD < HEALTH_DAYS,
  healthTs === 0
    ? 'no docs/governance/agent-health-*.md — health pass produced no artifact (H3: silence made VISIBLE; soft by design)'
    : healthAgeD < HEALTH_DAYS
      ? `newest health pass ${healthAgeD.toFixed(2)}d old via ${healthSrc}`
      : `health pass STALE — newest ${healthAgeD.toFixed(1)}d old via ${healthSrc} — runs in no automation? (soft by design)`);
