...
const STEPS = [
  ...
  // SMC dead-state auditor (ledger #79): every OrderStatus enum value must be reachable
  // (insert-entry / TRANSITIONS edge / DB-fn writer) or explicitly allowlisted (KNOWN_DEAD).
  // Direct tsx invoke like the sibling static gates (package.json is protect-paths).
  { name: 'SMC dead-state auditor (state-machine coverage, ledger #79)', cmd: `tsx apps/api/scripts/verify-state-machine-coverage.ts`, ci: true },
