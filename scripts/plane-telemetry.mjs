...
//              (field-scoped redaction, fail-closed → redactor_error stub on throw)
...
//   digest   [--run-id R] [--since 24h] [--status-line] → <1s rollup from the branch (+ local
//            scratch), schema_version-filtered, month-boundary glob, `telegram=… · push=…` line.
...
function cmdEmit(args) {
  ...
  let detail; let note; let target;
  try {
  ...
  const event = {
    ...
    kind,
    step: String(args.step || kind.toUpperCase()),
    outcome,
    target,
    detail,
    ...(note !== undefined ? { note } : {}),
...
function cmdDigest(args) {
  ...
  const out = [
    ...
    `events=${events.length} runs=${new Set(events.map((e) => e.run_id)).size}`,
    `by kind: ${by('kind')}`,
    `by outcome: ${by('outcome')}`,
    fails.length ? `top fails:\n${fails.slice(0, 3).map((e) => ` - [${e.run_id}] ${e.target}: ${e.detail}`).join('\n')}` : 'top fails: (none)',
    `calib: predicted=${predictions.length} resolved=${resolved.length} hit=${resolved.filter((p) => p.gap === 'hit').length} miss=${resolved.filter((p) => p.gap === 'miss').length} partial=${resolved.filter((p) => p.gap === 'partial').length} (reliability, not a score)`,
    statusLine(),
  ];
...
usage: node scripts/plane-telemetry.mjs <emit|predict|resolve|send|publish|digest|inbox> [--flags]
