...
pub async fn analytics(req: Request, ctx: RouteContext<()>) -> Result<Response> {
    ...
    for o in orders_of(listed, &loc) {
        ...
        let refused = !crate::services::orders::status::took_money(status);
        orders += 1;
        if refused {
            rejected += 1;
        }
        // `venue_took` is the one definition, tested beside the statuses it
        // reads: a refused or refunded order is nothing, and the tip is the
        // courier's money passing through.
        let took = crate::services::orders::status::venue_took(
