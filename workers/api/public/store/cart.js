...
export function openCart(){
  ...
  sheet(`
    ...
    <div class="clines">${lines.map(l => `<div class="cline">
      ${l.p.imageUrl ? `<img class="cthumb" src="${esc(l.p.imageUrl)}" alt="" loading="lazy">` : `<span class="cthumb cthumb-x"></span>`}
      ...
      <span class="qty"><button type="button" data-m="${esc(l.k)}" aria-label="−">−</button>
        <span>${l.q}</span><button type="button" data-a="${esc(l.k)}" aria-label="+">+</button></span></div>`).join('')}</div>
