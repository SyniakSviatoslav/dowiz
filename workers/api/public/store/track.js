...
// total, the way to be told, and the sentence the customer can leave. A
// customer with reduced motion or no WebGL loses nothing: the pills say it all.
//
...
export function openTracking(order){
  ...
  sheet(`
    <div class="tsheet" data-status="${esc(st)}">
      <p class="eyebrow">#${esc(String(order.id).slice(0, 8))}</p>
