...
/// The currencies this product renders. A code outside this union is a typo,
/// and the compiler is the cheapest place to find one: every call below took a
/// bare `string` before, so `money(n, 'EURO')` type-checked and then formatted
/// as a fallback nobody read.
export type CurrencyCode = 'ALL' | 'EUR' | 'USD';

/// Minor units per major unit. Lek has none; the euro and the dollar have two.
export const DECIMALS: Record<CurrencyCode, number> = { ALL: 0, EUR: 2, USD: 2 };

/// What `/api/public/rates` ships: parts per million of each target per one
/// unit of `base`.
export interface Rates {
  base: string;
  ppm: Record<string, number>;
  decimals?: Record<string, number>;
  /// True when the rates could not be fetched and the identity rate stands.
  stale?: boolean;
  asOf?: string;
}
...
export const CURRENCIES: readonly CurrencyCode[] = ['ALL', 'EUR', 'USD'];

export function decimalsOf(code: string): number {
  return DECIMALS[code as CurrencyCode] ?? 2;
}
...
export function format(amount: number, code: string, locale?: string): string {
...
export function formatter(
  { base, display, rates, locale }:
  { base: string; display?: string | null; rates?: Rates | null; locale?: string },
): (amount: number) => string {
  const want = display || base;
  const ppm: number | undefined = rates && rates.ppm ? rates.ppm[want] : undefined;
...
export async function loadRates(base: string, apiBase = '/api'): Promise<Rates> {
  try {
...
export function preferred(base: CurrencyCode | string): string {
  try {
    const v = localStorage.getItem(KEY);
    if (v && CURRENCIES.includes(v as CurrencyCode)) return v;
