...
//!
//! MOVED FROM `workers/api/src/command/room_rules.rs` (D7 phase 1), which now
//! re-exports this module, so `transfer` in the Worker and `amend` here share it.

use super::Refused;
use serde_json::{json, Value};

/// Why a line left a round, or why it was not charged. A CLOSED set, the
/// `WasteReason` pattern (`crate::stock`): a reason is a word the owner can
...
pub fn reprice(order: &mut Value, items: Vec<Value>, discount: i64) -> Result<(), Refused> {
    ...
    let paid = super::pay::paid_of(order);
    if paid > total {
...
pub fn restock(
    stock: &mut crate::stock::StockLog,
    order_id: &str,
    items: &[Value],
    boms: &[(String, String)],
) -> Result<(), Refused> {
    let led = stock.ledger().map_err(|e| Refused::Stock(e.to_string()))?;
    let mut evs = crate::stock::settle(&led, order_id, false);
    let lines: Vec<(String, i64)> = items
        ...
        .collect();
    evs.extend(crate::stock::reservations_for(order_id, &lines));
    if evs.is_empty() {
        return Ok(());
    }
