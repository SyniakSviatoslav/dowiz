...
pub struct SelfReproductionReport {
    ...
    ///
    /// `no_std` has no wall clock: this is `0` on production. The caller is
    /// expected to stamp `timestamp_us` at the port/FFI boundary once a host
    /// clock is available.
...
impl SelfReproducer {
    ...
    pub fn inspect(&mut self, source_bytes: &[u8]) -> SelfReproductionReport {
        ...
        let report = SelfReproductionReport {
            // no_std: no wall clock — the caller stamps `timestamp_us` at the
            // port/FFI boundary once a host clock is available.
            timestamp_us: 0,
            organs,
            total_lines,
            report_hash,
