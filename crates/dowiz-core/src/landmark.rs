...
impl Similarity {
    ...
    pub fn apply(&self, p: Point) -> Point {
        let (x, y) = p;
        let (c, s) = (crate::math::cos(self.theta), crate::math::sin(self.theta));
        let rx = c * x - s * y;
        let ry = s * x + c * y;
...
pub fn procrustes(src: &[Point], dst: &[Point]) -> Option<Similarity> {
    ...
    let scale = crate::math::sqrt(a * a + b * b) / norm;
    let theta = crate::math::atan2(b, a);
    // Translation maps the source centroid (scaled+rotated) onto dst centroid.
    let (c, s) = (crate::math::cos(theta), crate::math::sin(theta));
