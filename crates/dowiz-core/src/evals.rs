...
pub fn aurc(prob: &[f64], outcome: &[u8]) -> f64 {
    ...
    for (k, &i) in idx.iter().enumerate() {
        if (crate::math::round(prob[i]) as u8) != outcome[i] {
            cum_err += 1;
        }
...
//  - Persistence is opt-in via `EvalRow::to_jsonl` (pure) + the kernel shim's
//    `append_to` free function; the gate and EMA are pure (no fs) so they stay
//    testable offline. Writing to disk is the caller's act (analyze.mjs
//    pipeline), never hidden inside the kernel.
// ─────────────────────────────────────────────────────────────────────────────
...
/// Push a JSON string literal (quoted + escaped) onto `out`.
...
impl EvalRow {
    ...
    /// Emit the JSONL line `analyze.mjs` expects. Compact JSON (no whitespace),
    /// hand-rolled string building — no serde. Strings are escaped for `"`,
    /// `\`, and control chars so the `analyze.mjs` consumer re-parses as JSON.
    pub fn to_jsonl(&self) -> String {
        let mut s = String::new();
        s.push('{');
        s.push_str("\"timestamp\":");
        push_json_string(&mut s, &self.timestamp);
        s.push_str(",\"config_version\":");
        push_json_string(&mut s, &self.config_version);
        s.push_str(",\"meta\":{\"category\":");
        push_json_string(&mut s, &self.category);
        s.push_str(",\"subagent\":");
        push_json_string(&mut s, &self.subagent);
        s.push_str(",\"model\":");
        push_json_string(&mut s, &self.model);
        s.push_str("},\"core\":{\"passed\":");
        s.push_str(if self.passed { "true" } else { "false" });
        s.push_str(",\"gating_failed\":[");
        for (i, g) in self.gating_failed.iter().enumerate() {
            if i > 0 {
                s.push(',');
            }
            push_json_string(&mut s, g);
        }
        s.push_str("],\"soft_failed\":[");
        for (i, f) in self.soft_failed.iter().enumerate() {
            if i > 0 {
                s.push(',');
            }
            push_json_string(&mut s, f);
        }
        s.push_str("],\"checks\":[");
        for (i, c) in self.checks.iter().enumerate() {
            ...
            s.push_str(",\"passed\":");
            s.push_str(if c.passed { "true" } else { "false" });
            s.push_str(",\"durationMs\":");
            s.push_str(&c.duration_ms.to_string());
            s.push('}');
        }
        s.push_str("]}}");
        s
...
mod tests {
    ...
    fn eval_row_schema_matches_analyze_mjs() {
        ...
        let back = crate::json::parse(&line).expect("parse");
        assert_eq!(back.get("timestamp").and_then(|v| v.as_str()), Some("1700000000+00:00"));
        assert_eq!(back.get("config_version").and_then(|v| v.as_str()), Some("42"));
        assert_eq!(back.get("meta").and_then(|v| v.get("category")).and_then(|v| v.as_str()), Some("eval"));
        assert_eq!(back.get("core").and_then(|v| v.get("passed")).and_then(|v| v.as_bool()), Some(true));
        let checks = back.get("core").and_then(|v| v.get("checks")).and_then(|v| v.as_array()).expect("checks array");
        assert_eq!(checks[0].get("name").and_then(|v| v.as_str()), Some("recall_at_k"));
        assert_eq!(checks[0].get("durationMs").and_then(|v| v.as_i64()), Some(12));
