...
impl GeometricPoint {
    ...
    pub fn geodesic_step(&self, velocity: &[f64; GEO_DIMS], dt: f64, christoffel: &ChristoffelSymbols) -> GeometricPoint {
        ...
        for i in 0..GEO_DIMS {
            ...
            new_coords[i] = self.coords[i] + velocity[i] * dt - 0.5 * acc * crate::math::powi(dt, 2);
        }
...
impl MetricTensor {
    ...
    pub fn flrw(scale_factor: f64) -> Self {
        let mut g = [[0.0; GEO_DIMS]; GEO_DIMS];
        g[0][0] = -1.0;        // час
        g[1][1] = crate::math::powi(scale_factor, 2);
        g[2][2] = crate::math::powi(scale_factor, 2);
        g[3][3] = crate::math::powi(scale_factor, 2);
        for i in 4..GEO_DIMS { g[i][i] = 1.0; }
        MetricTensor { g }
    }
...
impl GeometricState {
    ...
    pub fn forward(&self, dt: f64) -> GeometricState {
        ...
        for i in 0..GEO_DIMS {
            ...
            new_pos[i] = self.position.coords[i] + self.velocity[i] * dt + 0.5 * self.acceleration[i] * crate::math::powi(dt, 2);
        }
        GeometricState { position: GeometricPoint { coords: new_pos }, velocity: new_vel, acceleration: self.acceleration, tick: self.tick + 1 }
    }
    ...
    pub fn backward(&self, dt: f64) -> GeometricState {
        ...
        for i in 0..GEO_DIMS {
            ...
            prev_pos[i] = self.position.coords[i] - self.velocity[i] * dt + 0.5 * self.acceleration[i] * crate::math::powi(dt, 2);
        }
...
impl PhaseSpace {
    ...
    pub fn add_node(&mut self, name: &str) {
        ...
        for i in 0..GEO_DIMS {
            let angle = crate::math::sin((idx as f64 * 2.0 * core::f64::consts::PI / 7.0));
            coords[i] = crate::math::sin((angle * (i as f64 + 1.0))) * 10.0;
        }
...
impl Complex {
    ...
    pub fn norm(&self) -> f64 { crate::math::sqrt(self.norm_sq()) }
...
impl Qubit {
    pub fn new(alpha: Complex, beta: Complex) -> Self {
        let n = crate::math::sqrt((alpha.norm_sq() + beta.norm_sq()));
    ...
    pub fn plus() -> Self { Qubit::new(Complex::one().scale(crate::math::sqrt(2.0f64).recip()), Complex::one().scale(crate::math::sqrt(2.0f64).recip())) }
    pub fn minus() -> Self { Qubit::new(Complex::one().scale(crate::math::sqrt(2.0f64).recip()), Complex::one().scale(crate::math::sqrt(-2.0f64).recip())) }
    ...
    pub fn hadamard(&self) -> Qubit {
        let s = crate::math::sqrt(2.0f64).recip();
...
impl Hamiltonian {
    ...
    pub fn evolve(&self, qubit: &Qubit, dt: f64) -> Qubit {
        ...
        let new_alpha = a.add(&b.scale(crate::math::sin(-(self.omega_x * theta))));
        let new_beta = b.add(&a.scale(crate::math::sin(-(self.omega_x * theta))));
        Qubit::new(new_alpha, new_beta)
    }

    /// Власні значення: E = ±ℏ·||ω||/2
    pub fn eigenvalues(&self) -> (f64, f64) {
        let w = crate::math::sqrt((crate::math::powi(self.omega_x, 2) + crate::math::powi(self.omega_y, 2) + crate::math::powi(self.omega_z, 2)));
...
impl QuantumMeasurement {
    ...
    pub fn measure(&mut self, qubit: &Qubit) -> u8 {
        ...
        let r = (crate::math::fract((self.shots as f64 * 1.618033988749895)) + 0.5) % 1.0;
...
impl Superposition {
    ...
    pub fn measure(&self, seed: u64) -> Option<&BasisState> {
        ...
        let r = (crate::math::fract((seed as f64 * 1.618033988749895)) + 0.5) % 1.0;
        let mut cum = 0.0;
    ...
    pub fn interfere(&self) -> Complex {
        ...
        for bs in &self.basis {
            let phase = Complex::new(crate::math::cos((bs.time_offset * core::f64::consts::TAU)), crate::math::sin((bs.time_offset * core::f64::consts::TAU)));
...
impl Spin {
    ...
    pub fn measure_up_probability(&self) -> f64 {
        crate::math::powi(self.amplitude, 2) * crate::math::powi(crate::math::cos((self.phase * 0.5)), 2)
    }
...
impl StandardModel {
    ...
    fn three_body_symplectic_step() {
        ...
        let e0 = state.velocities.iter().flat_map(|v| v.iter()).map(|x| crate::math::powi(*x, 2)).sum::<f64>() * 0.5;
        state.symplectic_step(0.01);
        let e1 = state.velocities.iter().flat_map(|v| v.iter()).map(|x| crate::math::powi(*x, 2)).sum::<f64>() * 0.5;
...
impl PseudoEuclideanMetric {
    ...
    pub fn pseudo_orthogonal_transform(&self, angles: &[f64]) -> Vec<Vec<f64>> {
        ...
        for k in 0..angles.len().min(n/2) {
            ...
            if j >= n { break; }
            let theta = angles[k];
            let c = crate::math::cos(theta);
            let s = crate::math::sin(theta);
            m[i][i] = c; m[i][j] = -s;
            m[j][i] = s; m[j][j] = c;
        }
        m
...
impl NdSpace {
    ...
    pub fn interval(&self, i: usize, j: usize) -> f64 {
        ...
        crate::math::sqrt(self.metric.interval_sq(&dx))
    }
        ...
        for i in 0..n {
            ...
            match self {
                SplitAlgebra::SplitComplex { .. } => {
                    ...
                    points.push(SplitAlgebra::SplitComplex { a: crate::math::cos(theta), b: crate::math::cos(theta) });
                }
                SplitAlgebra::SplitQuaternion { .. } => {
                    // a² + b² = c² + d² — сфера S² в нульовому конусі
                    let s = crate::math::sin(theta) * crate::math::sqrt(0.5f64);
                    points.push(SplitAlgebra::SplitQuaternion { a: crate::math::cos(theta), b: s, c: s, d: 0.0 });
                }
                SplitAlgebra::SplitOctonion { .. } => {
                    let mut c = [0.0; 8];
                    c[0] = crate::math::cos(theta);
                    for j in 1..4 { c[j] = crate::math::sin(theta) / crate::math::sqrt(3.0f64); }
                    for j in 4..8 { c[j] = crate::math::sin(theta) / crate::math::sqrt(3.0f64); }
...
impl NullGeodesic {
    ...
    pub fn has_reached(&self, target: &NdVector, tol: f64) -> bool {
        ...
        let d2: f64 = self.position.coords.iter().zip(&target.coords)
            .map(|(a, b)| crate::math::powi((a - b), 2)).sum();
        d2 < tol * tol
    }
    ...
    pub fn spacetime_interval_to(&self, target: &NdVector, p: usize) -> f64 {
        ...
        crate::math::sqrt(v.norm_sq(p).abs())
    }
}
...
impl LightFront {
    ...
    pub fn expand(&mut self, dt: f64, n_dims: usize) -> Vec<NdVector> {
        ...
        for i in 0..n_points {
            ...
            for j in 1..n_dims.min(4) {
                ...
                coords[j] = self.radius * crate::math::sin(angle) * 0.5 / crate::math::sqrt((n_dims as f64));
            }
            if n_dims > 4 {
                let spread = self.radius / crate::math::sqrt((n_dims as f64));
                for j in 4..n_dims {
                    coords[j] = spread * crate::math::sin((theta * (j as f64)));
                }
            }
    ...
    pub fn has_reached(&self, point: &NdVector) -> bool {
        let dx: f64 = crate::math::sqrt(self.center.coords.iter().zip(&point.coords)
            .skip(1).map(|(a, b)| crate::math::powi((a - b), 2)).sum::<f64>());
...
impl LocalTime {
    pub fn new(potential: f64) -> Self {
        let c = 299_792_458.0;
        let dilation = crate::math::sqrt((1.0 + 2.0 * potential / (c * c))).max(0.1).min(1.0);
...
impl GravitationalField {
    ...
    pub fn potential_at(&self, point: &NdVector) -> f64 {
        ...
        for i in 0..self.masses.len() {
            let dx: f64 = crate::math::sqrt(point.coords.iter().zip(&self.positions[i].coords)
                .map(|(a, b)| crate::math::powi((a - b), 2)).sum::<f64>());
    ...
    pub fn force_at(&self, point: &NdVector) -> NdVector {
        ...
        for i in 0..self.masses.len() {
            let dx: Vec<f64> = point.coords.iter().zip(&self.positions[i].coords)
                .map(|(a, b)| b - a).collect();
            let r = crate::math::sqrt(dx.iter().map(|x| crate::math::powi(*x, 2)).sum::<f64>()).max(1e-12);
            let coeff = self.g_const * self.masses[i] / (crate::math::powi(r, 3));
            for j in 0..f.len() {
                f[j] += coeff * dx[j];
            }
        }
    ...
    pub fn tick(&mut self, dt: f64) {
        ...
        for (i, lt) in self.local_times.iter_mut().enumerate() {
            ...
            if i < self.field.positions.len() {
                ...
                let c = 299_792_458.0;
                lt.dilation = crate::math::sqrt((1.0 + 2.0 * lt.potential / (c * c))).max(0.1).min(1.0);
            }
            lt.tick(dt);
        }
    }
...
impl UnifiedNavigator {
    ...
    pub fn navigate(&self, from_idx: usize, to_idx: usize) -> f64 {
        ...
        let interval = crate::math::sqrt(self.metric.interval_sq(&delta).abs());
...
impl TemporalNeuralNet {
    ...
    pub fn forward(&self, input: &[f64], time: f64) -> Vec<f64> {
        ...
        let te: Vec<f64> = (0..8).map(|i| {
            let freq = 1.0 / (crate::math::powf(10000.0_f64, 2.0 * i as f64 / 8.0));
            crate::math::sin((time * freq))
        }).collect();
        x.extend(&te);
...
impl TimeOracle {
    ...
    pub fn observe(&mut self, state: TimeState, nn: &TemporalNeuralNet) {
        ...
        for t in 1..=5 {
            ...
            let input: Vec<f64> = pos_coords.iter().copied().chain(core::iter::once(current.time + t as f64)).collect();
    ...
    fn compute_deltas(&mut self) {
        ...
        if let (Some(present), Some(past)) = (&self.present_state, self.past_states.last()) {
            let d: f64 = crate::math::sqrt(present.position.coords.iter().zip(&past.position.coords)
                .map(|(a, b)| crate::math::powi((a - b), 2)).sum::<f64>());
            self.deltas.push(d);
        }
        // Дельта: теперішнє → майбутнє (перший прогноз)
        if let (Some(present), Some(future)) = (&self.present_state, self.future_predictions.first()) {
            let d: f64 = crate::math::sqrt(future.position.coords.iter().zip(&present.position.coords)
                .map(|(a, b)| crate::math::powi((a - b), 2)).sum::<f64>());
            self.deltas.push(d);
        }
        // Дельта: минуле → майбутнє (повна зміна)
        if let (Some(past), Some(future)) = (self.past_states.last(), self.future_predictions.first()) {
            let d: f64 = crate::math::sqrt(future.position.coords.iter().zip(&past.position.coords)
                .map(|(a, b)| crate::math::powi((a - b), 2)).sum::<f64>());
            self.deltas.push(d);
        }
    }
    ...
    pub fn execute(&mut self, order: OrderType, price: f64) {
        match order {
            OrderType::Buy if self.budget >= price => {
                let qty = crate::math::floor((self.budget / price));
                self.position += qty;
...
impl ThreeBodyState {
    ...
    pub fn gravitational_force(a: usize, b: usize, pos: &[GeometricPoint; 3], masses: &[f64; 3]) -> [f64; 8] {
        ...
        let r2: f64 = dx.iter().map(|d| crate::math::powi(*d, 2)).sum::<f64>().max(1e-12);
    ...
    pub fn configure_lagrange_triangular(&mut self, side: f64) {
        ...
        self.positions[2].coords[1] = side * crate::math::sqrt(3.0f64) * 0.5;
    }
...
impl ThreeBodyNodeBus {
    ...
    pub fn dashboard(&self) -> String {
        format!("ThreeBodyNodeBus\n  Topology: {:?}\n  Round:    {}\n  Stable:   {}\n  Masses:   {:.1} {:.1} {:.1}\n  Energy:   {:.4}",
            ...
            self.state.velocities.iter().flat_map(|v| v.iter()).map(|x| crate::math::powi(*x, 2)).sum::<f64>() * 0.5)
    }
}
...
impl TriStateContext {
    ...
    pub fn update(&mut self, observation: &[f64], nn: &TemporalNeuralNet) {
        ...
        // Дельти
        self.delta_past_present = crate::math::sqrt(self.past.iter().zip(&self.present).map(|(a,b)| crate::math::powi((a-b), 2)).sum::<f64>());
        self.delta_present_future = crate::math::sqrt(self.present.iter().zip(&self.future).map(|(a,b)| crate::math::powi((a-b), 2)).sum::<f64>());
        self.delta_full = crate::math::sqrt(self.past.iter().zip(&self.future).map(|(a,b)| crate::math::powi((a-b), 2)).sum::<f64>());
...
impl TriStateAgent {
    ...
    pub fn decide(&mut self, observation: &[f64], expected: &[f64]) -> Vec<f64> {
        ...
        if !expected.is_empty() {
            let err: f64 = action.iter().zip(expected).map(|(a, e)| crate::math::powi((a - e), 2)).sum();
...
impl Matrix {
    ...
    pub fn eigenvalues_2x2(&self) -> (f64, f64) {
        ...
        else { let sqrt_disc = crate::math::sqrt(disc); ((trace + sqrt_disc) * 0.5, (trace - sqrt_disc) * 0.5) }
    }

    /// Frobenius норма.
    pub fn norm(&self) -> f64 {
        crate::math::sqrt(self.data.iter().flat_map(|r| r.iter()).map(|x| crate::math::powi(*x, 2)).sum::<f64>())
    }
...
impl NdExpert {
    pub fn new(name: &str, n_dims: usize, input_dim: usize, output_dim: usize) -> Self {
        ...
        for i in 0..n_dims { embedding[i] = crate::math::sin((i as f64 * 1.618)) * 10.0; }
