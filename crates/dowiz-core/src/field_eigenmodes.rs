    ...
    pub fn dct_modes_full_grid(&self) -> Decomp {
        ...
        for p in 0..w {
            for q in 0..h {
                let lam = 2.0
                    * (2.0
                        - crate::math::cos(core::f64::consts::PI * p as f64 / w as f64)
                        - crate::math::cos(core::f64::consts::PI * q as f64 / h as f64));
                pairs.push((p, q, lam));
            }
        }
        ...
        for (p, q, lam) in pairs {
            ...
            for y in 0..h {
                for x in 0..w {
                    let c = crate::math::cos(
                        core::f64::consts::PI * p as f64 * (x as f64 + 0.5) / w as f64,
                    ) * crate::math::cos(
                        core::f64::consts::PI * q as f64 * (y as f64 + 0.5) / h as f64,
                    );
                    let idx = y * w + x;
                    v[idx] = c;
                    ssq += c * c;
...
pub fn modal_advance(basis: &[Vec<f64>], values: &[f64], u0: &[f64], t: f64) -> Vec<f64> {
    ...
    for k in 0..r {
        ...
        let decay = crate::math::exp(-values[k] * t);
        let ck = c * decay;
        for i in 0..n {
            out[i] += ck * phi[i];
        }
    ...
    for gy in 0..grid.h {
        for gx in 0..grid.w {
            ...
            for (a, b) in [(1usize, 0usize), (0, 1), (1, 1), (2, 0), (0, 2)] {
                ...
                val += amp
                    * crate::math::cos(core::f64::consts::PI * a as f64 * x)
                    * crate::math::cos(core::f64::consts::PI * b as f64 * y);
            }
            out[i] = val;
        }
    }
...
mod tests {
    ...
    fn t2_eigenvalue_map_graph_and_field_sign() {
        ...
        for p in 0..4usize {
            for q in 0..4usize {
                let lg = 2.0
                    * (2.0
                        - crate::math::cos(core::f64::consts::PI * p as f64 / 4.0)
                        - crate::math::cos(core::f64::consts::PI * q as f64 / 4.0));
                target.push(lg);
            }
        }
    ...
    fn masked_grid_modal_basis_is_orthonormal_eigen() {
        ...
        for a in 0..n {
            ...
            let mut res = 0.0;
            for i in 0..n {
                res += crate::math::powi(lphi[i] - lam * basis[a][i], 2);
            }
            res = crate::math::sqrt(res) / crate::math::sqrt(n as f64);
