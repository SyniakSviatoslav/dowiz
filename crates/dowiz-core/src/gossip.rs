#![allow(unused)]
//! `dowiz_core::gossip` — lightweight topic-based pub/sub gossip (pure core).
//!
//! Timestamp-injected: `GossipMessage::new` / `GossipBus::publish` /
//! `GossipNode::publish` take `now_ms` explicitly; the kernel shim stamps it.
//!
...
impl GossipMessage {
    pub fn new(topic: GossipTopic, payload: Vec<u8>, seq: u64, now_ms: u64) -> Self {
        GossipMessage { topic, payload, timestamp_ms: now_ms, seq }
    }
...
impl GossipBus {
    ...
    pub fn publish(&mut self, topic: GossipTopic, payload: &[u8], now_ms: u64) {
        self.seq += 1;
        let msg = GossipMessage::new(topic, payload.to_vec(), self.seq, now_ms);
...
impl GossipNode {
    ...
    pub fn publish(&self, bus: &mut GossipBus, topic: GossipTopic, payload: &[u8], now_ms: u64) {
        bus.publish(topic, payload, now_ms);
    }
...
mod tests {
    ...
    fn subscribe_and_publish() {
        let mut bus = GossipBus::new();
        let id = bus.subscribe(GossipTopic::Telemetry);
        bus.publish(GossipTopic::Telemetry, b"cpu:0.5", NOW);
    ...
    fn multiple_subscribers() {
        ...
        bus.publish(GossipTopic::Prediction, b"latency:0.8", NOW);
    ...
    fn topic_isolation() {
        let mut bus = GossipBus::new();
        let id = bus.subscribe(GossipTopic::Telemetry);
        bus.publish(GossipTopic::Backup, b"snapshot", NOW);
        assert!(!bus.has_messages(id));
        bus.publish(GossipTopic::Telemetry, b"cpu:0.3", NOW);
        assert!(bus.has_messages(id));
    }
    ...
    fn unsubscribe() {
        ...
        bus.publish(GossipTopic::Error, b"err", NOW);
        assert!(!bus.has_messages(id));
    }
    ...
    fn gossip_node_register() {
        ...
        node.publish(&mut bus, GossipTopic::Telemetry, b"mem:0.7", NOW);
    ...
    fn queue_bounded() {
        ...
        for _ in 0..1500 {
            bus.publish(GossipTopic::Custom(1), b"data", NOW);
        }
        let msgs = bus.drain(id);
    ...
    fn multiple_topics_one_subscriber() {
        ...
        bus.publish(GossipTopic::Telemetry, b"cpu:0.5", NOW);
        bus.publish(GossipTopic::Error, b"err:1", NOW);
        let msgs = bus.drain(id);
    ...
    fn gossip_no_subscribers_no_crash() {
        let mut bus = GossipBus::new();
        bus.publish(GossipTopic::Telemetry, b"data", NOW);
    ...
    fn gossip_remove_and_resubscribe() {
        ...
        bus.publish(GossipTopic::Telemetry, b"after_resub", NOW);
    ...
    fn gossip_empty_payload() {
        ...
        bus.publish(GossipTopic::Custom(0), b"", NOW);
        let msgs = bus.drain(id);
        assert_eq!(msgs.len(), 1);
    ...
    fn gossip_invalid_utf8_payload() {
        let mut bus = GossipBus::new();
        let id = bus.subscribe(GossipTopic::Error);
        bus.publish(GossipTopic::Error, &[0xFF, 0xFE, 0x80], NOW);
        let msgs = bus.drain(id);
    ...
    fn gossip_flood_100k_messages() {
        ...
        let id = bus.subscribe(GossipTopic::Telemetry);
        for i in 0..100_000 {
            bus.publish(GossipTopic::Telemetry, &telemetry_payload("msg", i as f64), NOW);
        }
    ...
    fn gossip_multiple_subscribers_same_topic() {
        ...
        bus.publish(GossipTopic::Prediction, b"test", NOW);
        for &s in &subs {
    ...
    fn gossip_node_register_drain_cycle() {
        ...
        bus.publish(GossipTopic::Telemetry, b"t1", NOW);
        bus.publish(GossipTopic::Resilience, b"r1", NOW);
        bus.publish(GossipTopic::Telemetry, b"t2", NOW);
        let msgs = node.drain(&mut bus);
    ...
    fn gossip_unsubscribe_from_unsubscribed_topic() {
        ...
        bus.publish(GossipTopic::Error, b"test", NOW);
    ...
    fn gossip_jamming_binary_payloads() {
        ...
        for payload in &jamming_payloads {
            bus.publish(GossipTopic::Telemetry, payload, NOW);
        }
        let msgs = bus.drain(id);
    ...
    fn gossip_spoofed_topic_injection() {
        ...
        bus.publish(GossipTopic::Telemetry, b"sensor_data", NOW);
        bus.publish(GossipTopic::Prediction, b"pred_data", NOW);
        bus.publish(GossipTopic::Error, b"err_data", NOW);
    ...
    fn gossip_topic_spoofing_via_wrong_topic() {
        ...
        bus.publish(GossipTopic::Prediction, b"should_not_leak", NOW);
        assert!(!bus.has_messages(id),
    ...
    fn gossip_injection_too_large_payload_truncated() {
        ...
        bus.publish(GossipTopic::Telemetry, &large, NOW);
        let msgs = bus.drain(id);
    ...
    fn gossip_penetration_rapid_sub_unsub() {
        ...
        let id = bus.subscribe(GossipTopic::Telemetry);
        bus.publish(GossipTopic::Telemetry, b"post_stress", NOW);
        assert!(bus.has_messages(id));
    }
    ...
    fn gossip_timestamp_monotonic_per_subscriber() {
        ...
        let id = bus.subscribe(GossipTopic::Telemetry);
        for i in 0..50 {
            bus.publish(GossipTopic::Telemetry, &telemetry_payload("idx", i as f64), 1000 + i as u64);
        }
        let msgs = bus.drain(id);
    ...
    fn gossip_jitter_burst_reception() {
        ...
        for _ in 0..100 {
            bus.publish(GossipTopic::Telemetry, b"burst", NOW);
        }
        for &id in &ids {
            let msgs = bus.drain(id);
