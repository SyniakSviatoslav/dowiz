...
//! ## Usage
//! ```
//! use dowiz_core::resonance::ResonanceDetector;
//!
...
impl ResonanceDetector {
    ...
    fn goertzel(&self, target_freq: f64) -> f64 {
        ...
        let coeff = 2.0 * crate::math::cos(omega);
        let mut s0 = 0.0;
        let mut s1 = 0.0;
        let mut s2 = 0.0;
    ...
    pub fn oscillation_strength(&self) -> Option<(f64, f64)> {
        ...
        let variance = self.buffer.iter().map(|x| crate::math::powi(x - mean, 2)).sum::<f64>() / n as f64;
...
mod tests {
    ...
    fn sine_wave_detected() {
        ...
        for i in 0..256 {
            let v = crate::math::sin(i as f64 * 2.0 * core::f64::consts::PI * 0.1);
            det.feed(v);
        }
    ...
    fn noise_does_not_crash() {
        let mut det = ResonanceDetector::new(64);
        for i in 0..200 {
            let v = crate::math::sin(i as f64 * 7.3) * 0.5 + crate::math::cos(i as f64 * 11.7) * 0.3;
            det.feed(v);
        }
    ...
    fn peak_frequencies_returns_top_k() {
        let mut det = ResonanceDetector::new(64);
        for i in 0..128 {
            let v = crate::math::sin(i as f64 * 0.2) + crate::math::cos(i as f64 * 0.05);
            det.feed(v);
        }
        let peaks = det.peak_frequencies(3);
        assert_eq!(peaks.len(), 3);
    ...
    fn detect_oscillation_helper() {
        ...
        for i in 0..200 {
            data.push(crate::math::sin(i as f64 * 0.3));
        }
    ...
    fn resonance_load_test_long_series() {
        ...
        for i in 0..10_000 {
            let v = crate::math::sin(i as f64 * 0.1) + crate::math::cos(i as f64 * 0.05);
            det.feed(v);
        }
    ...
    fn resonance_detect_oscillation_threshold() {
        let data: Vec<f64> = (0..200).map(|i| crate::math::sin(i as f64 * 0.3)).collect();
    ...
    fn resonance_goertzel_numerical_stability() {
        let mut det = ResonanceDetector::new(128);
        for i in 0..200 {
            let v = crate::math::sin(i as f64 * 2.0 * core::f64::consts::PI * 0.05);
            det.feed(v);
        }
    ...
    fn resonance_peak_frequencies_k_larger_than_bins() {
        let mut det = ResonanceDetector::new(64);
        for i in 0..128 {
            det.feed(crate::math::sin(i as f64 * 0.2));
        }
    ...
    fn resonance_jamming_pure_noise_no_panic() {
        ...
        for i in 0..1000 {
            let v = crate::math::sin(i as f64 * 7.0) * 0.5 + crate::math::cos(i as f64 * 13.0) * 0.5;
            det.feed(v);
        }
        let pg = det.periodogram();
    ...
    fn resonance_injection_frequency_sweep() {
        ...
        for i in 0..2000 {
            let phase = crate::math::sin(i as f64 * 0.01);
            let freq = 0.001 + (i as f64 / 2000.0) * 0.499;
            det.feed(crate::math::sin(phase * 6.2832 * freq));
        }
    ...
    fn resonance_time_critical_large_buffer_no_overflow() {
        ...
        for i in 0..100_000 {
            det.feed(crate::math::sin(i as f64 * 0.01));
        }
    ...
    fn resonance_jamming_alternating_nan_sine() {
        ...
        for i in 0..500 {
            if i % 2 == 0 {
                ...
                det.feed(crate::math::sin(i as f64 * 0.1));
            }
        }
        let pg = det.periodogram();
        assert!(pg.iter().all(|(_, m)| m.is_finite()),
