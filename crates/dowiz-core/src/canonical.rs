...
//! - strings are escaped with a single authority (a local `escape_into`,
//!   inlined byte-identically from the kernel's `fdr::json` escaper).
//!
...
/// Escape `s` into `out` — the single string-escaping authority. Only
/// `" \ \n \r \t` are escaped; every other char passes through verbatim.
/// Byte-identical to the kernel's `fdr::json::escape_into` (inlined here so
/// this no_std module stays self-contained).
