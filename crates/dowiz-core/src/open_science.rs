#![allow(unused)]
//! `dowiz_core::open_science`.rs — Open Science Desktop reimplementation: AI research workbench.
//!
//! # What this is
...
impl ResearchWorkbench {
    ...
    fn audit(&mut self, action: &str, target: Option<u64>, actor: &str, now_us: u64) {
    ...
    pub fn add_paper(&mut self, title: &str, abstract_text: &str, categories: &[String], arxiv_id: &str, now_us: u64) -> u64 {
        ...
        self.audit("paper_added", Some(id), "workbench", now_us);
        id
    }
    ...
    pub fn create_notebook(&mut self, title: &str, content: &str, tags: Vec<String>, related_papers: Vec<u64>, now_us: u64) -> u64 {
        ...
        self.audit("notebook_created", Some(id), "workbench", now_us);
        id
    }
    ...
    pub fn generate_report(&mut self, title: &str, sections: Vec<(String, String)>, reference_paper_ids: Vec<u64>, now_us: u64) -> u64 {
        ...
        self.audit("report_generated", Some(id), "workbench", now_us);
        id
    }
...
mod tests {
    ...
    fn search_papers_finds_by_keyword() {
        let mut wb = make_workbench();
        wb.add_paper("Machine Learning Basics", "abstract about ML", &["cs.LG".to_string()], "2101.00101", NOW);
        wb.add_paper("Deep Neural Networks", "abstract about DNN", &["cs.LG".to_string()], "2101.00202", NOW);
        wb.add_paper("Quantum Computing", "abstract about QC", &["quant-ph".to_string()], "2101.00303", NOW);
    ...
    fn search_notebooks_by_tag() {
        let mut wb = make_workbench();
        wb.create_notebook("A", "content", vec!["ml".to_string()], vec![], NOW);
        wb.create_notebook("B", "content", vec!["physics".to_string()], vec![], NOW);
        wb.create_notebook("C", "content", vec!["ml".to_string(), "deep".to_string()], vec![], NOW);
    ...
    fn audit_log_records_actions() {
        let mut wb = make_workbench();
        wb.add_paper("P1", "abstract", &[], "2101.001", NOW);
        wb.create_notebook("N1", "content", vec![], vec![], NOW);
        wb.generate_report("R1", vec![], vec![], NOW);
...
"#;
    ...
    fn related_papers_stored_in_notebook() {
        let mut wb = make_workbench();
        let paper_id = wb.add_paper("Related", "abstract", &[], "2101.001", NOW);
    ...
    fn report_includes_references() {
        let mut wb = make_workbench();
        let paper1 = wb.add_paper("Paper 1", "abstract1", &[], "2101.001", NOW);
        let paper2 = wb.add_paper("Paper 2", "abstract2", &[], "2101.002", NOW);
