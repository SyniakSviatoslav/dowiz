...
impl ObservabilityHub {
    ...
    /// Take a telemetry snapshot from caller-supplied samples + timestamp (the no_std
    /// form — the kernel shim's `snapshot_now` samples `/proc` and stamps the wall clock).
    pub fn snapshot(
        &mut self,
        timestamp_us: u64,
        cpu: Option<crate::typed_metrics::ProcCpuSample>,
        mem: Option<crate::typed_metrics::MemSample>,
...
mod tests {
    ...
    fn hub_record_metric_and_snapshot() {
        ...
        let snap = hub.snapshot(0, None, None);
    ...
    fn hub_clear_resets_state() {
        ...
        assert_eq!(hub.snapshot(0, None, None).total_events, 0);
    }
