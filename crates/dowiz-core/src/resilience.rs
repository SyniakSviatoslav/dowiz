#![allow(unused)]
//! `dowiz_core::resilience` — backup, failover, dynamic switching (pure core).
//!
//! Timestamp-injected: `BackupState::new`, `record_outcome(s)`, `record_failover`,
//! `quick_failover`, `quick_backup` take `now_ms` explicitly; the kernel shim stamps it.
//!
...
impl BackupState {
    pub fn new(metrics: Vec<f64>, label: &str, now_ms: u64) -> Self {
        let ts = now_ms;
        let id = ts;
...
impl ResilienceManager {
    ...
    pub fn record_outcome(&mut self, avg_metric: f64, friction: f64, error_prob: f64, now_ms: u64) -> FailoverStrategy {
    ...
    pub fn record_outcomes(&mut self, outcomes: &[PredictedOutcome], now_ms: u64) -> FailoverStrategy {
        ...
        self.record_outcome(avg_metric, avg_friction, avg_error, now_ms)
    }
    ...
    pub fn record_failover(&mut self, now_ms: u64) {
...
pub fn quick_failover(outcomes: &[PredictedOutcome], now_ms: u64) -> FailoverStrategy {
    let mut mgr = ResilienceManager::new(ResiliencePolicy::default());
    mgr.record_outcomes(outcomes, now_ms)
}

/// Quick backup: create a backup snapshot from a metrics vector.
pub fn quick_backup(metrics: Vec<f64>, now_ms: u64) -> BackupState {
    BackupState::new(metrics, "quick_backup", now_ms)
}
...
mod tests {
    ...
    fn resilience_manages_levels() {
        ...
        let strategy = mgr.record_outcome(0.8, 0.7, 0.8, NOW);
    ...
    fn backup_verify_passes() {
        let b = BackupState::new(vec![0.5, 0.3, 0.1], "test", NOW);
        assert!(b.verify());
    }

    #[test]
    fn backup_verify_fails_on_corruption() {
        let mut b = BackupState::new(vec![0.5, 0.3, 0.1], "test", NOW);
    ...
    fn backup_store_roundtrip() {
        ...
        store.store(BackupState::new(vec![1.0, 2.0], "first", NOW));
        store.store(BackupState::new(vec![3.0, 4.0], "second", NOW));
    ...
    fn circuit_breaker_trigger() {
        ...
        mgr.record_outcome(0.8, 0.7, 0.8, NOW); // Critical
        assert!(mgr.should_open_circuit());
    }
    ...
    fn consecutive_failures_escalate() {
        let mut mgr = ResilienceManager::new(ResiliencePolicy::default());
        for _ in 0..5 {
            mgr.record_outcome(0.8, 0.7, 0.8, NOW); // Each record is Critical
        }
    ...
    fn record_outcomes_from_predictions() {
        ...
        let mut mgr = ResilienceManager::new(ResiliencePolicy::default());
        let _strategy = mgr.record_outcomes(&outcomes, NOW);
    ...
    fn quick_failover_convenience() {
        ...
        let strategy = quick_failover(&outcomes, NOW);
    ...
    fn reset_clears_state() {
        let mut mgr = ResilienceManager::new(ResiliencePolicy::default());
        mgr.record_outcome(0.8, 0.7, 0.8, NOW);
        assert_eq!(mgr.level(), DegradationLevel::Critical);
        mgr.reset();
        assert_eq!(mgr.level(), DegradationLevel::Normal);
    ...
    fn resilience_nan_inputs_safe() {
        let mut mgr = ResilienceManager::new(ResiliencePolicy::default());
        let strategy = mgr.record_outcome(f64::NAN, f64::INFINITY, f64::NEG_INFINITY, NOW);
    ...
    fn resilience_rapid_level_oscillation() {
        ...
        for i in 0..100 {
            if i % 2 == 0 {
                mgr.record_outcome(0.95, 0.95, 0.95, NOW); // Failed
            } else {
                mgr.record_outcome(0.05, 0.05, 0.05, NOW); // Normal
            }
        }
    ...
    fn resilience_max_consecutive_failures_exceeded() {
        ...
        for _ in 0..5 {
            mgr.record_outcome(0.95, 0.95, 0.95, NOW);
        }
    ...
    fn resilience_empty_outcomes_list() {
        let mut mgr = ResilienceManager::new(ResiliencePolicy::default());
        let strategy = mgr.record_outcomes(&[], NOW);
    ...
    fn backup_store_capacity_overflow() {
        ...
        for i in 0..100 {
            store.store(BackupState::new(vec![i as f64], "load", NOW));
        }
    ...
    fn backup_verify_corrupted_checksum() {
        let b = BackupState::new(vec![1.0, 2.0, 3.0], "test", NOW);
        assert!(b.verify());
    ...
    fn quick_failover_empty_outcomes() {
        let strategy = quick_failover(&[], NOW);
    ...
    fn resilience_cascade_single_bad_prediction() {
        ...
        mgr.record_outcome(0.95, 0.9, 0.5, NOW);
        ...
        for _ in 0..20 {
            mgr.record_outcome(0.95, 0.9, 0.8, NOW);
        }
        assert!(mgr.level() >= DegradationLevel::Warning,
    ...
    fn resilience_isolation_good_after_bad() {
        ...
        for i in 0..40 {
            if i % 2 == 0 {
                mgr.record_outcome(0.9, 0.8, 0.9, NOW); // bad
            } else {
                mgr.record_outcome(0.1, 0.05, 0.02, NOW); // good
            }
        }
    ...
    fn resilience_circuit_breaker_chain_isolation() {
        ...
        for _ in 0..100 {
            mgr1.record_outcome(0.99, 0.99, 0.99, NOW);
        }
        // mgr2 must remain normal
        mgr2.record_outcome(0.2, 0.1, 0.05, NOW);
    ...
    fn resilience_jamming_nan_inf_outcomes() {
        ...
        mgr.record_outcome(f64::NAN, f64::NAN, f64::NAN, NOW);
        mgr.record_outcome(f64::INFINITY, f64::INFINITY, f64::INFINITY, NOW);
        mgr.record_outcome(-0.0, -0.0, -0.0, NOW);
    ...
    fn resilience_record_outcomes_nan_list() {
        ...
        let strategy = mgr.record_outcomes(&outcomes, NOW);
    ...
    fn resilience_should_recover_after_cooldown() {
        ...
        // Drive to critical
        for _ in 0..50 {
            mgr.record_outcome(0.99, 0.99, 0.99, NOW);
        }
        mgr.record_failover(NOW);
        let now = NOW;
        assert!(mgr.should_recover(now + 100),
    ...
    fn resilience_failover_strategy_escalates() {
        ...
        // Normal state
        assert_eq!(mgr.record_outcome(0.2, 0.1, 0.05, NOW), FailoverStrategy::EnsembleReduced);
        // Escalate to warning
        for _ in 0..10 {
            mgr.record_outcome(0.8, 0.7, 0.6, NOW);
        }
