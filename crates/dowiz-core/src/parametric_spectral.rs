#![allow(unused)]
//! `parametric_spectral` — Parametric Surface Spectral Library.
//!
//! # Architecture
...
impl Spin {
    ...
    pub fn distance(&self, other: &Spin) -> f32 {
        ...
        crate::math::sqrt((du * du + dv * dv) as f64) as f32
    }
        ...
        for dc in [-1i32, 0, 1] {
            for dr in [-1i32, 0, 1] {
                ...
                for (idx, _spin) in self.grid[cell].iter().enumerate() {
                    let du = query_u - _spin.u;
                    let dv = query_v - _spin.v;
                    let d = crate::math::sqrt((du * du + dv * dv) as f64) as f32;
