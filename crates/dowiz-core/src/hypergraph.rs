...
impl Hypergraph {
    ...
    pub fn vertex_centrality(&self) -> Vec<f64> {
        ...
        for _ in 0..iterations {
            ...
            // Normalize
            let norm = crate::math::sqrt(result.iter().map(|x| x * x).sum::<f64>());
            if norm < tolerance {
                break;
            }
            for v in 0..n {
    ...
    pub fn spectral_embedding_2d(&self) -> Vec<(f64, f64)> {
        ...
        // Normalize both
        let norm1 = crate::math::sqrt(centrality.iter().map(|x| x * x).sum::<f64>());
        ...
        let norm2 = crate::math::sqrt(second_vec.iter().map(|x| x * x).sum::<f64>());
