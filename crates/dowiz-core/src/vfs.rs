...
//! vfs.rs — virtual filesystem seam (no_std migration enabler).
//!
//! The no_std core: [`Vfs`] trait + [`VfsError`] + [`DirEntry`] + [`Metadata`] +
//! [`VfsFile`] trait + free functions (bodies cfg-gated).
//! Std-gated: [`StdFs`] impl, [`StdFile`] impl, [`open_file`], `From`/`Error` bridges,
//! `map_io_err`. no_std fallback: free functions return `Err(VfsError::NotFound)`.
...
// compiling unchanged: `?` on a `Result<_, VfsError>` auto-converts.
#[cfg(feature = "std")]
...
mod std_impls {
    ...
    pub(crate) fn map_io_err(e: &std::io::Error) -> VfsError {
        use std::io::ErrorKind::*;
        match e.kind() {
    ...
    pub fn open_file(path: &str, mode: OpenMode) -> Result<StdFile, VfsError> {
        ...
        let f = o.open(path).map_err(|e| map_io_err(&e))?;
        Ok(StdFile { inner: f })
    }
}

#[cfg(feature = "std")]
pub use std_impls::{open_file, StdFile, StdFs};

// ── Free functions — the single authority (call sites route through these) ──
//
// Paths are `&str` (no_std-compatible). Bodies delegate to StdFs when `feature =
// "std"` is active; on no_std they return `Err(VfsError::NotFound)`.

/// Read a whole file to bytes.
pub fn read(path: &str) -> Result<Vec<u8>, VfsError> {
    #[cfg(feature = "std")]
    {
        std_impls::StdFs.read(path)
    }
    #[cfg(not(feature = "std"))]
    {
...
pub fn read_to_string(path: &str) -> Result<String, VfsError> {
    #[cfg(feature = "std")]
    {
        std_impls::StdFs.read_to_string(path)
    }
    #[cfg(not(feature = "std"))]
    {
        Err(VfsError::NotFound)
...
pub fn write(path: &str, contents: &[u8]) -> Result<(), VfsError> {
    #[cfg(feature = "std")]
    {
        std_impls::StdFs.write(path, contents)
    }
    #[cfg(not(feature = "std"))]
    {
        Err(VfsError::NotFound)
...
pub fn append(path: &str, contents: &[u8]) -> Result<(), VfsError> {
    #[cfg(feature = "std")]
    {
        std_impls::StdFs.append(path, contents)
    }
    #[cfg(not(feature = "std"))]
    {
        Err(VfsError::NotFound)
...
/// `mkdir -p`.
pub fn create_dir_all(path: &str) -> Result<(), VfsError> {
    #[cfg(feature = "std")]
    {
        std_impls::StdFs.create_dir_all(path)
    }
    #[cfg(not(feature = "std"))]
    {
        Err(VfsError::NotFound)
...
pub fn remove_file(path: &str) -> Result<(), VfsError> {
    #[cfg(feature = "std")]
    {
        std_impls::StdFs.remove_file(path)
    }
    #[cfg(not(feature = "std"))]
    {
        Err(VfsError::NotFound)
...
pub fn remove_dir_all(path: &str) -> Result<(), VfsError> {
    #[cfg(feature = "std")]
    {
        std_impls::StdFs.remove_dir_all(path)
    }
    #[cfg(not(feature = "std"))]
    {
        Err(VfsError::NotFound)
...
pub fn rename(from: &str, to: &str) -> Result<(), VfsError> {
    #[cfg(feature = "std")]
    {
        std_impls::StdFs.rename(from, to)
    }
    #[cfg(not(feature = "std"))]
    {
        Err(VfsError::NotFound)
