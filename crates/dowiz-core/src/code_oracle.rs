#![allow(unused)]
//! `dowiz_core::code_oracle` — code change prediction + ETA oracle (pure core).
//!
...
//!
//! ZERO deps. The wall-clock-stamped entry points (`ChangeRecord::new`,
//! `EtaOracle::record`) take `now_ms` explicitly so this module stays `no_std`;
//! the kernel shim wraps them as free functions.
...
impl ChangeRecord {
    pub fn new(modules: &[&str], added: u64, removed: u64, eta_min: f64, now_ms: u64) -> Self {
...
impl EtaOracle {
    ...
    /// Record a completed change at the explicit `now_ms` timestamp.
    pub fn record(&mut self, modules: &[&str], added: u64, removed: u64, eta_minutes: f64, now_ms: u64) {
        let record = ChangeRecord::new(modules, added, removed, eta_minutes, now_ms);
        ...
        let mut snap_values = BTreeMap::new();
        ...
        self.chronos.snapshot_at(snap_values, now_ms);
    ...
    pub fn eta_confidence(&self) -> (f64, f64) {
        ...
        (mean_err, crate::math::sqrt(var_err))
    }
...
mod tests {
    ...
    fn oracle_records_and_predicts() {
        ...
        oracle.record(&["predictor.rs"], 48, 36, 10.0, 1000);
        oracle.record(&["predictor.rs", "lib.rs"], 120, 80, 20.0, 2000);
        oracle.record(&["predictor.rs"], 30, 15, 5.0, 3000);
        oracle.record(&["lib.rs", "pid.rs", "gossip.rs"], 200, 150, 30.0, 4000);
    ...
    fn oracle_confidence_is_finite() {
        let mut oracle = EtaOracle::new();
        for i in 0..10 {
            oracle.record(&["test.rs"], 10 + i*5, 5 + i*3, (i + 1) as f64 * 3.0, 1000 + i as u64);
        }
    ...
    fn impact_prediction_finite() {
        ...
        oracle.record(&["a.rs", "b.rs"], 200, 100, 30.0, 2000);
    ...
    fn dashboard_renders() {
        let mut oracle = EtaOracle::new();
        oracle.record(&["test.rs"], 10, 5, 2.0, 1000);
    ...
    fn oracle_predict_impact_without_similar_falls_back() {
        let mut oracle = EtaOracle::new();
        oracle.record(&["a.rs"], 100, 50, 15.0, 1000);
        oracle.record(&["a.rs"], 200, 100, 30.0, 2000);
        oracle.record(&["b.rs"], 50, 25, 8.0, 3000);
    ...
    fn change_record_total_lines() {
        let rec = ChangeRecord::new(&["a.rs"], 100, 50, 15.0, 1000);
