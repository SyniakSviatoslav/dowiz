...
/// Pure-alloc in-memory archival stand-in (coldest tier). Demonstrates the ONLY
...
mod tests {
    ...
    fn optical_compressed_cannot_reach_determinism_plane() {
        ...
        //   - it does NOT implement `core::hash::Hash` (so it cannot be a BTreeMap/
            ...
            T: ?Sized,
        {
            ...
            trait IsHash {}
            impl<T: core::hash::Hash> IsHash for T {}
