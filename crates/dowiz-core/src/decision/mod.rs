//! decision/mod.rs — no_std core MoE mesh DecisionUnit family (BLUEPRINT-P-F, Layer F).
//!
...
//!
//! Host-only seam: the FDR-shadow telemetry method `decide_with_shadow` (item 51) is deliberately
//! NOT in this core — it is the ONLY `std`/kernel-side dependency (it writes `crate::fdr::ring`
//! records). It lives as a free function in the kernel shim `dowiz-kernel/src/decision/mod.rs`.
