#![allow(unused)]
//! `dowiz_core::tracker` — comprehensive error tracking, deterministic logging,
//! telemetry, reverse replay, inverse simulation (pure core).
//!
//! Timestamp-injected: `TrackedError::new` / `LoggedEvent::new` take `now_ms`;
//! the kernel shim stamps it.
//!
...
impl TrackedError {
    pub fn new(module: &'static str, kind: &'static str, message: impl Into<String>, now_ms: u64) -> Self {
        TrackedError {
            module,
            kind,
...
impl LoggedEvent {
    pub fn new(module: &'static str, event_type: &'static str, payload: Vec<u8>, now_ms: u64) -> Self {
        static EVENT_SEQ: AtomicU64 = AtomicU64::new(1);
        let ts = now_ms;
        let checksum = crate::checksum::checksum_fold(&payload);
        LoggedEvent {
    ...
    pub fn verify(&self) -> bool {
        crate::checksum::checksum_fold(&self.payload) == self.checksum
    }
}
    ...
    fn evaluate(&self, input: &[f64], observed: &[f64]) -> f64 {
        match self.forward_model {
            Some(model) => {
                ...
                output.iter().zip(observed).map(|(a, b)| { let d = a - b; d * d }).sum()
            }
            None => {
                // Without a forward model, assume identity (input ≈ output)
                input.iter().zip(observed).map(|(a, b)| { let d = a - b; d * d }).sum()
            }
        }
    }
}
...
mod tests {
    ...
    fn tracked_error_basic() {
        let err = TrackedError::new("pid", "overflow", "integral exceeded max", NOW);
    ...
    fn tracked_error_chain() {
        let cause = TrackedError::new("sensor", "jamming", "NaN received", NOW);
        let err = TrackedError::new("resilience", "failover", "circuit opened", NOW)
            .with_cause(cause);
    ...
    fn tracked_error_critical_classification() {
        let e1 = TrackedError::new("test", "panic", "oops", NOW);
        assert!(e1.is_critical());
        let e2 = TrackedError::new("test", "timeout", "slow", NOW);
        assert!(!e2.is_critical());
    }

    #[test]
    fn tracked_error_unique_sequence() {
        let a = TrackedError::new("a", "x", "first", NOW);
        let b = TrackedError::new("b", "y", "second", NOW);
    ...
    fn event_log_basic_record_and_iter() {
        ...
        log.record(LoggedEvent::new("test", "info", b"hello".to_vec(), NOW));
        log.record(LoggedEvent::new("test", "warn", b"world".to_vec(), NOW));
        assert_eq!(log.len(), 2);
    ...
    fn event_log_reverse_iter() {
        let mut log = EventLog::new(100);
        for i in 0..5 {
            log.record(LoggedEvent::new("t", "ev", vec![i], NOW));
        }
    ...
    fn event_log_integrity() {
        let mut log = EventLog::new(100);
        log.record(LoggedEvent::new("m1", "data", b"valid".to_vec(), NOW));
        log.record(LoggedEvent::new("m2", "data", b"also valid".to_vec(), NOW));
    ...
    fn event_log_filter_by_module() {
        let mut log = EventLog::new(100);
        log.record(LoggedEvent::new("pid", "update", vec![1], NOW));
        log.record(LoggedEvent::new("gossip", "pub", vec![2], NOW));
        log.record(LoggedEvent::new("pid", "tune", vec![3], NOW));
    ...
    fn event_log_reverse_replay_to_unwind() {
        let mut log = EventLog::new(100);
        for i in 0..20 {
            log.record(LoggedEvent::new("t", "state", vec![i], NOW));
        }
        let events: Vec<_> = log.iter().collect();
    ...
    fn tracker_event_log_integration_with_errors() {
        let mut log = EventLog::new(100);
        let err = TrackedError::new("predictor", "NaN_input", "metric 2 is NaN", NOW);
        log.record(LoggedEvent::new("predictor", "error", err.message.as_bytes().to_vec(), NOW));
        assert_eq!(log.len(), 1);
        assert!(log.verify_all());
    ...
    fn tracker_full_cycle_error_log_replay() {
        ...
        let root = TrackedError::new("sensor", "jamming", "spike detected", NOW);
        let err = TrackedError::new("resilience", "failover", "circuit breaker opened", NOW)
            .with_cause(root);

        event_log.record(LoggedEvent::new(
            err.module, "circuit_breaker",
            err.message.as_bytes().to_vec(), NOW));
