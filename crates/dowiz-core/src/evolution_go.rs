...
///
/// `std::time::SystemTime` is the only wall-clock source the kernel can reach
/// without an external dependency, and it does not exist in `core`/`alloc`.
/// Under the `std` feature this returns the real epoch microseconds; under
/// `no_std` there is no clock, so it returns `0` (the caller must supply a real
/// timestamp through a clock seam instead of trusting this value).
///
/// innovate: no_std fallback fabricates `0` for `timestamp_us`/`paired_at_us`.
/// Upgrade trigger: thread a clock trait (e.g. `Fn() -> u64`) through
/// `EvolutionGo::new` so no_std embedders supply a real epoch source, honoring
/// "named absence" (never fabricate 0).
fn now_micros() -> u64 {
    #[cfg(feature = "std")]
    {
    ...
    pub fn confirm_qr_pairing(&mut self, session_id: u64) -> Result<(), SessionError> {
        ...
        match session.state {
            SessionState::AwaitingQrScan | SessionState::QrScanned => {
                ...
                session.paired_at_us = Some(now_micros());
                Ok(())
            }
