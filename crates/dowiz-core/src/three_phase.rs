...
/// Monotonic millisecond clock for `no_std`. `std::time::SystemTime` is
/// unavailable in `no_std`; `phase_timings_ms` is diagnostic only, so an
/// atomic counter that advances once per `record()` call is an adequate
/// monotonic stand-in.
...
impl ThreePhaseVerifier {
    ...
    fn record(&mut self, verdict: Verdict, phase: u8) -> Verdict {
        ...
        self.phase_timings_ms[phase as usize - 1] = now_ms();
        match verdict {
            Verdict::Pass => {
