...
//!
//! Timestamps (`last_success_us` / `last_failure_us`) are injected as explicit
//! `now_us_val: u64` parameters — the no_std core has no wall clock; the kernel
//! held-handle shim provides `now_us()` and callers pass it in.
...
impl ProxyEndpoint {
    ...
    /// Record a successful request (`now_us_val` = injected monotonic timestamp).
    pub fn record_success(&mut self, latency_us: u64, now_us_val: u64) {
        self.consecutive_failures = 0;
        self.total_requests += 1;
        self.total_successes += 1;
        self.last_success_us = now_us_val;
    ...
    /// Record a failed request (`now_us_val` = injected monotonic timestamp).
    pub fn record_failure(&mut self, now_us_val: u64) {
        self.consecutive_failures += 1;
        self.total_requests += 1;
        self.last_failure_us = now_us_val;
    }
}
...
impl ProxyPool {
    ...
    pub fn record_success(&mut self, index: usize, latency_us: u64, now_us_val: u64) {
        if let Some(ep) = self.endpoints.get_mut(index) {
            ep.record_success(latency_us, now_us_val);
        }
    }

    /// Record a failure for the proxy at the given index.
    pub fn record_failure(&mut self, index: usize, now_us_val: u64) {
        if let Some(ep) = self.endpoints.get_mut(index) {
            ep.record_failure(now_us_val);
        }
    }
...
mod tests {
    ...
    fn record_failure_increments_consecutive() {
        ...
        assert_eq!(ep.last_failure_us, 2);
