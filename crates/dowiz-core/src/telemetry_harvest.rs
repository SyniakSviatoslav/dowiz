...
impl HarvestRecord {
    pub fn new(model: &str, task: &str, success: bool, value: f64, cost: f64, now_ms: u64) -> Self {
        HarvestRecord {
...
impl HarvestLedger {
    ...
    pub fn record(
        ...
        let rec = HarvestRecord::new(model, task, success, value, cost, now_ms);
...
use alloc::vec::Vec;
    ...
    fn test_harvest_record_roundtrip() {
        let rec = HarvestRecord::new("enrich", "lookup", true, 0.85, 1.2, NOW);
    ...
    fn test_harvest_record_sanitize_nan_value() {
        let rec = HarvestRecord::new("test", "nan_check", false, f64::NAN, 5.0, NOW);
        assert_eq!(rec.value, 0.0, "NaN value must be sanitized to 0.0");
    }

    #[test]
    fn test_harvest_record_sanitize_negative_cost() {
        let rec = HarvestRecord::new("test", "neg_cost", true, 0.5, -3.0, NOW);
    ...
    fn test_ev_score_monotonic() {
        ...
        for _ in 0..10 {
            ledger.record("test", "good", true, 0.9, 1.0, NOW);
        }
        ...
        ledger.record("test", "bad", false, 0.0, 100.0, NOW);
    ...
    fn test_ev_score_all_failures() {
        let mut ledger = HarvestLedger::new(100);
        for _ in 0..5 {
            ledger.record("test", "fail", false, 1.0, 1.0, NOW);
        }
    ...
    fn test_ev_score_zero_cost() {
        let mut ledger = HarvestLedger::new(100);
        ledger.record("test", "free", true, 1.0, 0.0, NOW);
    ...
    fn test_track_to_jsonl_format() {
        let mut ledger = HarvestLedger::new(100);
        ledger.record("enrich", "lookup", true, 0.85, 1.2, NOW);
        ledger.record("intent", "detect", true, 0.92, 0.5, NOW);
    ...
    fn test_harvest_truncation() {
        ...
        for i in 0..(max + 100) {
            ledger.record("test", "overflow", i % 2 == 0, i as f64 * 0.01, 1.0, NOW);
        }
        assert_eq!(
            ledger.len(),
            max,
    ...
    fn test_harvest_truncation_fifo_order() {
        let max = 3;
        let mut ledger = HarvestLedger::new(max);
        ledger.record("m", "t", true, 1.0, 1.0, NOW); // evicted
        ledger.record("m", "t", true, 2.0, 1.0, NOW); // evicted
        ledger.record("m", "t", true, 3.0, 1.0, NOW); // kept
        ledger.record("m", "t", true, 4.0, 1.0, NOW); // kept
        ledger.record("m", "t", true, 5.0, 1.0, NOW); // kept
    ...
    fn test_harvest_report_mixed() {
        let mut ledger = HarvestLedger::new(100);
        ledger.record("a", "x", true, 0.8, 2.0, NOW);
        ledger.record("a", "x", false, 0.0, 10.0, NOW);
        let report = ledger.report();
