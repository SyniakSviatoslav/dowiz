...
impl SpectralGraph {
    ...
    pub fn normalized_laplacian(&mut self) -> &Tensor2 {
        ...
        for i in 0..self.n {
            ...
            deg_inv_sqrt[i] = if d > 0.0 { 1.0 / crate::math::sqrt(d) } else { 0.0 };
        }
        for i in 0..self.n {
            for j in 0..self.n {
                let val = if i == j {
    ...
    pub fn eigenvector_centrality(&self, iterations: usize) -> Vec<f64> {
        ...
        let mut central = vec![1.0 / crate::math::sqrt(self.n as f64); self.n];
        for _ in 0..iterations {
            ...
            let norm: f64 = crate::math::sqrt(next.iter().map(|x| x * x).sum::<f64>());
            if norm > 0.0 {
                for x in &mut next {
                    *x /= norm;
                }
